CREATE procedure dbo.xsp_get_next_unique_code_for_table_by_branch
(
		...
		begin
			...
			begin
				...
				begin
					...
					set @run_length += (len(@sys_brc_code) + len(@sys_doc_code) + len(@p_year) + len(@p_month) + @dot_a) + 1 ;
					set @year_length += (len(@sys_brc_code) + len(@sys_doc_code) + @dot_b) + 1 ;
					set @month_length += (len(@sys_brc_code) + len(@sys_doc_code) + len(@p_year) + @dot_b) + 1 ;
				end ;
				else
				begin
					if @p_delimiter <> ''
