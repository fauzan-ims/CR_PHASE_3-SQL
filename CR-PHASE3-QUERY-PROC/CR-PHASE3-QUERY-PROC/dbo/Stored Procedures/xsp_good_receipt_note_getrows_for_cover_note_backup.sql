CREATE PROCEDURE dbo.xsp_good_receipt_note_getrows_for_cover_note_backup
(
...
begin
	...
	where	grn.status			  = 'POST'
	and		pri.category_type = 'ASSET'
	and		pr.procurement_type = 'PURCHASE'
	and		grnd.receive_quantity <> 0
	and		grn.cover_note_status = case @p_cover_note_status
									 when 'ALL' then grn.cover_note_status
			...
			and (
						...
						or	grn.cover_note_status							like 	'%'+@p_keywords+'%'
		...
		order by	case
					when @p_sort_by = 'asc' then case @p_order_by
						...
						when 5	then grn.remark
						when 6	then grn.cover_note_status
					end
				end asc
				,case
