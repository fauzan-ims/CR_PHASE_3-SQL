CREATE PROCEDURE [dbo].[xsp_reverse_new_journal_multiple_final]
(
	@p_code			   nvarchar(50)
...
begin
	declare @msg							  nvarchar(max)
			...
			,@asset							  nvarchar(50)
			,@code_asset_expense			  nvarchar(50)
	...
	begin try
		...
		begin --insert journal FINAL GRN
			...
			begin
				...
				select	@code_asset_expense = podo.asset_code
				from	dbo.final_good_receipt_note						fgrn
						left join dbo.final_good_receipt_note_detail	fgrnd on (fgrnd.final_good_receipt_note_code  = fgrn.code)
						left join dbo.good_receipt_note_detail			grnd on (grnd.id							  = fgrnd.good_receipt_note_detail_id)
						left join dbo.good_receipt_note					grn on (grn.code							  = grnd.good_receipt_note_code)
				...
				begin
					...
					set @transaction_name = N'Reverse New Final Good Receipt Note ' + @p_final_grn_code + N' From PO ' + @po_no + N'.' + N' Vendor ' + @vendor_name ;
					...
					exec dbo.xsp_ifinproc_interface_journal_gl_link_transaction_insert @p_code							= @journal_final output
																					   ...
																					   ,@p_transaction_name				= 'Reverse New Final Good Receipt Note'
					...
					from		dbo.master_transaction_parameter				mtp
								...
								--															and	 aird.purchase_order_id		  = pod.id
								--														)
								--inner join dbo.ap_invoice_registration			air on (
								--														   air.code							  = aird.invoice_register_code
								--														   and	air.STATUS					  = 'POST'
								--													   )
								left join dbo.supplier_selection_detail			ssd on (ssd.id								  = pod.supplier_selection_detail_id)
								left join dbo.quotation_review_detail			qrd on (qrd.id								  = ssd.quotation_detail_id)
								inner join dbo.procurement						prc on (prc.code collate Latin1_General_CI_AS = isnull(qrd.reff_no, ssd.reff_no))
								inner join dbo.procurement_request				pr on (pr.code								  = prc.procurement_request_code)
					where		mtp.process_code		  = 'S240400002'
								and grnd.receive_quantity <> 0
					...
					while @@fetch_status = 0
					begin
						set @asset = isnull(isnull(@asset_code, @asset_code_final),@code_asset_expense) ;
						...
						begin
							...
							begin
								set @orig_amount_cr = @return_value--0 ;
								set @orig_amount_db = 0--@return_value ;
							end ;
							else
							begin
								set @orig_amount_cr = 0--abs(@return_value) ;
								set @orig_amount_db = abs(@return_value)--0 ;
							end ;
						end ;
						...
						begin
							if @return_value <> 0
							begin
								set @remarks_journal = 'Reverse ' + isnull(@transaction_name, '') + N' ' + isnull(convert(nvarchar(3), @recive_quantity), '') + N' ' + isnull(@uom_name, '') + N' ' + isnull(@item_name_for_jrnl, '') + N'. PO No : ' + isnull(@po_no, '') ;
								...
								begin
									...
									begin
										set @remarks_journal = 'Reverse ' + isnull(@transaction_name, '') + N' ' + isnull(@item_name_for_jrnl, '') + N'. PO No : ' + isnull(@po_no, '') ;
