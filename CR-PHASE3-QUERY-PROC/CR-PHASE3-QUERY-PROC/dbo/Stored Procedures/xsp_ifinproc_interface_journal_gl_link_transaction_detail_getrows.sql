CREATE PROCEDURE [dbo].[xsp_ifinproc_interface_journal_gl_link_transaction_detail_getrows]
(
...
begin
	...
	from	dbo.ifinproc_interface_journal_gl_link_transaction_detail jtd
	left join dbo.journal_gl_link jgl on (jgl.code = jtd.gl_link_code)
	--left join dbo.master_item_group_gl igl on (igl.gl_asset_code = jtd.gl_link_code)
	where	jtd.gl_link_transaction_code = @p_gl_link_transaction_code
		...
		select		jtd.id
					,jtd.branch_name
					,jtd.gl_link_code
					,jtd.gl_link_code  'gl_link_name'
					--,isnull(jgl.gl_link_name,igl.gl_asset_name) 'gl_link_name'
					,jtd.remarks
					,orig_currency_code
					,jtd.base_amount_db
					,jtd.base_amount_cr
		...
		where		jtd.gl_link_transaction_code = @p_gl_link_transaction_code
					and	(
							jtd.branch_name					like '%' + @p_keywords + '%'
							or	jtd.gl_link_code			like '%' + @p_keywords + '%'
							or	orig_currency_code			like '%' + @p_keywords + '%'
							or	jtd.base_amount_cr			like '%' + @p_keywords + '%'
							or	jtd.base_amount_db			like '%' + @p_keywords + '%'
							or	jtd.orig_amount_cr			like '%' + @p_keywords + '%'
		...
		order by	case
					when @p_sort_by = 'asc' then case @p_order_by
														...
														when 4 then jtd.orig_currency_code
														when 5 then cast(jtd.base_amount_db as sql_variant)
														when 6 then cast(jtd.base_amount_cr as sql_variant)
														when 7 then jtd.division_name
												 end
					end asc
					,case
