CREATE PROCEDURE [dbo].[xsp_agreement_main_lookup_for_waive]
(
	@p_keywords		nvarchar(50)
	,@p_pagenumber	int
	,@p_rowspage	int
	,@p_order_by	int
	,@p_sort_by		nvarchar(5)
	--
	,@p_branch_code nvarchar(50)
	,@p_client_no	nvarchar(50) = '' -- Louis Senin, 07 Juli 2025 11.31.01 -- 
)
as
begin
	declare @rows_count int = 0 ;

	if exists
	(
		select	1
		from	sys_global_param
		where	code	  = 'HO'
				and value = @p_branch_code
	)
	begin
		set @p_branch_code = 'ALL' ;
	end ;

	select	@rows_count = count(1)
	from	agreement_main
	where	((agreement_status = 'GO LIVE') or 
			(
				agreement_no in 
					(
						select	agreement_no 
						from	dbo.agreement_asset_late_return
					)
			))
			and branch_code	 = case @p_branch_code
								   when 'ALL' then branch_code
								   else @p_branch_code
							   end
			-- Louis Senin, 07 Juli 2025 11.31.47 -- 
			and client_no	 = case
								   when @p_client_no = '' then client_no
								   else @p_client_no
							   end
			-- Louis Senin, 07 Juli 2025 11.31.47 -- 
			and
			(
				agreement_no like '%' + @p_keywords + '%'
				or	agreement_external_no like '%' + @p_keywords + '%'
				or	client_name like '%' + @p_keywords + '%'
			) ;

	select		agreement_no
				,agreement_external_no
				,client_no
				,client_name
				,@rows_count 'rowcount'
	from		agreement_main
	where		((agreement_status = 'GO LIVE') or 
			(
				agreement_no in 
					(
						select	agreement_no 
						from	dbo.agreement_asset_late_return
					)
			))
				and branch_code	 = case @p_branch_code
									   when 'ALL' then branch_code
									   else @p_branch_code
								   end
				-- Louis Senin, 07 Juli 2025 11.31.47 -- 
				and client_no	 = case
									   when @p_client_no = '' then client_no
									   else @p_client_no
								   end
				-- Louis Senin, 07 Juli 2025 11.31.47 -- 
				and
				(
					agreement_no like '%' + @p_keywords + '%'
					or	agreement_external_no like '%' + @p_keywords + '%'
					or	client_name like '%' + @p_keywords + '%'
				)
	order by	case
					when @p_sort_by = 'asc' then case @p_order_by
													 when 1 then agreement_no
													 when 2 then client_name
												 end
				end asc
				,case
					 when @p_sort_by = 'desc' then case @p_order_by
													   when 1 then agreement_no
													   when 2 then client_name
												   end
				 end desc offset ((@p_pagenumber - 1) * @p_rowspage) rows fetch next @p_rowspage rows only ;
end ;
