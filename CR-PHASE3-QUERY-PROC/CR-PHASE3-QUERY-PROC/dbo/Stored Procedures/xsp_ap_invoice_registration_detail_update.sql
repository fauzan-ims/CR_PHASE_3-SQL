CREATE PROCEDURE dbo.xsp_ap_invoice_registration_detail_update
(
	@p_id				bigint
...
begin
	...
	begin try
		...
		begin
			set @msg = 'The Comma at the end of PPN cannot be anything other than 0' ;

			raiserror(@msg, 16, -1) ;
		end ;
		else if (right(@p_pph, 2) <> '00')
		begin
			set @msg = 'The Comma at the end of PPH cannot be anything other than 0' ;

			raiserror(@msg, 16, -1) ;
		end ;
		else if (right(@p_total_amount, 2) <> '00')
		begin
			set @msg = 'The Comma at the end of Total Amount cannot be anything other than 0' ;

			raiserror(@msg, 16, -1) ;
		end ;
		else if ((@p_ppn/@receive_quantity) > @p_purchase_amount)--sepria(09052025) validasi ppnnya per quantity
		begin
			set @msg = 'PPN cannot bigger than unit price.';  

			raiserror(@msg, 16, -1) ;
		end ;
		else if ((@p_pph/@receive_quantity) > @p_purchase_amount)--sepria(09052025) validasi ppnnya per quantity
		begin
