...
CREATE PROCEDURE [dbo].[xsp_ap_invoice_registration_return]
(
	@p_code			   nvarchar(50)
	--
	,@p_mod_date	   datetime
...
begin
	...
	begin try
	--CR Priority: 11092025: sepria: kondisi yg hanya bisa di return adalah invoice dengan status POST (sebelum adanya CR PRIORITY). setelah cr priority, tidak bisa lakukan return invoice yg sudah approve.
	if exists (
				select distinct a.grn_detail_id, a.item_name from dbo.ap_invoice_registration_detail a
				inner join dbo.ap_invoice_registration b on b.code = a.invoice_register_code
				where b.status = 'POST' and a.grn_detail_id is null and b.code = @p_code
	)
	begin
		if exists
		(
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[xsp_ap_invoice_registration_return] TO [ims-raffyanda]
    AS [dbo];
