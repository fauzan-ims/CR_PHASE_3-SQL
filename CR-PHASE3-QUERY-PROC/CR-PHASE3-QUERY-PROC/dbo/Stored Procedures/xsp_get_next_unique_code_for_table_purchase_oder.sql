CREATE PROCEDURE dbo.xsp_get_next_unique_code_for_table_purchase_oder
(
			...
			begin
				...
				begin
					...
					begin
						...
						if (@sys_brc_code = '')
						begin
							set @run_length += ( len(@sys_doc_code) + len(@p_year) + len(@p_month) + @dot_a) + 1 ;
						...
						begin
							set @run_length += (len(@sys_brc_code)  +len(@p_delimiter)+ len(@sys_doc_code) + len(@p_year) + len(@p_month) + @dot_a) + 1 ;
					...
					begin
						...
						if (@sys_brc_code = '')
						begin
							set @run_length += (len(@p_custom_prefix) + len(@p_year) + len(@p_month) + @dot_a) + 1 ;
							set @year_length += (len(@p_custom_prefix) + @dot_b) + 1 ;
							set @month_length += (len(@p_custom_prefix) + len(@p_year) + @dot_b) + 1 ;
						end
						else
						begin
							set @run_length += (len(@sys_brc_code) +len(@p_delimiter) + len(@p_custom_prefix) + len(@p_year) + len(@p_month) + @dot_a) + 1 ;
							--set @year_length += (len(@sys_brc_code) +len(@p_delimiter) + len(@p_custom_prefix) + @dot_b) + 1 ;
							set @year_length += (len(@p_run_number_length) + len(@p_delimiter) + len(@sys_brc_code) + len(@p_delimiter) + len(@p_custom_prefix) + len(@p_delimiter) + len(@month_length));
							--set @month_length += (len(@sys_brc_code) +len(@p_delimiter) + len(@p_custom_prefix) + len(@p_year) + @dot_b) + 1 ;
							set @month_length += (len(@p_run_number_length) + len(@p_delimiter) + len(@sys_brc_code) + len(@p_delimiter) + len(@p_custom_prefix));
						end
					end ;
					...
					begin
						if @sys_brc_code = '' and	@sys_doc_code <> ''
							set @p_unique_code = @sys_doc_code + @p_delimiter + @p_year + @p_month + @p_delimiter + @next_run_number ;
						else if @sys_doc_code = '' and @sys_brc_code <> ''
							set @p_unique_code = @sys_brc_code + @p_delimiter + @p_year + @p_month + @p_delimiter + @next_run_number ;
						else
						begin
							if @sys_brc_code = ''  and	@sys_doc_code = ''
								set @p_unique_code = @p_year + @p_month + @p_delimiter + @next_run_number ;
							else
								set @p_unique_code = @sys_brc_code + @p_delimiter + @sys_doc_code + @p_delimiter + @p_year + @p_month + @p_delimiter + @next_run_number ;
						end ;
					end ;
					else
						if @p_branch_code <> ''
						begin
							--set @p_unique_code = @p_branch_code + @p_delimiter + @p_custom_prefix + @p_delimiter + @p_year + @p_month + @p_delimiter + @next_run_number ;
							set @p_unique_code = @next_run_number + @p_delimiter + @p_branch_code + @p_delimiter + @p_custom_prefix + @p_delimiter + @p_month + @p_delimiter + @p_year ;
						end
						else
						begin
							set @p_unique_code = @p_custom_prefix + @p_delimiter + @p_year + @p_month + @p_delimiter + @next_run_number ;
						end
				end ;
