...
CREATE PROCEDURE [dbo].[xsp_good_receipt_note_detail_update]
(
	@p_id						bigint
...
begin
	...
	begin try
		...
		select	@total_object_info = count(id) 
		...
		else if ((@p_ppn_amount/@p_receive_quantity) > @new_unit_price) --sepria(09052025) validasi ppnnya per quantity
		begin
			set @msg = 'PPN cannot bigger than Total Amount ' +  convert(nvarchar(50), @unit_price) ;  

			raiserror(@msg, 16, -1) ;
		end ;
		else if ((@p_pph_amount/@p_receive_quantity) > @new_unit_price) --sepria(09052025) validasi pphnya per quantity
		begin
		...
		if not exists --raffy 2025/06/12 penambahan validasi agar object info selalu diinput 
		(
			select	1 
			from	dbo.purchase_order_detail_object_info
			where	good_receipt_note_detail_id =  @p_id
		...
		begin
			set @msg = 'Please Input Object Info GRN'
			raiserror (@msg, 16, -1);
		end



		if (@p_receive_quantity <> @total_object_info)
		begin
        set @msg = 'Please add object info for each item in the receive quantity'
			raiserror (@msg, 16, -1);
		end
