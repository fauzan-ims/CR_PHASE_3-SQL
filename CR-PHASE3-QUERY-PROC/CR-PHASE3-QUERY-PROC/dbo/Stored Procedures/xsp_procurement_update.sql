CREATE PROCEDURE [dbo].[xsp_procurement_update]
(
	@p_code							nvarchar(50)
	,@p_procurement_request_item_id bigint
	,@p_procurement_request_code	nvarchar(50)
	,@p_branch_name					nvarchar(250)
	,@p_item_name					nvarchar(250)
	,@p_item_type_code				nvarchar(50)	= ''
	,@p_item_type_name				nvarchar(50)	= ''
	,@p_quantity_request			int
	,@p_approved_quantity			int
	,@p_specification				nvarchar(4000)
	,@p_remark						nvarchar(4000)
	,@p_new_purchase				nvarchar(20)
	,@p_purchase_type_code			nvarchar(50)	= ''
	,@p_purchase_type_name			nvarchar(50)	= ''   
	,@p_quantity_purchase			int				= ''
	,@p_status						nvarchar(20)
	,@p_unit_from					nvarchar(25)	= ''
	--
	,@p_mod_date					datetime
	,@p_mod_by						nvarchar(15)
	,@p_mod_ip_address				nvarchar(15)
)
as
begin

	declare		@msg				nvarchar(max) 
				,@reff_no			nvarchar(50);

	if (@p_new_purchase = 'NO')
	begin
		set @p_purchase_type_code = null ;
	end ;
	else
	begin
		set @p_purchase_type_code = @p_purchase_type_code ;
	end ;

	begin TRY
    
		select	@reff_no = prq.reff_no
		from	dbo.procurement pr
				inner join dbo.procurement_request prq on (prq.code = pr.procurement_request_code)
		where	pr.code = @p_code ;

		if (isnull(@reff_no, '') = '')
		begin
			update	procurement
			set		procurement_request_item_id = @p_procurement_request_item_id
					,procurement_request_code	= @p_procurement_request_code
					,branch_name				= @p_branch_name
					,item_name					= @p_item_name
					,item_type_code				= @p_item_type_code
					,item_type_name				= @p_item_type_name
					,quantity_request			= @p_quantity_request
					,approved_quantity			= @p_approved_quantity
					,specification				= @p_specification
					,remark						= @p_remark
					,new_purchase				= @p_new_purchase
					,purchase_type_code			= @p_purchase_type_code
					,purchase_type_name			= @p_purchase_type_name
					,quantity_purchase			= @p_quantity_purchase
					,status						= @p_status
					,unit_from					= @p_unit_from
					--
					,mod_date					= @p_mod_date
					,mod_by						= @p_mod_by
					,mod_ip_address				= @p_mod_ip_address
			where	code						= @p_code ;
		end
		else
		begin
			update	procurement
			set		procurement_request_item_id = @p_procurement_request_item_id
					,procurement_request_code	= @p_procurement_request_code
					,branch_name				= @p_branch_name
					,item_name					= @p_item_name
					,item_type_code				= @p_item_type_code
					,item_type_name				= @p_item_type_name
					,quantity_request			= @p_quantity_request
					,approved_quantity			= @p_approved_quantity
					,specification				= @p_specification
					,remark						= @p_remark
					,new_purchase				= @p_new_purchase
					,purchase_type_code			= @p_purchase_type_code
					,purchase_type_name			= @p_purchase_type_name
					,quantity_purchase			= @p_quantity_purchase
					,status						= @p_status
					--,unit_from					= @p_unit_from
					--
					,mod_date					= @p_mod_date
					,mod_by						= @p_mod_by
					,mod_ip_address				= @p_mod_ip_address
			where	code						= @p_code ;
		end

	end try
	begin catch
		declare @error int ;

		set @error = @@error ;

		if (@error = 2627)
		begin
			set @msg = dbo.xfn_get_msg_err_code_already_exist() ;
		end ;

		if (len(@msg) <> 0)
		begin
			set @msg = 'V' + ';' + @msg ;
		end ;
		else
		begin
			if (error_message() like '%V;%' or error_message() like '%E;%')
			begin
				set @msg = error_message() ;
			end
			else 
			begin
				set @msg = 'E;' + dbo.xfn_get_msg_err_generic() + ';' + error_message() ;
			end
		end ;

		raiserror(@msg, 16, -1) ;

		return ;
	end catch ; 
end ;
