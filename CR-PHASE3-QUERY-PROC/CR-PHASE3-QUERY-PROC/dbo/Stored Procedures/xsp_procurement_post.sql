CREATE PROCEDURE [dbo].[xsp_procurement_post]
(
	@p_code						 nvarchar(50)
...
begin
	...
	begin try
		...
		select	@remark							= prc.remark
				...
				,@accesories_amount				= prc.accesories_amount
				,@accesories_discount_amount	= prc.accesories_discount_amount
		...
		set PURCHASE_TYPE_CODE		= 'WTQTN'
			,PURCHASE_TYPE_NAME		= 'WITH QUOTATION'
			,MOD_DATE				= @p_mod_date
			,MOD_BY					= @p_mod_by
			,MOD_IP_ADDRESS			= @p_mod_ip_address
		where CODE = @p_code
		...
		begin
			...
			select @remark_quo =	stuff((
			 ...
			 set remark = @unit_from + ' for ' + @remark_quo + ' - ' + @remark
			 where code = @code
		end ;
		...
		--												  ,@p_requestor_code					= @requestor_code
		--												  ,@p_requestor_name					= @requestor_name
		--												  ,@p_supplier_selection_detail_status	= 'HOLD'
		...
		if (@application_no <> '')
		begin
			set @description_log = 'Procurement proceed with quotation, Asset no : ' + @asset_no + ' - ' + @item_name
