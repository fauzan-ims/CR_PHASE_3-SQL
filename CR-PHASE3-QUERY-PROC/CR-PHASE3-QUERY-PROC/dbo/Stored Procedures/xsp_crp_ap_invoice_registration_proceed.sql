CREATE PROCEDURE dbo.xsp_crp_ap_invoice_registration_proceed
(
	@p_code			   nvarchar(50)
	--
	,@p_mod_date	   datetime
...
declare @msg						nvarchar(max)
	...
	begin try
		if exists (
					...
					and invd.grn_code not in (select invoice_or_gnr_no from dbo.list_data_cleansing_invoice_dan_monitoring_ap_sebelum_crpriority_naik)
				)
		begin
        /*
			--19092025: sepria
			grn yang baru/ posting setelah cr priority naik, tidak bisa di gabung dengan invoice yang grn lama.
		*/
		...
		begin
					...
					exec dbo.xsp_crp_reverse_journal_grn @p_code			= @temp_grn_code
					...
					begin
						...
						begin
							...
							begin
								...
								exec dbo.xsp_crp_reverse_journal_multiple_final @p_code				= @temp_grn_code
							...
							begin
					    		--bikin jurnal reverse untuk FINAL
								exec dbo.xsp_crp_reverse_journal_final @p_code				= @temp_grn_code
																   ,@p_final_grn_code	= @code_final
																   ,@p_company_code		= 'DSF'
																   ,@p_mod_date			= @p_mod_date
																   ,@p_mod_by			= @p_mod_by
		...
		begin
					--Jurnal baru GRN
					exec dbo.xsp_crp_new_journal_grn @p_code			= @temp_grn_code
												 ,@p_mod_date		= @p_mod_date
												 ,@p_mod_by			= @p_mod_by
												 ,@p_mod_ip_address = @p_mod_ip_address
					...
					begin
						...
						begin
							...
							begin
								--Jurnal baru FINAL
								exec dbo.xsp_crp_new_journal_multiple_final @p_code					= @temp_grn_code
																		,@p_invoice_code		= @p_code
																		,@p_final_grn_code		= @code_final
																		,@p_company_code		= 'DSF'
																		,@p_mod_date			= @p_mod_date
							...
							begin								
								--Jurnal baru FINAL
								exec dbo.xsp_crp_new_journal_final @p_code				= @temp_grn_code
															   ,@p_final_grn_code	= @code_final
															   ,@p_company_code		= 'DSF'
															   ,@p_mod_date			= @p_mod_date
															   ,@p_mod_by			= @p_mod_by
		...
		-- Auto POST
		exec dbo.xsp_crp_ap_invoice_registration_post @p_code				= @p_code
												  ,@p_company_code		= 'DSF'
												  ,@p_mod_date			= @p_mod_date	  
												  ,@p_mod_by			= @p_mod_by		  
												  ,@p_mod_ip_address	= @p_mod_ip_address
