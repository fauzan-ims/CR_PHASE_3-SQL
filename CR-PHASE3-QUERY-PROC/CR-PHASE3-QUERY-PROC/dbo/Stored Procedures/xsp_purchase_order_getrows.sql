CREATE PROCEDURE dbo.xsp_purchase_order_getrows
(
...
begin
	...
	declare @purchase_order table
	(
		purchase_order_code [nvarchar](50)
	)  

	insert	@purchase_order
	select	distinct d.PO_CODE	collate latin1_general_ci_as 'purchase_order_code'
			from	dbo.PURCHASE_ORDER_DETAIL d
			where	(spesification		like '%' + @p_keywords + '%'
			or		d.description			like '%' + @p_keywords + '%')
	UNION All

	select	distinct po.code 'purchase_order_code'
	from	purchase_order po
	where	po.status	   = case @p_status
	...
	where	po.company_code	   = @p_company_code
			...
			and po.code in (select purchase_order_code from @purchase_order)
