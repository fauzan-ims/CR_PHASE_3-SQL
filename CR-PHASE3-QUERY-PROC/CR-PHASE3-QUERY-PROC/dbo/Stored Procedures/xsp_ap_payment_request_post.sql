CREATE PROCEDURE dbo.xsp_ap_payment_request_post
(
	@p_code			   nvarchar(50)
	--
	,@p_mod_date	   datetime
	...
	begin try
		...
		begin
			...
			(
				...
				where	detail.good_receipt_note_code = grn.code
					...
					left join dbo.purchase_order_detail_object_info podoi on (
																				 ...
																				 and   podoi.good_receipt_note_detail_id = detailgrn.id
																			 )
			...
			begin
				...
				if (@transaction_code = 'PYMNTR') -- 20052025: SEPRIA, UNTUK ACCOUNT PAYABLE SUPPLIER DI GABUNG JADI 1 BARIS SAJA per cabang
				begin
					if not exists (select 1 from dbo.ifinproc_interface_payment_request_detail where gl_link_code = @gl_link_code and payment_request_code	= @interface_code and branch_code = @branch_code_request)
					begin
					...
					begin
						update	dbo.ifinproc_interface_payment_request_detail
						set		orig_amount			= orig_amount + @orig_amount_db 
						where	gl_link_code		= @gl_link_code
						and		payment_request_code	= @interface_code
						and		branch_code = @branch_code_request ;
					end
                end
                else
				begin
				...
				begin
					...
					begin
						...
						begin
							...
							select	@invoice_date = b.invoice_date
							from	dbo.ap_invoice_registration_detail	   a
									inner join dbo.ap_invoice_registration b on a.invoice_register_code = b.code
							where	a.grn_detail_id = @id_grn_detail ;

							set @next_due_date = dateadd(month, 1, @invoice_date)

							update	ifinams.dbo.monitoring_gps
							set		first_payment_date	= @invoice_date
									,total_paid			= @invoice_amount
									,mod_date			= @p_mod_date
									,mod_ip_address		= @p_mod_ip_address
									,mod_by				= @p_mod_by
							where	fa_code = @code_asset

							--update	ifinams.dbo.asset_gps_schedule
							--set		periode = 'Subscribe ke 1 dari ' + CONVERT(varchar, @invoice_date, 23) + ' sampai dengan ' + CONVERT(varchar, @next_due_date, 23)
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[xsp_ap_payment_request_post] TO [ims-raffyanda]
    AS [dbo];
