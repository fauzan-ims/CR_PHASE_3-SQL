CREATE PROCEDURE dbo.xsp_ap_invoice_registration_proceed_backup
(
	@p_code			   nvarchar(50)
	--
	,@p_mod_date	   datetime
...
declare @msg						nvarchar(max)
	...
	begin try
		...
		begin
				...
				if (@ppn > 0)
				begin
					if (isnull(@faktur_no,@faktur_no_old) = '')
					begin
		...
		---- Auto POST
		exec dbo.xsp_ap_invoice_registration_post_backup @p_code				= @p_code
														,@p_company_code		= 'DSF'
														,@p_mod_date			= @p_mod_date	  
														,@p_mod_by				= @p_mod_by		  
														,@p_mod_ip_address		= @p_mod_ip_address
