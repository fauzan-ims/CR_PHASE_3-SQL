...
CREATE PROCEDURE [dbo].[xsp_ap_invoice_registration_return_from_approval]
(
	@p_code			   nvarchar(50)
	--
	,@p_mod_date	   datetime
...
begin
	...
	begin try
		...
		BEGIN
			...
			begin
			    set @transaction_name = 'Reverse Invoice Register ' + @p_code + ' From PO ' + @purchase_order_code + '.' + ' Vendor ' + @vendor_name
				...
				begin
					...
					begin
						...
						set @remarks_journal = @transaction_name + ' ' + '. Invoice No : ' + @p_code
						...
						begin
							...
							else if(@transaction_code = 'INVAPS')
							begin
								exec dbo.xsp_ifinproc_interface_journal_gl_link_transaction_detail_insert @p_gl_link_transaction_code		= @gl_link_transaction_code
																										  ,@p_company_code					= 'DSF'
																										  ,@p_branch_code					= @branch_code
						...
						if(@transaction_code = 'INVVAT')
						begin							
							declare curr_inv_ppn cursor fast_forward read_only for
							select	ardf.faktur_no
			...
			begin
				...
				while @@fetch_status = 0
				begin
					if exists (select 1 from dbo.ifinproc_interface_journal_gl_link_transaction where reff_source_no = @temp_final_code and transaction_name = 'Final Good Receipt Note')
					begin
		...
		begin
			set @msg = 'Cannot return this data, data already proceed.' ;
			raiserror(@msg, 16, 1) ;
		end ;
	end try
	begin catch
