CREATE PROCEDURE dbo.xsp_crp_new_journal_multiple_final
(
	@p_code				nvarchar(50)
		...
		begin --insert journal FINAL GRN
			...
			begin
				...
				begin
					...
					set		new_final_journal_code	= @journal_final
							,new_final_journal_date	= @journal_date
							--
							,mod_date				= @p_mod_date
							,mod_by					= @p_mod_by
					...
					from		dbo.master_transaction_parameter				mtp
								...
								--														   and	air.code = @p_invoice_code --air.STATUS					  = 'ON PROCESS'
								--													   )
								left join dbo.supplier_selection_detail			ssd on (ssd.id								  = pod.supplier_selection_detail_id)
								left join dbo.quotation_review_detail			qrd on (qrd.id								  = ssd.quotation_detail_id)
								inner join dbo.procurement						prc on (prc.code collate Latin1_General_CI_AS = isnull(qrd.reff_no, ssd.reff_no))
								inner join dbo.procurement_request				pr on (pr.code								  = prc.procurement_request_code)
					where		mtp.process_code		  = 'CRPS240400002'
								and grnd.receive_quantity <> 0
					...
					begin
						...
						begin
							if @return_value <> 0
							begin
								set @remarks_journal = 'NEW ' + isnull(@transaction_name, '') + N' ' + isnull(convert(nvarchar(3), @recive_quantity), '') + N' ' + isnull(@uom_name, '') + N' ' + isnull(@item_name_for_jrnl, '') + N'. PO No : ' + isnull(@po_no, '') ;
								...
								begin
									...
									begin
										set @remarks_journal = 'NEW ' + isnull(@transaction_name, '') + N' ' + isnull(@item_name_for_jrnl, '') + N'. PO No : ' + isnull(@po_no, '') ;
					...
					begin
						set @msg = N'Journal does not balancetttt' ;
