...
CREATE PROCEDURE [dbo].[xsp_final_grn_request_detail_karoseri_getrows]
(
...
begin
	...
	from	dbo.final_grn_request_detail_karoseri				   frdk
			...
			left join dbo.final_grn_request_detail_karoseri_lookup frdkl on frdk.final_grn_request_detail_karoseri_id = frdkl.id
	...
	select		frdk.id
				...
				,frdkl.grn_code
				,frdkl.item_name
				,isnull(grnd.item_name, '') 'item_name_asset'
				,isnull(frd.plat_no, '')	'plat_no'
				,isnull(frd.engine_no, '')	'engine_no'
				,isnull(frd.chasis_no, '')	'chassis_no'
				,frdk.application_no
				,@rows_count				'rowcount'
	from		dbo.final_grn_request_detail_karoseri				   frdk
				left join dbo.final_grn_request_detail				   frd on frd.id									  = frdk.final_grn_request_detail_id
