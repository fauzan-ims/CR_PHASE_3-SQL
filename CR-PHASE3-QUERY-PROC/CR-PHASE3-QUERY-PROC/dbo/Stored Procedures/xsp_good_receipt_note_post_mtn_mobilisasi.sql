CREATE PROCEDURE dbo.xsp_good_receipt_note_post_mtn_mobilisasi
...
BEGIN
	...
	declare @msg								nvarchar(max)
			...
			,@journal_date						datetime = DATEADD(DAY, -@p_posting_date, dbo.xfn_get_system_date())
	...
	begin try
		...
		begin --insert journal FINAL GRN
			...
			begin
				...
				group by pr.branch_code, pr.branch_name
				...
				begin
					...
					set @transaction_name = 'Final Good Receipt Note ' + @p_final_grn_code + ' From PO ' + @po_no + '.' + ' Vendor ' + @vendor_name
					...
					from		dbo.master_transaction_parameter			  mtp
								...
								left JOIN dbo.purchase_order_detail_object_info podoi ON (pod.id = podoi.purchase_order_detail_id AND grnd.id = podoi.good_receipt_note_detail_id)
								left join dbo.supplier_selection_detail		  ssd on (ssd.id								= pod.supplier_selection_detail_id)
								left join dbo.quotation_review_detail		  qrd on (qrd.id								= ssd.quotation_detail_id)
								inner join dbo.procurement					  prc on (prc.code collate Latin1_General_CI_AS = isnull(qrd.reff_no, ssd.reff_no))
								inner join dbo.procurement_request				pr on (pr.code								  = prc.procurement_request_code)
					...
					begin
						...
						if(@gl_link_code = 'ASTEXPS' and @pph_for_expense = 0)
						begin
							set @gl_link_code = 'ASTEXPSWTHT'
						end

						--untuk membedakan jika mobilisasi memiliki pph atau tidak
						if(@gl_link_code = 'ASMBLSCOST' and @pph_for_expense = 0)
						begin
							set @gl_link_code = 'ASMBLSCOST WTPPH'
						end
						--SELECT @is_rent
							...
							begin
								if @return_value <> 0
								begin
									set @remarks_journal = isnull(@transaction_name,'') + ' ' + isnull(convert(nvarchar(3),@recive_quantity),'') + ' ' + isnull(@uom_name,'') + ' ' +  isnull(@item_name_for_jrnl,'') + '. PO No : ' + isnull(@po_no,'')
									...
									begin
										...
										begin
											set @remarks_journal = isnull(@transaction_name,'') + ' ' +  isnull(@item_name_for_jrnl,'') + '. PO No : ' + isnull(@po_no,'')
									...
									BEGIN
										if not exists (select 1 from dbo.ifinproc_interface_journal_gl_link_transaction_detail where gl_link_transaction_code = @journal_final and gl_link_code = @gl_link_code)
										BEGIN
											set @remarks_journal = isnull(@transaction_name,'') + ' ' +  isnull(@item_name_for_journal,'')
