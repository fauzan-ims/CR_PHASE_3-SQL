CREATE PROCEDURE dbo.xsp_crp_ap_invoice_registration_post
(
	@p_code			   nvarchar(50)
...
begin
	declare @msg								nvarchar(max)
            ...
            ,@supplier_nitku								NVARCHAR(50) --(+) CR NITKU Raffyanda (2025/02/20)
            ,@supplier_npwp_ho								NVARCHAR(50)
			,@id_faktur							int
	...
	BEGIN TRY
		...
		BEGIN
			...
			select		distinct
						...
						,ssd.supplier_nitku
						,ssd.supplier_npwp_pusat
						,airdk.ID --(+) Raffy 2025/04/29 Penambahan agar journal yang terbentuk sesuai dengan quantity nya IMON 2504000136
			from		dbo.master_transaction_parameter					mtp
						...
						inner join dbo.procurement							prc on (prc.code collate latin1_general_ci_as				  = isnull(qrd.reff_no, ssd.reff_no))
						inner join dbo.procurement_request					pr on (pr.code												  = prc.procurement_request_code)
						inner join ifinsys.dbo.master_tax_scheme_detail		mts on (mts.tax_scheme_code									  = ird.tax_code collate sql_latin1_general_cp1_ci_as)
			where		mtp.process_code   = 'CRPSGS230600004'
			...
			begin
				...
				--SELECT @sp_name'@sp_name',@invoice_id'@invoice_id',@return_value'@return_value'
				...
				begin
					...
					exec dbo.xsp_ifinproc_interface_journal_gl_link_transaction_detail_insert @p_gl_link_transaction_code		= @gl_link_transaction_code
																							  ...
																							  ,@p_agreement_no					= @po_code --@asset_code
																							  ...
																							  ,@p_ext_sale_type					= ''
																							  ,@p_ext_vendor_nitku				= @supplier_nitku
																							  ,@p_ext_vendor_npwp_pusat			= @supplier_npwp_ho
																							  ,@p_cre_date						= @p_mod_date
																							  ,@p_cre_by						= @p_mod_by
																							  ,@p_cre_ip_address				= @p_mod_ip_address
																							  ,@p_mod_date						= @p_mod_date
