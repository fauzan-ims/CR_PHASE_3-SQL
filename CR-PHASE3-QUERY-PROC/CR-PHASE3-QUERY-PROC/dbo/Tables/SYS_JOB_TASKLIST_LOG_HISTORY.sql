CREATE TABLE [dbo].[SYS_JOB_TASKLIST_LOG_HISTORY] (
...
GO
			...
			CREATE TRIGGER [dbo].[SYS_JOB_TASKLIST_LOG_HISTORY_Insert_Audit] 
			ON [dbo].[SYS_JOB_TASKLIST_LOG_HISTORY]    
			FOR INSERT    
			AS    
			
 INSERT INTO [dbo].[Z_AUDIT_SYS_JOB_TASKLIST_LOG_HISTORY]
...
GO
			...
			CREATE TRIGGER [dbo].[SYS_JOB_TASKLIST_LOG_HISTORY_Delete_Audit]    
			ON [dbo].[SYS_JOB_TASKLIST_LOG_HISTORY]    
			FOR DELETE    
			AS   
  INSERT INTO [dbo].[Z_AUDIT_SYS_JOB_TASKLIST_LOG_HISTORY]
...
GO
			...
			CREATE TRIGGER [dbo].[SYS_JOB_TASKLIST_LOG_HISTORY_Update_Audit]      
			ON [dbo].[SYS_JOB_TASKLIST_LOG_HISTORY]    
			FOR UPDATE    
			AS 
...
SELECT CONVERT(bigint,[ID]) as [ID],[JOB_TASKLIST_CODE],[STATUS],[START_DATE],[END_DATE],[LOG_DESCRIPTION],[RUN_BY],[FROM_ID],[TO_ID],[NUMBER_OF_ROWS],[CRE_DATE],[CRE_BY],[CRE_IP_ADDRESS],[MOD_DATE],[MOD_BY],[MOD_IP_ADDRESS],'Old','Update',SUSER_SNAME(),getdate(),@UpdatedCols  FROM DELETED  
			   END
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'JOB, MANUAL', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'SYS_JOB_TASKLIST_LOG_HISTORY', @level2type = N'COLUMN', @level2name = N'RUN_BY';
