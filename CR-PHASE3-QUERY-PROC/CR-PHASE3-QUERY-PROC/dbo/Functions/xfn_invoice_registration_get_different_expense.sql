...
CREATE FUNCTION [dbo].[xfn_invoice_registration_get_different_expense]
(
	@p_id	BIGINT
...
begin
	...
	declare @return_amount				decimal(18,2)
			...
			,@price_amount_grn			decimal(18,2)
			,@price_amount_inv			decimal(18,2)

	--sepria 14/08/2025: CR Priority. untuk expense, jika nilai dari grn dan invoice beda, maka selisih di insert ke intransit
	select	@grnd_id			= invd.grn_detail_id
	...
	from	dbo.ap_invoice_registration_detail				invd
			...
			inner join ifinbam.dbo.master_item mti on mti.code = pri.item_code
	where	invd.id = @p_id
	and		grnd.receive_quantity	<> 0

	if (@procurement_type <> 'purchase')
	begin
		select	@price_amount_grn =  isnull(price_amount,0) - isnull(discount_amount,0)
		from	dbo.good_receipt_note_detail
		where	id = @grnd_id

		select	@price_amount_inv = (isnull(purchase_amount,0) - isnull(discount,0))
		from	dbo.ap_invoice_registration_detail
		where	id = @p_id

		set @return_amount = isnull(@price_amount_grn,0) - isnull(@price_amount_inv,0)
	end
