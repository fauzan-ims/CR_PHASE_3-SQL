...
CREATE FUNCTION dbo.xfn_et_get_invoice_amount
(
...
begin
	...
	select	@ar_amount = dbo.xfn_et_get_invoice_ar_amount(@p_reff_no, @p_agreement_no, @p_date) ;

	--select	@pph = dbo.xfn_agreement_get_ol_pph(@p_agreement_no, @p_date) ;

	--return isnull(@ar_amount - @pph, 0) ;
	return isnull(round(@ar_amount, 0), 0) ;
end ;
