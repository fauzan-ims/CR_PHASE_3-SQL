CREATE FUNCTION dbo.xfn_agreement_get_et_admin
(
...
begin
	...
	from	dbo.agreement_charges with (nolock)
	where	agreement_no	 = @p_agreement_no
			and charges_code = 'CETA' ;

	set @os_principal = dbo.xfn_et_get_os_principal(@p_reff_no, @p_agreement_no, @p_date) ;

	if (@calculate_type = 'PCT')
	begin
		set @et_admin = @os_principal * (@charges_rate / 100) ;
	end ;
	else
	begin
		set @et_admin = @charges_amount ;
	end ;

	return isnull(@et_admin, 0) ;
end ;
