CREATE FUNCTION dbo.xfn_due_date_period_application(@p_asset_no nvarchar(50),@p_installment_no int)
returns @t table (asset_no nvarchar(50),installment_no nvarchar(50),period_date datetime, period_due_date datetime)
AS
BEGIN
	declare @due_date				datetime
      ...
      -- mengambil multipier di master payment schedule
		select	@schedule_month = multiplier
				...
				--,@tenor = aa.periode
				,@first_payment = am.first_payment_type
				,@period = am.periode
				--,@billing_mode_date = aa.billing_mode_date
		from	dbo.master_billing_type mbt with(nolock)
				inner join dbo.application_asset aa with(nolock) on (aa.billing_type = mbt.code)
		...
		from	dbo.application_amortization with(nolock)
		where	asset_no	   = @p_asset_no
				and installment_no = @p_installment_no - 1 
		else
		select	@due_date_before = due_date
		from	dbo.application_amortization with(nolock)
		where	asset_no	   = @p_asset_no
		...
		set @no = @p_installment_no - 1 ;
		set @first_duedate = @due_date ;

		if @first_payment = 'ARR' and @p_installment_no = 1
		begin
			set @billing_date = dateadd(month, @schedule_month * -1, @first_duedate) ;
			if @billing_date <> @handover_date
				set @first_duedate = isnull(@handover_date,@billing_date) 
		end ;
		
		set @due_date = dateadd(month, (@schedule_month), @first_duedate) ;
		set @no = @no + 1 ;
		...
		if @max_installment_no = @p_installment_no and @first_payment='ADV'
		begin
