CREATE PROCEDURE dbo.xsp_faktur_allocation_getrows
(
...
begin
	...
	and		(
				...
				or	convert(varchar(30),as_of_date,103)		like '%' + @p_keywords + '%'
			) ;
	...
	select		code
				...
				,status
				,remark
				,convert(varchar(30),as_of_date,103) 'as_of_date'
				,@rows_count 'rowcount'
	from		faktur_allocation
	where		branch_code	= case @p_branch_code
								when 'ALL' then branch_code
	...
	order by	case
					when @p_sort_by = 'asc' then case @p_order_by
													 ...
													 when 4 then remark
													 when 5 then cast(as_of_date as sql_variant)
													 when 6 then status
												 end
				end asc
				,case
