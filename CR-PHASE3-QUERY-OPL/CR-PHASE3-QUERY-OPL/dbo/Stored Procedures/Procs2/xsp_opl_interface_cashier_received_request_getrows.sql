CREATE PROCEDURE dbo.xsp_opl_interface_cashier_received_request_getrows
(
...
begin
	...
	where	crr.branch_code		= case @p_branch_code
			...
			and crr.request_status != 'CANCEL' 
			and (
					crr.code									like '%' + @p_keywords + '%'
					or	crr.branch_name							like '%' + @p_keywords + '%'
					or	crr.doc_reff_name						like '%' + @p_keywords + '%'
	...
	where		crr.branch_code		= case @p_branch_code
				...
				and (
						crr.code									like '%' + @p_keywords + '%'
						or	crr.doc_reff_code						like '%' + @p_keywords + '%'
						or	crr.doc_reff_name						like '%' + @p_keywords + '%'
						or	crr.branch_name							like '%' + @p_keywords + '%'
						or	convert(varchar(30), request_date, 103) like '%' + @p_keywords + '%'
						or	convert(varchar(30), crr.mod_date, 103) like '%' + @p_keywords + '%'
	...
	order by	case
					when @p_sort_by = 'asc' then case @p_order_by
													 ...
													 when 4 then crr.doc_reff_code + crr.doc_reff_name
