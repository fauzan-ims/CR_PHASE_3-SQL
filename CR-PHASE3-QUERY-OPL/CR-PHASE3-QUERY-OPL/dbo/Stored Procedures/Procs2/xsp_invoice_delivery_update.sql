CREATE PROCEDURE dbo.xsp_invoice_delivery_update
(
	@p_code				  nvarchar(50)
	,@p_branch_code		  nvarchar(50)
	,@p_branch_name		  nvarchar(250)
	,@p_status			  nvarchar(10)
	,@p_date			  datetime	
	,@p_method			  nvarchar(10)	
	,@p_employee_code	  nvarchar(50)		= ''
	,@p_employee_name	  nvarchar(250)		= ''
	,@p_external_pic_name nvarchar(250)		= ''
	,@p_email			  nvarchar(250)		= ''
	,@p_remark			  nvarchar(4000)	= ''
	-- Louis Rabu, 02 Juli 2025 10.17.37 -- 
	,@p_delivery_result		   nvarchar(10)	 = NULL
	,@p_delivery_received_date datetime		 = null
	,@p_delivery_received_by   nvarchar(250) = null
	,@p_delivery_doc_reff_no   nvarchar(50)	 = null
	,@p_delivery_reject_date   datetime		 = null
	,@p_delivery_reason_code   nvarchar(50)	 = null
	,@p_client_address		   nvarchar(max) = null
	-- Louis Rabu, 02 Juli 2025 10.17.37 -- 
	--
	,@p_mod_date		  datetime
	,@p_mod_by			  nvarchar(15)
	,@p_mod_ip_address	  nvarchar(15)
)
as
begin
	declare @msg nvarchar(max) ;

	begin try
	--if  (@p_email = '')
	--begin
	--	set @msg = 'Please Fill a Mandatory Field OR Format Is Invalid'
	--	raiserror(@msg ,16,-1);
	--end

	--if  (@p_employee_code = '')
	--begin
	--	set @msg = 'Please Fill a Mandatory Field OR Format Is Invalid'
	--	raiserror(@msg ,16,-1);
	--end

	--	if  (@p_external_pic_name = '')
	--begin
	--	set @msg = 'Please Fill a Mandatory Field OR Format Is Invalid'
	--	raiserror(@msg ,16,-1);
	--end

		update	invoice_delivery
		set		branch_code			 = @p_branch_code
				,branch_name		 = @p_branch_name
				,status				 = @p_status
				,date				 = @p_date
				,method				 = @p_method
				,employee_code		 = @p_employee_code
				,employee_name		 = @p_employee_name
				,external_pic_name	 = @p_external_pic_name
				,email				 = @p_email
				,remark				 = @p_remark
				-- Louis Rabu, 02 Juli 2025 10.17.58 -- 
				,delivery_result		= @p_delivery_result		   
				,delivery_received_date = @p_delivery_received_date 
				,delivery_received_by   = @p_delivery_received_by   
				,delivery_doc_reff_no   = @p_delivery_doc_reff_no   
				,delivery_reject_date   = @p_delivery_reject_date   
				,delivery_reason_code   = @p_delivery_reason_code   
				-- Louis Rabu, 02 Juli 2025 10.18.02 -- 
				,client_address			= @p_client_address
				--
				,mod_date			 = @p_mod_date
				,mod_by				 = @p_mod_by
				,mod_ip_address		 = @p_mod_ip_address
		where	code = @p_code ;
	end try
	Begin catch
		declare @error int ;

		set @error = @@error ;

		if (@error = 2627)
		begin
			set @msg = dbo.xfn_get_msg_err_code_already_exist() ;
		end ;

		if (len(@msg) <> 0)
		begin
			set @msg = 'V' + ';' + @msg ;
		end ;
		else
		begin
			if (error_message() like '%V;%' or error_message() like '%E;%')
			begin
				set @msg = error_message() ;
			end
			else 
			begin
				set @msg = 'E;' + dbo.xfn_get_msg_err_generic() + ';' + error_message() ;
			end
		end ;

		raiserror(@msg, 16, -1) ;

		return ;
	end catch ; 
end ;