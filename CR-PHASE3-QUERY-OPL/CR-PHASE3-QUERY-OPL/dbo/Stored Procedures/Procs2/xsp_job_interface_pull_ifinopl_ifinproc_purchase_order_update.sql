...
exec xsp_job_interface_pull_ifinopl_ifinproc_purchase_order_update
*/

CREATE PROCEDURE dbo.xsp_job_interface_pull_ifinopl_ifinproc_purchase_order_update
as
begin
	...
	where	sp_name = 'xsp_job_interface_pull_ifinopl_ifinproc_purchase_order_update' ; -- sesuai dengan nama sp ini

	if (@is_active <> '0')
	begin
		declare		c_purchase_order cursor for
		select		id
		from		ifinproc.dbo.proc_interface_purchase_order_update
		...
		begin
			begin try
				...
				insert into dbo.opl_interface_purchase_order_update
				(
					purchase_code
					,po_code
					,eta_po_date
