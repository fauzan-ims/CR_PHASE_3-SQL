...
as
	begin
		...
		begin
			...
			--set	@query =	'select ' + @column_name + ' = max(left( ' + @p_column_name + ',' + cast(@p_run_number_length as nvarchar) + ' ))
			--				from ' + @p_tabel_name + '
			--				where right(' + @p_column_name + ', 4) = ''' + @p_year + '''
			--				and	cre_by <>'' MIGRASI'''

			--execute sp_executesql @query

			select	@column_name = max(left(INVOICE_EXTERNAL_NO, @p_run_number_length))
			from	dbo.invoice
			where	right(INVOICE_EXTERNAL_NO, 4) = @p_year
			and		cre_by <> 'MIGRASI' ;
