create PROCEDURE dbo.xsp_job_interface_pull_ifinopl_ifinams_additional_invoice
as
declare @msg			   nvarchar(max)
	...
	begin try
		...
		where	sp_name = 'xsp_job_interface_pull_ifinopl_ifinams_additional_invoice' ;

		if (@is_active <> '0')
		begin
			declare curr_additional_invoice cursor for
			select		id
			from		ifinams.dbo.ifinams_interface_additional_request
			...
			begin
				begin try
					...
					update	ifinams.dbo.ifinams_interface_additional_request
					set		job_status = 'POST'
					where	id = @id_interface ;
