CREATE PROCEDURE dbo.xsp_due_date_change_main_get_todo
(
...
begin
	...
	begin try
		select		@p_todo_code 'todo_code'
					,ddcm.branch_name
					,'PENDING CHANGE DUE DATE'
					,count(1) as count
		from		due_date_change_main ddcm
					inner join dbo.agreement_main am on (am.agreement_no = ddcm.agreement_no)
		where		ddcm.change_status = 'HOLD'
					and ddcm.cre_by	   = @p_user_id
		group by	ddcm.branch_name ;
	end try
	begin catch
		declare @error int ;
