---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE dbo.xsp_master_machinery_unit_update
(
	@p_code						   nvarchar(50)
...
begin
	...
	begin try
		if exists (select 1 from master_machinery_unit where description = @p_description and code <> @p_code)
		begin
			set @msg = 'description already exist';
			raiserror(@msg, 16, -1) ;
		end 

		update	master_machinery_unit
		set		machinery_category_code		= @p_machinery_category_code
				...
				,machinery_name				= upper(@p_machinery_name)
