---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE dbo.xsp_master_document_contract_group_insert
(
	@p_code									nvarchar(50)  OUTPUT
	,@p_description							nvarchar(250)
	,@p_is_active							nvarchar(1)
	,@p_contract_type						nvarchar(15) = 'APPLICATION'
	,@p_dim_count							int
...
begin
	...
	exec dbo.xsp_get_next_unique_code_for_table @code output, 
	                                            ...
	                                            @p_custom_prefix = N'DCG',                 
	                                            @p_year = @year,                          
	                                            @p_month = @month,                        
	                                            @p_table_name = N'MASTER_DOCUMENT_CONTRACT_GROUP',                    
	...
	begin try
	insert into master_document_contract_group
	(
		code
		,description
		,is_active
