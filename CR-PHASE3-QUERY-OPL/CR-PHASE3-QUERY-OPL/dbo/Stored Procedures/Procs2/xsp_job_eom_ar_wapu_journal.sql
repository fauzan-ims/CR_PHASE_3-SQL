/*
exec xsp_job_eom_ar_wapu_journal
*/
-- Louis Senin, 06 Maret 2023 13.53.59 -- 
CREATE PROCEDURE dbo.xsp_job_eom_ar_wapu_journal
as
begin
	declare @msg				  nvarchar(max)
			...
			,@reff_name			  nvarchar(50)	= N'EOM AR WAPU'
		...
		begin 
			...
			(
				'NEW', 'CANCEL'
						...
						and billing_to_faktur_type <> '01'
						and is_journal_ppn_wapu	   = '0'
						and CRE_BY <> 'MIGRASI'
			...
			begin
				...
				-- sepria 12mar2024: penomoran transaction code dijadikan unik
				set @transaction_code = 'EAW.' + @journal_branch_code + '.' + cast(year(@eod_date) as nvarchar(4)) + cast(month(@eod_date) as nvarchar(4)) ;
				...
				begin
					begin 
						...
						where	mtp.process_code = 'EOMPPNWAPU' ;
						...
						begin
							...
							if (right(isnull(@return_value,'00'), 2) <> '00')
							begin
					...
					update	dbo.invoice
					set		is_journal_ppn_wapu = '1'
							,is_journal_ppn_wapu_date = dbo.xfn_get_system_date()
							--
							,mod_date = @mod_date
							,mod_by = @mod_by
