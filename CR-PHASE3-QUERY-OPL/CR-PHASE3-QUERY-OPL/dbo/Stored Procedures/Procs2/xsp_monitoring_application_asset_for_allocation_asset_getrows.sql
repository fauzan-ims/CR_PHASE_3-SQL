...
begin
	...
	where	--aa.application_no = @p_application_no
           ...
           -- and aa.purchase_status		  = case @p_status
											--when 'ALL' then aa.purchase_status
											--else @p_status
										--END
			AND (
				(@p_status = 'GTS'  AND aa.purchase_status = aa.purchase_status AND aa.is_request_gts = 1)
				OR
				(@p_status = 'NONE' AND aa.purchase_status = 'NONE' AND aa.is_request_gts = 0)
				OR
				(@p_status = 'AGREEMENT' AND aa.purchase_status = 'AGREEMENT' AND aa.is_request_gts = 0)
				OR
				(@p_status = 'REALIZATION' AND aa.purchase_status = 'REALIZATION' AND aa.is_request_gts = 0)
				OR
				(@p_status = 'DELIVERY' AND aa.purchase_status = 'DELIVERY' AND aa.is_request_gts = 0)
				OR
				(@p_status = 'ALL' AND aa.is_request_gts = 0)
			)
