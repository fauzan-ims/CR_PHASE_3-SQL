---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE dbo.xsp_master_vehicle_model_update
(
	@p_code							nvarchar(50)
...
begin
	...
	begin try
		if exists (select 1 from master_vehicle_model where description = @p_description and vehicle_merk_code = @p_vehicle_merk_code and code <> @p_code)
		begin
			set @msg = 'Description already exist';
			raiserror(@msg, 16, -1) ;
		end 

		update master_vehicle_model
		set
