CREATE PROCEDURE dbo.xsp_invoice_getrows_for_settlement_pph_invoice
(
...
begin
	...
	(
			...
			,settlement_status				NVARCHAR(50)
			,payment_reff_no				NVARCHAR(50)
			,file_path						NVARCHAR(250)
			,file_name						NVARCHAR(250)
			,payment_reff_date				VARCHAR(50)
			,agreement_external_no			NVARCHAR(50)
			,billing_to_npwp				NVARCHAR(50)
	...
	WHERE	(
					...
					or convert(varchar(30),payment_reff_date, 103)												like '%' + @p_keywords + '%'
					OR agreement_external_no										like '%' + @p_keywords + '%'
					OR billing_to_npwp												like '%' + @p_keywords + '%'
					OR npwp_name													like '%' + @p_keywords + '%'
					OR aging_days													like '%' + @p_keywords + '%'
				);
	...
	order by	case
					when @p_sort_by = 'asc' then case @p_order_by
													 ...
													 when 3 then billing_to_npwp
													 when 4 then cast(total_pph_amount as sql_variant)
													 when 5 then file_path
													 when 7 then payment_reff_no
													 when 8 then cast(payment_reff_date as sql_variant)
													 when 9 then settlement_status
													 when 10 then aging_days--CAST(datediff(day, inv.invoice_due_date, dbo.xfn_get_system_date()) as sql_variant)
												 end
				end asc
				,case
