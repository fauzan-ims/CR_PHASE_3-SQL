---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE dbo.xsp_master_electronic_model_insert
(
	@p_code							nvarchar(50)
...
begin
	...
	begin try
		if exists (select 1 from master_electronic_model where description = @p_description)
		begin
			set @msg = 'Description already exist';
			raiserror(@msg, 16, -1) ;
		end 

		insert into master_electronic_model
		(
			code
