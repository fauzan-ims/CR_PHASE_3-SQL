/*
exec xsp_job_interface_in_ifinopl_approval_request
*/
CREATE PROCEDURE dbo.xsp_job_interface_in_ifinopl_approval_request
as
	...
	where	sp_name = 'xsp_job_interface_in_ifinopl_approval_request' -- sesuai dengan nama sp ini
	...
	begin
		...
		where		request_status = 'POST'
					and isnull(approval_status, '') <> ''
					and job_status in ('HOLD','FAILED')
					AND id NOT IN ('38388')--diexclude karena sudah diconfirm ke itd, tapi belum ada jawaban lagi
			...
			begin try
				begin transaction
					...
					begin
						...
						exec dbo.xsp_opl_interface_approval_request_approve @p_code				= @reff_no
					...
					begin
						exec dbo.xsp_opl_interface_approval_request_return @p_code				= @reff_no
																			,@p_approval_status	= @approval_status
																			,@p_approval_code	= @approval_code
																			,@p_request_status	= @request_status
																			,@p_mod_date		= @mod_date		
					...
					begin
						exec dbo.xsp_opl_interface_approval_request_reject @p_code				= @reff_no
																			,@p_approval_status	= @approval_status
																			,@p_approval_code	= @approval_code
																			,@p_request_status	= @request_status
																			,@p_mod_date		= @mod_date		
