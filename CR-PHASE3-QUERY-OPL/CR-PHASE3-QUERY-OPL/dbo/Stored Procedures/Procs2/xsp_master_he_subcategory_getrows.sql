---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE dbo.xsp_master_he_subcategory_getrows
(
...
begin
	...
	from	master_he_subcategory mfs
			inner join dbo.master_he_category mfc on (mfc.code = mfs.he_category_code)
	where	(
		...
		select		mfs.code
					,mfs.description
					,mfc.description 'he_category_desc'
					,case mfs.is_active
						 when '1' then 'Yes'
						 else 'No'
					 end 'is_active'
