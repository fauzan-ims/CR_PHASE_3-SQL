...
CREATE PROCEDURE [dbo].[xsp_et_main_to_handover_asset_insert]
(
	@p_code			   nvarchar(50)
	,@p_agreement_no   nvarchar(50)
	,@p_et_date		   nvarchar(50)
	--
	,@p_cre_date	   datetime
...
begin
	...
	begin try
		...
		select	asset_no
				...
				,ISNULL(isnull(aa.fa_name, aa.replacement_fa_name),aa.ASSET_NAME)
				,isnull(aa.pickup_name, '')
				,aa.pickup_address
		...
		begin
			...
			set @remark = 'Penarikan Unit Sewa, Early Termination Untuk Agreement No :  ' + ISNULL(@agreement_external_no,'') + '. dari Asset : ' + ISNULL(@fa_code,'') + ' - ' + ISNULL(@fa_name,'') + '.'
			...
			begin
					set @remark = 'No Need To Pickup Unit Sewa, Early Termination Untuk Agreement No :  ' + ISNULL(@agreement_external_no,'') + '. dari Asset : ' + ISNULL(@fa_code,'') + ' - ' + ISNULL(@fa_name,'') + '.'
			end ;
			...
			exec dbo.xsp_opl_interface_handover_asset_insert @p_code					= @handover_asset_code output
															 ...
															 ,@p_unit_condition			= N''
															 ,@p_reff_no				= @asset_no
															 ,@p_reff_name				= N'EARLY TERMINATION'
