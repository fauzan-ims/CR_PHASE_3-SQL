create PROCEDURE dbo.xsp_due_date_change_main_generate_amortization_backup
(
...
begin
	...
	begin try
		...
		select	 multiplier
				...
				,ddcd.new_due_date_day --dateadd(month, mbt.multiplier, ddcd.new_due_date_day)
		...
		begin
			...
			begin
				select	@total_days = abs(datediff(day, due_date, @due_date))
						,@total_real_day = abs(datediff(month, due_date, @due_date))
		...
		-- select @total_days * 1.0 , day(eomonth(@due_date)) , @schedule_month, (day(eomonth(@due_date)) * @schedule_month), (@total_days * 1.0 / (day(eomonth(@due_date)) * @schedule_month))
		--select dbo.fn_get_floor(((@total_days * 1.0 / 366) * @lease_rounded_amount), 1) ;
		--if (@rounding_type = 'DOWN')
		--	set @propotional_rental_amount = dbo.fn_get_floor(((@total_days * 1.0 / (30 * @schedule_month)) * @lease_rounded_amount), @rounding_value) ;
		--else if (@rounding_type = 'UP')
		--	set @propotional_rental_amount = dbo.fn_get_ceiling(((@total_days * 1.0 / (30 * @schedule_month)) * @lease_rounded_amount), @rounding_value) ;
		--else
		--	set @propotional_rental_amount = dbo.fn_get_round(((@total_days * 1.0 / (30 * @schedule_month)) * @lease_rounded_amount), @rounding_value) ;

		--set @propotional_rental_amount = dbo.fn_get_floor(((@total_days * 1.0 / (30 * @schedule_month)) * @lease_rounded_amount), 1) ;
		--set @propotional_rental_amount = dbo.fn_get_floor(((@total_days * 1.0 / (datediff(day, @due_date, dateadd(month, 1, @due_date) ) * @schedule_month)) * @lease_rounded_amount), 1) ;
		if exists (select 1 from dbo.agreement_asset where asset_no = @p_asset_no and agreement_no in ('0001761.4.10.01.2024','0001762.4.10.01.2024'))
		begin
			set @propotional_rental_amount = dbo.fn_get_floor(((@lease_rounded_amount / 12.00) * 11.00), 1)
		end
		else if exists (select 1 from dbo.agreement_asset where asset_no = @p_asset_no and agreement_no in ('0001911.4.10.02.2024'))
		begin
			set @propotional_rental_amount =  121000000 
		end
		else if exists (select 1 from dbo.agreement_asset where asset_no = @p_asset_no and agreement_no in ('0001912.4.10.02.2024'))
		begin
			set @propotional_rental_amount =   53063448 
		end
		else if exists (select 1 from dbo.agreement_asset where asset_no = @p_asset_no and agreement_no in ('0001913.4.10.02.2024'))
		begin
			set @propotional_rental_amount =    53245172  
		end
		else
		begin
			--set @propotional_rental_amount = dbo.fn_get_floor(((@total_days * 1.0 / (day(eomonth(@due_date)) * @schedule_month)) * @lease_rounded_amount), 1) ; 
			set @propotional_rental_amount = dbo.fn_get_ceiling(((@total_days * 1.0 / (day(eomonth(@due_date)) * @schedule_month)) * @lease_rounded_amount), 1) ; 
		end
		...
		begin
			...
			if exists (select 1 from dbo.agreement_asset where asset_no = @p_asset_no and agreement_no in ('0001911.4.10.02.2024','0001912.4.10.02.2024','0001913.4.10.02.2024'))
			begin
				insert into dbo.due_date_change_amortization_history
				(
					due_date_change_code
				...
				select	@p_due_date_change_code
						...
						,@p_asset_no
						,due_date
						,eomonth(dateadd(month, 1, billing_date))
		...
		select	@max_billing_no		= max(installment_no) + 1
				,@installment_no	= max(installment_no) + 1
		...
		begin
			...
			begin
			--select dateadd(month, (@schedule_month * (1)), @duedate_next)
			...
			if exists (select 1 from dbo.agreement_asset where asset_no = @p_asset_no and agreement_no in ('0001911.4.10.02.2024','0001912.4.10.02.2024','0001913.4.10.02.2024'))
			begin
				set @new_billing_date = dateadd(month, 1, @new_billing_date)
			end
