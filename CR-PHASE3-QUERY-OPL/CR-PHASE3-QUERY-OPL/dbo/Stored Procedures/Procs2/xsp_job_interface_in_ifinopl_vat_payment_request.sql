CREATE PROCEDURE dbo.xsp_job_interface_in_ifinopl_vat_payment_request
as
	...
	begin try
		...
		where	sp_name = 'xsp_job_interface_in_ifinopl_vat_payment_request' -- sesuai dengan nama sp ini
			...
			begin
				begin try
					begin transaction
						...
						begin    
								if(@payment_source_no = 'VAT OUT FOR OPERATING LEASE')
								begin
									--update vat_payment jadi PAID dan isi ke field process_date, process_reff_no, process_reff_name (tambah kolom baru di vat_payment)
									update	dbo.invoice_vat_payment
									set		status				= 'PAID'
											,process_date		= @mod_date
											,process_reff_no	= @code
								...
								else if (@payment_source_no = 'WITHHOLDING FOR OPERATING LEASE')
								begin
									--update withholding jadi PAID dan isi ke field process_date, process_reff_no, process_reff_name (tambah kolom baru di withholding)
