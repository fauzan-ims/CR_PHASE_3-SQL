CREATE PROCEDURE dbo.xsp_master_document_group_insert
(
	@p_code									nvarchar(50)  OUTPUT
	,@p_description							nvarchar(250)
	,@p_is_active							nvarchar(1)
	,@p_dim_count							int
	,@p_document_group_type_code			nvarchar(50)
...
begin
	...
	(
		select	1
		from	dbo.master_document_group
	...
	exec dbo.xsp_get_next_unique_code_for_table @p_code output, 
	                                            ...
	                                            @p_custom_prefix = N'MDG',                 
	                                            @p_year = @year,                          
	                                            @p_month = @month,                        
	                                            @p_table_name = N'MASTER_DOCUMENT_GROUP',                    
	...
	begin try
	insert into master_document_group
	(
		code
		,description
		,is_active
	...
	(
		...
		,@p_is_active
		,@p_dim_count
		,replace(@p_document_group_type_code, ' ', '')
		,@p_dim_1
		,@p_operator_1
		,@p_dim_value_from_1
		,@p_dim_value_to_1
