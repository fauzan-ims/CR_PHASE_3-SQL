CREATE PROCEDURE [dbo].[xsp_application_asset_getrow]
(
...
begin
	...
	select	@monthly_total_budget = sum(isnull(budget_amount, 0))
	...
	select	aa.asset_no
			...
			,aa.additional_charge_amount 'additional_charge_amount'
			,aa.lease_amount
			,aa.round_type
			,aa.round_amount
			,aa.lease_rounded_amount
			,aa.net_margin_amount
			,isnull(@monthly_total_budget, 0) + isnull(aa.gps_installation_amount, 0) 'monthly_total_budget'
			,isnull(mw.description, am.level_status) 'level_status'
			...
			,aa.otr_amount
			,aa.is_use_gps
			,aa.gps_monthly_amount
			,aa.gps_installation_amount
			,aa.monthly_rental_rounded_amount
			-- fauzan 12-02-2025
			,aa.client_nitku
			-- Louis Senin, 07 Juli 2025 14.33.30 -- 
			,asset_status
			,unit_source
			,start_due_date
			,prorate
			-- Louis Senin, 07 Juli 2025 14.33.35 -- 
