CREATE PROCEDURE dbo.xsp_agreement_asset_amortization_getrows_backup
(
...
BEGIN
	...
	from
			(
				select	ast.agreement_no 'agreement_no'
						...
						,ast.billing_amount
						,ast.invoice_no
						,CASE WHEN ast.BILLING_AMOUNT = 0 THEN ast.INVOICE_NO
							ELSE ai.invoice_external_no
							END 'invoice_external_no'
						...
						,CASE WHEN ast.BILLING_AMOUNT = 0 THEN 'PAID'
							ELSE ai.invoice_status
							END 'status'
				...
				from	agreement_asset_amortization ast with (nolock)
						OUTER APPLY
				(
					SELECT	MAX(aip.voucher_no) voucher_no
							,MAX(aip.payment_date) payment_date
							,MAX(aip.invoice_no) invoice_no
							,ISNULL(MAX(da.allocation_trx_date),ISNULL(MAX(ct.cashier_trx_date),ISNULL(MAX(sa.allocation_trx_date),MAX(aip.payment_date)))) cashier_trx_date
					FROM	agreement_invoice_payment aip with (nolock)
							...
							LEFT JOIN ifinfin.dbo.cashier_transaction ct with (nolock) ON (ct.voucher_no = aip.voucher_no)
							LEFT JOIN ifinfin.dbo.deposit_allocation da with (nolock) ON (da.voucher_no = aip.voucher_no)
							LEFT JOIN ifinfin.dbo.suspend_allocation sa with (nolock) ON (sa.voucher_no = aip.voucher_no)
							left join dbo.opl_interface_cashier_received_request oiprr with (nolock) on (oiprr.invoice_no = aip.invoice_no)
					WHERE	(
								...
								or oiprr.REQUEST_AMOUNT = 0)
							) GROUP BY aip.transaction_no
					HAVING	SUM(aip.PAYMENT_AMOUNT) > 0 OR SUM(aip.PAYMENT_AMOUNT) = 0
				) aaa
						outer apply
				(
	...
	(
		N'OVDP', N'LRAP'
						...
						left join dbo.invoice ai with (nolock) on (
	...
	(
		'NEW', 'CANCEL'
				...
				select	aind.AGREEMENT_NO 'agreement_no'
						...
						,ao.obligation_day
						,ao.obligation_amount
				from	dbo.INVOICE ain with (nolock)
						inner join dbo.INVOICE_DETAIL aind with (nolock) on aind.invoice_no = ain.INVOICE_NO
						outer apply
						(
							...
							from	dbo.agreement_invoice_payment ainp with (nolock)
									left join ifinfin.dbo.cashier_transaction ct with (nolock) on (ct.voucher_no = ainp.voucher_no)
									left join ifinfin.dbo.deposit_allocation da with (nolock) on (da.voucher_no = ainp.voucher_no)
									left join ifinfin.dbo.suspend_allocation sa with (nolock) on (sa.voucher_no = ainp.voucher_no)
							where	(
