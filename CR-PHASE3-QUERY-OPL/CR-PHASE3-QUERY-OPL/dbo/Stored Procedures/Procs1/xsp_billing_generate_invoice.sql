...
/*
exec dbo.xsp_billing_generate_invoice @p_code = N'' -- nvarchar(50)
...
CREATE PROCEDURE dbo.xsp_billing_generate_invoice
(
	@p_code		nvarchar(50)
	--
	,@p_mod_date			datetime
...
begin
	...
	begin try 
		...
		begin
			...
			select	distinct bg.agreement_no
					...
					--,aa.billing_to_name
					--,am.client_name -- (+) Ari 2023-09-13 ket : change billing name to client name (-) ini gak boleh gini karna billing to name digunakan untuk select dibawah
 					,am.credit_term
					,case
			...
			begin 
				...
				(
					...
					where	inv.generate_code	  = @p_code
							...
							and inv.client_name	  = @client_name
				...
				begin 
					-- Louis Senin, 25 September 2023 19.16.41 -- untuk insert ke invoice
				...
				begin
					...
					--jika potong pph 
					if (@is_invoice_deduct_pph = '1' or @settlement_type = 'PKP' )
					begin
