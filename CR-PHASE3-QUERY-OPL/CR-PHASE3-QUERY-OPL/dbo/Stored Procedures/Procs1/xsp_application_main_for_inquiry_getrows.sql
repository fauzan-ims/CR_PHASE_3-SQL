CREATE PROCEDURE dbo.xsp_application_main_for_inquiry_getrows
(
	...
	,@p_application_status nvarchar(50) = 'ALL'
	,@p_level_status	   nvarchar(50) = 'ALL'
...
begin
	...
	where	ap.branch_code			  = case @p_branch_code
			...
			and ap.application_status = case @p_application_status
											when 'ALL' then ap.application_status
											else @p_application_status
										end
			and ap.level_status		  = case @p_level_status
											when 'ALL' then ap.level_status
											else @p_level_status
										end
	...
	select		ap.application_no
				...
				,isnull(ap.client_name, cm.client_name) 'client_name'
				,ap.branch_name
