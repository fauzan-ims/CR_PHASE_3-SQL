...
-- Louis Rabu, 14 Desember 2022 11.59.10 -- 
CREATE PROCEDURE dbo.xsp_application_asset_purchase_request
(
...
BEGIN
	...
	begin try
		...
		begin
			...
			begin
				...
				if not exists (
					...
					from	dbo.purchase_request
					where	asset_no = @p_asset_no
							and	categori_type = @category_type
							and request_status <> 'CANCEL'
							and fa_category_name = @fa_category_name
					)
				begin
		...
		--push ke interface final grn request for cr_priority
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[xsp_application_asset_purchase_request] TO [ims-raffyanda]
    AS [dbo];
