CREATE PROCEDURE dbo.xsp_application_asset_update
(
...
begin
	...
	declare @msg						   nvarchar(max)
			...
			,@sum_year_akhir_tenor			int
			,@unit_source					nvarchar(15)
	...
	begin try 
		...
		if(@p_billing_mode NOT IN ('NORMAL', 'END MONTH'))
		begin
			if (
		...
		if (@unit_source= 'STOCK')--@asset_condition = 'USED')
		begin
			...
			set @asset_purchase_month = datediff(month,@purchase_date,dbo.xfn_get_system_date())

			--sepria 02052025: nilai otr diambil dari tahun penggunaan unit dan jika sudah 12bulan dikali dengan rate RV pada tahun pemakaian sampai tahun pengajuan (hanya tahun)
			--set @sum_asset_year = floor(@asset_purchase_month / 12)
			set @sum_asset_year = year(dbo.xfn_get_system_date()) - year(@purchase_date)
			...
			--sepria 02052025: untuk nilai RV PCT dan AMOUNT, ambil nilai rv rate berdasarkan ((tenor + tahun pengajuan) - tahun kendaraan) tahun diakhir kontrak
			--set @p_asset_rv_pct = isnull(@asset_rv_rate,0)
			set @sum_year_akhir_tenor = (convert(decimal(18,2), @periode/12) + (year(dbo.xfn_get_system_date()))) - @asset_year ; 

			select	@p_asset_rv_pct = case
										 when @p_transmisi = 'AT' then rv_rate_at
										 else rv_rate
									 end
			from	ifinbam.dbo.master_model_rv
			where	model_code = @p_model_code
					and year   = case
									 --when @sum_asset_year = 0 then 1
									 when @sum_year_akhir_tenor = 0 then 0
									 else @sum_year_akhir_tenor
								 end ;
		end ;
		else
		begin
		...
		if (@spaf_rate > 0 and @unit_source <> 'STOCK')--@asset_condition <> 'USED')
		begin
