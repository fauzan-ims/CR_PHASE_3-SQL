...
begin
	...
	select	replace(aalr.agreement_no,'.','/')
			...
			,late_return_days
			,os_obligation_amount
			,case when aalr.os_obligation_amount - (CASE WHEN inv.invoice_status = 'PAID' then	ISNULL(aalr.invoice_amount,0) ELSE 0 END)
												 - (CASE WHEN cn.STATUS = 'POST' then ISNULL(aalr.credit_amount,0) ELSE 0 END)
												 - (CASE WHEN wv.WAIVED_STATUS = 'APPROVE' then ISNULL(aalr.waive_amount,0) ELSE 0 END) = 0 then 'PAID'
