CREATE PROCEDURE dbo.xsp_agreement_main_getrows_for_pending_billing
(
...
begin
	...
	and		is_pending_billing = case	@p_invoice_status
								 when	'ALL' then is_pending_billing
	...
	select		am.agreement_no                    
				...
				,case 
					am.is_pending_billing	when '1' then 'PENDING'
										when '0' then 'NORMAL'
										else ''
				 end 'is_pending'
				,@rows_count 'rowcount'
	...
	order by	case 
					when @p_sort_by='asc' then	case @p_order_by
													...
													when 3 then facility_name
													when 4 then cast(agreement_date as sql_variant)   
													when 5 then cast(termination_date as sql_variant)
													when 6 then agreement_status
												end		                    	 
					end asc 
				,case 
