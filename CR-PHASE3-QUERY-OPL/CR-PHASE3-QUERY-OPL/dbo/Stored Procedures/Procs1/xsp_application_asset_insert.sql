CREATE PROCEDURE dbo.xsp_application_asset_insert
	(
		...
		-- Louis Senin, 07 Juli 2025 13.59.25 -- 
		,@p_unit_source							nvarchar(20)  = ''
		----,@p_start_due_date						datetime
		,@p_prorate								nvarchar(10)  = ''
		-- Louis Senin, 07 Juli 2025 13.59.27 -- 
		--
		,@p_cre_date						    datetime
	...
	begin
		...
		begin try
			...
			-- Louis Senin, 07 Juli 2025 14.01.46 -- 
			...
			if (@p_unit_source='STOCK')--(@p_asset_condition = 'USED') OR 
			begin
			...
			if (
					@p_otr_amount <= 0 and @p_unit_source <> 'STOCK' --@p_asset_condition <> 'USED'
				)
			begin
				set @msg = 'OTR Amount must be greater than 0' ;
			...
			if (@p_asset_rv_pct > 0)
			begin
				if (@p_unit_source = 'STOCK')--@p_asset_condition = 'USED')
				begin
			...
			(
				...
				,client_nitku
				-- Louis Senin, 07 Juli 2025 14.02.07 -- 
				,unit_source
				--,start_due_date
				,prorate
				,asset_status
				-- Louis Senin, 07 Juli 2025 14.02.14 -- 
				--
				,cre_date
				,cre_by
				,cre_ip_address
			...
			begin
				...
				if (@p_asset_type_code = 'VHCL') --jika asset type nya vehicle
				begin
				...
				else if (@p_asset_type_code = 'ELEC') --jika type asset nya electric
				begin
				...
				else if (@p_asset_type_code = 'HE') --jika type asset nya heavy equipment
				begin
				...
				else if (@p_asset_type_code = 'MCHN') --jika type asset nya machine
				begin
				...
				set @remark_log = 'Insert Asset : ' + ISNULL(@code,'') + ' ' + ISNULL(@unit_desc,'') + ' Asset Year : ' + ISNULL(@p_asset_year,'');
