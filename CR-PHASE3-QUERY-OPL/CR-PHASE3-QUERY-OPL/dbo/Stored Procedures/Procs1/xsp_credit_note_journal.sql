CREATE PROCEDURE dbo.xsp_credit_note_journal
(
...
begin
	...
	begin try
		select	@branch_code = cn.branch_code
				...
				,@process_code = case
									 when isnull(is_journal, '0') = '1' then 'INVCRENT'
									 else 'INVCRENTND'
								 end
				,@is_journal_ppn_wapu = isnull(inv.is_journal_ppn_wapu, '0')
				,@invoice_external_no = inv.invoice_external_no
				,@invoice_name = inv.invoice_name
				,@invoice_no = inv.INVOICE_NO
		from	dbo.credit_note cn
				inner join dbo.invoice inv on (inv.invoice_no = cn.invoice_no)
		where	cn.code = @p_reff_code ;
		
		set @transaction_name = @p_reff_name + ' For : ' + @invoice_external_no
		...
		begin
			...
			select  mt.sp_name
					,mtp.debet_or_credit
					,mtp.gl_link_code
					,mt.transaction_name + ' - ' + ind.description
					,ind.id
					,ind.agreement_no
					,ind.asset_no
					,ind.description
			...
			from	dbo.master_transaction_parameter mtp 
					...
					inner join dbo.credit_note_detail cnd on (cnd.credit_note_code = @p_reff_code and cnd.adjustment_amount > 0)
					inner join dbo.invoice_detail ind on (ind.id = cnd.invoice_detail_id)
			...
			begin
					...
					if (
							isnull(@gl_link_code, '') = 'OLARWND'
							or	isnull(@gl_link_code, '') = 'OLARW'
						)
					begin
						if (@is_journal_ppn_wapu = '1')
						begin
							set @gl_link_code = N'PPNOUT' ;
						end ;
						else
						begin
							set @gl_link_code = N'OLARWND' ;
						end ;
					end ;
					...
					begin
						...
						set @transaction_name = @p_reff_name + ' For Asset : ' + @asset_no + ' ' + @description
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[xsp_credit_note_journal] TO [ims-raffyanda]
    AS [dbo];
