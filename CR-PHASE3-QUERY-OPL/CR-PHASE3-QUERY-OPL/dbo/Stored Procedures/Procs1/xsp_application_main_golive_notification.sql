CREATE PROCEDURE dbo.xsp_application_main_golive_notification
(
...
begin
	...
	begin try
		...
		select	@email					= 'chaprir4@gmail.com' --isnull(cci.email, cpi.email)
				...
				,@vendor_phone_no		= ''
				,@recipient_name		= isnull(cci.full_name, cpi.full_name)
				,@client_name			= isnull(cci.full_name, cpi.full_name)
		...
		set @header_message = 'Application Golive No. ' + @p_application_no + ' - ' + ' Facility ' + @facility_name ;
		...
		exec dbo.xsp_opl_interface_notification_request_insert @p_id						 = @id output -- bigint
															   ...
															   ,@p_transaction_type_code	 = N'GOLIVE' -- nvarchar(50)
