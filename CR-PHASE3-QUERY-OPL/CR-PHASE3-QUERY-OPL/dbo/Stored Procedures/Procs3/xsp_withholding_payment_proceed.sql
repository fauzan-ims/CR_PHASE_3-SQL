CREATE PROCEDURE dbo.xsp_withholding_payment_proceed
(
...
begin
	...
	begin try
		...
		--select data tax bank dari global param
		...
		select @total_pph_amount		= total_pph_amount
		...
		begin
			select	@withhold_code = code
			from	dbo.invoice_pph_payment
			where	status = 'HOLD'
			and		currency_code = @currency_code ;
			
			exec dbo.xsp_invoice_pph_payment_detail_insert @p_id						= 0
														   ,@p_tax_payment_code			= @withhold_code
														   ,@p_invoice_no				= @p_invoice_no
														   ,@p_pph_amount				= @total_pph_amount
			...
			update	dbo.invoice
			set		payment_pph_code	= @withhold_code
					,payment_pph_date	= @date
					--
					,cre_date			= @p_cre_date
					,cre_by				= @p_cre_by
		...
		begin
			...
			exec dbo.xsp_get_next_unique_code_for_table @p_unique_code			= @code output
														...
														,@p_custom_prefix		= N'IVPH'
														,@p_year				= @year 
														,@p_month				= @month 
														,@p_table_name			= N'INVOICE_PPH_PAYMENT' 
														,@p_run_number_length	= 6 
														,@p_delimiter			= '.'
														,@p_run_number_only		= N'0'

			exec dbo.xsp_invoice_pph_payment_insert @p_code						= @code
													,@p_branch_code				= @branch_code
													,@p_branch_name				= @branch_name
													,@p_status					= 'HOLD'
													,@p_date					= @date
													,@p_remark					= 'WITHHOLDING PAYMENT'
													,@p_total_pph_amount		= @total_pph_amount
													,@p_process_date			= null
													,@p_process_reff_no			= ''
													,@p_process_reff_name		= ''
