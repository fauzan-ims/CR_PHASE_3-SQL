--created by, Rian at 29/05/2023 

CREATE PROCEDURE dbo.xsp_realization_return
(
	@p_code			   nvarchar(50)
	,@p_status		   nvarchar(15)
	,@p_result		   nvarchar(4000) = null
	--
	,@p_mod_date	   datetime
	,@p_mod_by		   nvarchar(15)
	,@p_mod_ip_address nvarchar(15)
)
as
begin
	declare @msg nvarchar(max) ;

	begin try

		--validasi jika remark tidak di isi
		if (isnull(@p_result , '') = '' and @p_status = 'VERIFICATION')
		begin
			set @msg = 'Please Insert Result' ;

			raiserror(@msg, 16, -1) ;
		end ;

		if (@p_status = 'VERIFICATION')
		begin
			set @p_status = 'ON PROCESS'
		end
		else if (@p_status = 'ON PROCESS')
		begin
			set @p_status = 'HOLD'
		end

		--update status menjadi on process
		update	dbo.realization
		set		status			= @p_status
				,RESULT			= @p_result
				--
				,mod_date		= @p_mod_date
				,mod_by			= @p_mod_by
				,mod_ip_address = @p_mod_ip_address 
		where	code			= @p_code
	end try
	begin catch
		declare @error int ;

		set @error = @@error ;

		if (@error = 2627)
		begin
			set @msg = dbo.xfn_get_msg_err_code_already_exist() ;
		end ;

		if (len(@msg) <> 0)
		begin
			set @msg = 'V' + ';' + @msg ;
		end ;
		else
		begin
			if (
				   error_message() like '%V;%'
				   or	error_message() like '%E;%'
			   )
			begin
				set @msg = error_message() ;
			end ;
			else
			begin
				set @msg = 'E;' + dbo.xfn_get_msg_err_generic() + ';' + error_message() ;
			end ;
		end ;

		raiserror(@msg, 16, -1) ;

		return ;
	end catch ;
end ;
