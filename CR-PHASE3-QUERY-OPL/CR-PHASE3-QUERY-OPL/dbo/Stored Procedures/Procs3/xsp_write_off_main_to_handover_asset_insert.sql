CREATE PROCEDURE dbo.xsp_write_off_main_to_handover_asset_insert
(
	@p_code			   nvarchar(50)
	,@p_agreement_no   nvarchar(50)
	,@p_wo_date		   nvarchar(50)
	--
	,@p_cre_date	   datetime
...
begin
	...
	begin try
		...
		begin
			...
			set @remark = 'Penarikan Unit Sewa, Write Off Untuk Agreement No :  ' + @agreement_external_no + '. dari Asset : ' + @fa_code + ' - ' + @fa_name + '.'
			
			exec dbo.xsp_opl_interface_handover_asset_insert @p_code					= @handover_asset_code output
															 ,@p_branch_code			= @branch_code
															 ,@p_branch_name			= @branch_name
															 ,@p_status					= N'HOLD'  
															 ,@p_transaction_date		= @p_wo_date
															 ,@p_type					= N'PICK UP'
															 ,@p_remark					= @remark
															 ,@p_fa_code				= @fa_code
															 ,@p_fa_name				= @fa_name
