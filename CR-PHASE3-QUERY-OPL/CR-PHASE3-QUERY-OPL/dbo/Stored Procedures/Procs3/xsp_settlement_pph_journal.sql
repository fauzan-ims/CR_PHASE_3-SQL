CREATE PROCEDURE dbo.xsp_settlement_pph_journal
(
	...
	begin try
		...
		begin
			...
			from	dbo.master_transaction_parameter mtp 
					...
					inner join dbo.agreement_main agm on (agm.agreement_no = ind.agreement_no)
			...
			begin
					...
					if(@payment_type = 'ADV')
					begin
						if(@code_trans = 'INV_AR')
						begin
							set @return_value = 0
						end
					end
					else if(@payment_type = 'ARR')
					begin
						if(@code_trans = 'INV_PDD')
						begin
							set @return_value = 0
						end
					end
