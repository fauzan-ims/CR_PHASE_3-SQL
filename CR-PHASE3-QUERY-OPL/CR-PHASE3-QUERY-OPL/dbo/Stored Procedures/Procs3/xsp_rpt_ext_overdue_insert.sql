CREATE PROCEDURE dbo.xsp_rpt_ext_overdue_insert
as
begin
	declare @msg			   nvarchar(max)
			--,@p_cre_date	   datetime		= '2023-12-31'
			--,@asof_date		   nvarchar(8)	= N'20231231'
			,@p_cre_date	   datetime		= DATEADD(DAY, -9, dbo.xfn_get_system_date())
	...
	begin try
		...
		select		eomonth(@p_cre_date)
					...
					,@p_cre_by
					,@p_cre_ip_address
		FROM	dbo.agreement_main					   am WITH (NOLOCK) --	on am.agreement_no = ai.agreement_no
		inner join dbo.agreement_asset		   aa WITH (NOLOCK) on aa.agreement_no = am.agreement_no -- and ai.ASSET_NO = aa.ASSET_NO 
		inner join dbo.agreement_asset_vehicle aav WITH (NOLOCK) on (aav.asset_no = aa.asset_no)
		outer apply
		(
			select		--top 1
