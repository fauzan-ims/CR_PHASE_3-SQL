...
CREATE PROCEDURE dbo.xsp_rpt_list_faktur_pajak_ar_detail
(
...
---------------------------------------------------------------------------------------------------
--fauzan 06032025
DECLARE @XmlAwal NVARCHAR(MAX);
DECLARE @XmlAkhir NVARCHAR(MAX);
DECLARE @XmlData NVARCHAR(MAX);
DECLARE @Data NVARCHAR(MAX);

SET @XmlAwal = '<TaxInvoiceBulk xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="TaxInvoice.xsd"><TIN>XXXXXXXXXXXXXXXX</TIN><ListOfTaxInvoice>';
SET @XmlAkhir = '</ListOfTaxInvoice></TaxInvoiceBulk>';
SET @XmlData = @XmlAwal;

DECLARE cursor_faktur CURSOR FOR
SELECT
    '<TaxInvoice><TaxInvoiceDate>' + CONVERT(VARCHAR(10), GETDATE(), 120) + '</TaxInvoiceDate><TaxInvoiceOpt>Normal</TaxInvoiceOpt><TrxCode>01</TrxCode><AddInfo><Data>' +
    CASE
        WHEN t.user_id IS NULL AND t.kode_objek IS NULL AND t.nama_trx IS NULL AND t.harga_satuan IS NULL AND t.jumlah_barang IS NULL AND t.harga_total IS NULL AND t.diskon IS NULL AND t.dpp IS NULL AND t.ppn IS NULL AND t.tarif_ppnbm IS NULL AND t.ppnbm IS NULL AND t.invoice_code IS NULL AND t.jumlah_barang_str IS NULL AND t.referensi IS NULL
        THEN 'NULL'
        ELSE
            '<user_id>' + ISNULL(CONVERT(NVARCHAR(50), t.user_id), 'NULL') + '</user_id>' +
            '<kode_objek>' + ISNULL(t.kode_objek, 'NULL') + '</kode_objek>' +
            '<nama_trx>' + ISNULL(t.nama_trx, 'NULL') + '</nama_trx>' +
            '<harga_satuan>' + ISNULL(CONVERT(NVARCHAR(50), t.harga_satuan), 'NULL') + '</harga_satuan>' +
            '<jumlah_barang>' + ISNULL(CONVERT(NVARCHAR(50), t.jumlah_barang), 'NULL') + '</jumlah_barang>' +
            '<harga_total>' + ISNULL(CONVERT(NVARCHAR(50), t.harga_total), 'NULL') + '</harga_total>' +
            '<diskon>' + ISNULL(CONVERT(NVARCHAR(50), t.diskon), 'NULL') + '</diskon>' +
            '<dpp>' + ISNULL(CONVERT(NVARCHAR(50), t.dpp), 'NULL') + '</dpp>' +
            '<ppn>' + ISNULL(CONVERT(NVARCHAR(50), t.ppn), 'NULL') + '</ppn>' +
            '<tarif_ppnbm>' + ISNULL(CONVERT(NVARCHAR(50), t.tarif_ppnbm), 'NULL') + '</tarif_ppnbm>' +
            '<ppnbm>' + ISNULL(CONVERT(NVARCHAR(50), t.ppnbm), 'NULL') + '</ppnbm>' +
            '<invoice_code>' + ISNULL(CONVERT(NVARCHAR(50), t.invoice_code), 'NULL') + '</invoice_code>' +
            '<jumlah_barang_str>' + ISNULL(CONVERT(NVARCHAR(50), t.jumlah_barang_str), 'NULL') + '</jumlah_barang_str>' +
            '<referensi>' + ISNULL(CONVERT(NVARCHAR(50), t.referensi), 'NULL') + '</referensi>'
    END +
    '</Data></AddInfo></TaxInvoice>'
FROM
    dbo.rpt_list_faktur_pajak_ar_detail_detail AS t;

OPEN cursor_faktur;
FETCH NEXT FROM cursor_faktur INTO @Data;
...
CLOSE cursor_faktur;
DEALLOCATE cursor_faktur;

SET @XmlData = @XmlData + @XmlAkhir;
