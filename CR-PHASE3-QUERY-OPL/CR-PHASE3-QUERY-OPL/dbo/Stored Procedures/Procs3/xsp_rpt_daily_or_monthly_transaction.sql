...
CREATE PROCEDURE [dbo].[xsp_rpt_daily_or_monthly_transaction]
(
...
BEGIN
	...
	begin TRY
		...
		set	@report_title = 'Report Daily or Monthly Transaction'

		insert into dbo.rpt_daily_or_monthly_transaction
		(
			user_id
			,report_company
			,report_image
		...
		select	@p_user_id
				...
				,am.periode
				,am.agreement_date
				,ags.asset_interest_rate--borrowing_interest_rate
				,ags.roa_pct
				,ags.asset_rv_pct
		...
		WHERE	am.agreement_status = 'GO LIVE'
				AND	am.branch_code = CASE @p_branch_code
									...
									ELSE @p_branch_code
								END	
				AND	day (am.agreement_date) =  case 
									WHEN @p_date = '' THEN day(am.agreement_date) 
									ELSE @p_date
								END	
				and month(am.agreement_date) = @month
				and year(am.agreement_date)	= @p_year
                
		if not exists (select * from dbo.rpt_daily_or_monthly_transaction where user_id = @p_user_id)
		begin
				insert into dbo.rpt_daily_or_monthly_transaction
				(
				    user_id
