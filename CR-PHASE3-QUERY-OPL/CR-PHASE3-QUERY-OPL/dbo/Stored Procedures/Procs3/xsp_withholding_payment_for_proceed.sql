CREATE PROCEDURE dbo.xsp_withholding_payment_for_proceed
(
	@p_code					   nvarchar(50)
	--
	,@p_mod_date			   datetime
...
begin
	declare @msg								nvarchar(max) 
			...
			,@pph_amount_detail					decimal(18,2)
			,@description						nvarchar(4000)
			,@remark							nvarchar(4000);

	begin try
		select @invoice_no					= ippd.invoice_no
			  ,@invoice_external_no			= inv.invoice_external_no
			  ,@pph_amount					= ippd.pph_amount
			  ,@branch_code					= ipp.branch_code
			  ,@branch_name					= ipp.branch_name
			  ,@currency					= inv.currency_code
			  ,@agreement_no				= ind.agreement_no
			  ,@asset_no					= ind.asset_no
			  ,@total_amount				= isnull(ipp.total_pph_amount,0)
		...
		from dbo.invoice_pph_payment_detail ippd
		left join dbo.invoice_pph_payment ipp on (ipp.code = ippd.tax_payment_code)
		left join dbo.invoice inv on (inv.invoice_no = ippd.invoice_no)
		...
		set @remark		=  'Payment WITHHOLDING from Invoice Operating lease '+ @p_code
		...
		exec dbo.xsp_opl_interface_payment_request_insert @p_code					= @code output
														  ...
														  ,@p_payment_source		= 'WITHHOLDING FOR OPERATING LEASE'
			...
			begin
				...
				into @pph_amount_detail
					,@description
				
				while @@fetch_status = 0
				begin
				    set @remarks = 'Withhold Payment Invoice No ' + isnull(@invoice_external_no,'') + ' Faktur No ' + isnull(@faktur_no,'') + ' Agreement ' + isnull(@agreement_no,'') + ' ' + isnull(@client_name,'') + ' ' + isnull(@asset_no,'') + ' ' + isnull(@asset_name,'') + ' - ' + @description;
					exec dbo.xsp_opl_interface_payment_request_detail_insert @p_id							= 0
																			 ...
																			 ,@p_orig_amount				= @pph_amount_detail
																			 ,@p_division_code				= ''
																			 ,@p_division_name				= ''
																			 ,@p_department_code			= ''
																			 ,@p_department_name			= ''
