...
CREATE PROCEDURE dbo.xsp_rpt_surat_peringatan_i
(
...
begin
	...
	begin try
		...
		insert into dbo.rpt_surat_peringatan_i
		(
			user_id
			,report_image
			,report_title
		...
		select	distinct
				...
				,wl.letter_date
				,am.client_name
				,agas.billing_to_address --cad.addresinv.client_address
				,''--cci.full_name
				...
				,invd.description
				,invd.billing_no
				,inv.total_billing_amount + inv.total_ppn_amount --invd.billing_amount + invd.ppn_amount
				,0--ao.obligation_amount
		...
		IF NOT EXISTS (SELECT * FROM dbo.RPT_SURAT_PERINGATAN_I WHERE user_id = @p_user_id)
		BEGIN
			INSERT INTO dbo.rpt_surat_peringatan_i
		(
			user_id
		...
		SELECT	DISTINCT
				...
				,wl.letter_date
				,am.client_name
				,agas.billing_to_address --cad.ADDRESS,--inv.client_address
				,''--cci.full_name
				,am.agreement_external_no
				,am.agreement_date
		...
		insert into dbo.rpt_surat_peringatan_i_lampiran_i
		(
			user_id
			,nomor_surat
		...
		if not exists(SELECT * FROM dbo.RPT_SURAT_PERINGATAN_I_LAMPIRAN_I where user_id=@p_user_id)
		begin
				insert into dbo.rpt_surat_peringatan_i_lampiran_i
		(
			user_id
