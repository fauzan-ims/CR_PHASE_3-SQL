...
begin
	...
	and		inv.invoice_status not in ('cancel','reject')
	...
	order by	case
					when @p_sort_by = 'asc' then case @p_order_by
													...
													when 6 then cast(case when inv.invoice_status = 'post' then (case when cast(inv.invoice_due_date as date) < cast(dbo.xfn_get_system_date() as date) then datediff(day, inv.invoice_due_date, dbo.xfn_get_system_date()) else 0 end)
																when inv.invoice_status = 'paid' then (case when cast(inv.invoice_due_date as date) < cast(agp.payment_date as date) then datediff(day, inv.invoice_due_date, agp.payment_date) else 0 end)
