/*
exec xsp_regenerate_invoice_journal_not_due_to_due
*/
-- Louis Selasa, 13 Juni 2023 13.44.52 -- 
CREATE PROCEDURE dbo.xsp_regenerate_invoice_journal_not_due_to_due
...
begin
	declare @msg				  nvarchar(max)
			...
			,@reff_name			  nvarchar(50)	= N'MTN AR DUE INVOICE'
	...
	begin try
		begin 
			...
			begin
				...
				set @transaction_name = @reff_name + N' For : ' + replace(@p_invoice_no, '.', '/') + N' - ' + N'. Periode : ' + cast(year(@eod_date) as nvarchar(4)) + cast(month(@eod_date) as nvarchar(4)) ;
				-- sepria 12mar2024: penomoran transaction code dijadikan unik, di dalam sp insertnya di tambahkan kode transaksi
				set @transaction_code = 'ARDUE.' + replace(@p_invoice_no, '.', '/') + '.' + cast(year(@eod_date) as nvarchar(4)) + cast(month(@eod_date) as nvarchar(4)) ;
