-- Louis Rabu, 28 Desember 2022 16.01.07 -- 
CREATE PROCEDURE [dbo].[xsp_write_off_main_jurnal]
(
	...
	begin try
		...
		begin
			...
			select	mtp.gl_link_code
					,case ddt.transaction_code
						 when 'SCDP' then rv.transaction_amount
						 else ddt.transaction_amount
					 end
					,mtp.debet_or_credit
					,mt.transaction_name
			from	dbo.write_off_transaction ddt
					inner join dbo.master_transaction_parameter mtp on (
																		   mtp.transaction_code = ddt.transaction_code
			...
			(
				...
				where	wo_code				 = @p_reff_code
						and transaction_code = 'RSDV'
			) rv
			where	ddt.wo_code		   = @p_reff_code
