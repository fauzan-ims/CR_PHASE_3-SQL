...
CREATE PROCEDURE dbo.xsp_rpt_invoice_denda_keterlambatan
(
...
begin
	...
	begin try
		...
		set @report_title = 'INVOICE DENDA KETERLAMBATAN PEMBAYARAN SEWA KENDARAAN' ;
		...
		insert into dbo.rpt_invoice_denda_keterlambatan
		(
			user_id
			,agreement_no
			,report_company
		...
		select	@p_user_id
				...
				,@npwp_no
				,dbo.xfn_bulan_indonesia(aam.start_sewa)
				,dbo.xfn_bulan_indonesia(aam.end_sewa)
				,dbo.xfn_bulan_indonesia(aam.start_sewa)
				...
				,sbs.signer_name
				,sp.description
				,'Denda Keterlambatan Pembayaran Sewa ' + cast(aas.jumlah as nvarchar(50)) + ' Kendaraan'
				,dbo.terbilang(ai.ovd_penalty_amount)
				,'JAKARTA'
		...
		insert into dbo.rpt_invoice_denda_keterlambatan_detail
		(
			user_id
			,jenis
			,type
