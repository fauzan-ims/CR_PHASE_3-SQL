CREATE procedure dbo.xsp_repossession_letter_for_settlement_getrows
(
...
begin
	...
	where	--rl.is_remedial = @p_is_remedial
			...
			and (
					...
					or	rl.letter_no								like '%' + @p_keywords + '%'
					or	rl.letter_status							like '%' + @p_keywords + '%'
					or	rl.result_status							like '%' + @p_keywords + '%'
				) ;
		...
		order by	case
					when @p_sort_by = 'asc' then case @p_order_by
														...
														when 4 then rl.letter_no								
														when 5 then rl.letter_status							
														when 6 then rl.result_status
												 end
				end asc
				,case
