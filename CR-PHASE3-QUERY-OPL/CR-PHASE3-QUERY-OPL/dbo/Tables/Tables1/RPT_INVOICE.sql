CREATE TABLE [dbo].[RPT_INVOICE] (
    [USER_ID]               NVARCHAR (50)   NOT NULL,
    [REPORT_COMPANY]        NVARCHAR (250)  NOT NULL,
    [REPORT_TITLE]          NVARCHAR (250)  NOT NULL,
    [REPORT_IMAGE]          NVARCHAR (250)  NOT NULL,
    [COMPANY_ADDRESS]       NVARCHAR (4000) NULL,
    [COMPANY_PHONE_AREA]    NVARCHAR (5)    NULL,
    [COMPANY_PHONE_NO]      NVARCHAR (15)   NULL,
    [INVOICE_NO]            NVARCHAR (50)   NOT NULL,
    [BRANCH_NAME]           NVARCHAR (250)  NOT NULL,
    [INVOICE_TYPE]          NVARCHAR (10)   NOT NULL,
    [INVOICE_DATE]          DATETIME        NOT NULL,
    [INVOICE_DUE_DATE]      DATETIME        NOT NULL,
    [INVOICE_NAME]          NVARCHAR (250)  NOT NULL,
    [CLIENT_NAME]           NVARCHAR (250)  NOT NULL,
    [CLIENT_ADDRESS]        NVARCHAR (4000) NOT NULL,
    [CLIENT_AREA_PHONE_NO]  NVARCHAR (4)    NOT NULL,
    [CLIENT_PHONE_NO]       NVARCHAR (15)   NOT NULL,
    [CLIENT_NPWP]           NVARCHAR (50)   NOT NULL,
    [CURRENCY_CODE]         NVARCHAR (3)    NOT NULL,
    [TOTAL_BILLING_AMOUNT]  DECIMAL (18, 2) NOT NULL,
    [CREDIT_BILLING_AMOUNT] DECIMAL (18, 2) NULL,
    [TOTAL_DISCOUNT_AMOUNT] DECIMAL (18, 2) NOT NULL,
    [TOTAL_PPN_AMOUNT]      DECIMAL (18, 2) NOT NULL,
    [CREDIT_PPN_AMOUNT]     DECIMAL (18, 2) NULL,
    [TOTAL_PPH_AMOUNT]      DECIMAL (18, 2) NOT NULL,
    [CREDIT_PPH_AMOUNT]     DECIMAL (18, 2) NULL,
    [STAMP_DUTY_AMOUNT]     DECIMAL (18, 2) CONSTRAINT [DF_RPT_INVOICE_STAMP_DUTY_AMOUNT] DEFAULT ((0)) NULL,
    [TOTAL_AMOUNT]          DECIMAL (18, 2) NOT NULL,
    [FAKTUR_NO]             NVARCHAR (50)   NULL,
    [BANK_NAME]             NVARCHAR (4000) NULL,
    [BANK_ACCOUNT_NAME]     NVARCHAR (50)   NULL,
    [BANK_ACCOUNT_NO]       NVARCHAR (20)   NULL,
    [DESCRIPTION]           NVARCHAR (4000) NULL,
    [CRE_DATE]              DATETIME        NOT NULL,
    [CRE_BY]                NVARCHAR (15)   NOT NULL,
    [CRE_IP_ADDRESS]        NVARCHAR (15)   NOT NULL,
    [MOD_DATE]              DATETIME        NOT NULL,
    [MOD_BY]                NVARCHAR (15)   NOT NULL,
    [MOD_IP_ADDRESS]        NVARCHAR (15)   NOT NULL
);

