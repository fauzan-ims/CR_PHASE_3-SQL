...
CREATE PROCEDURE dbo.xsp_deposit_allocation_reject
(
	@p_code					nvarchar(50)
	--
	,@p_mod_date			datetime
...
begin
	...
	begin try
			...
			--if exists (select 1 from dbo.deposit_allocation where code = @p_code and allocation_status <> 'HOLD')
		if exists (select 1 from dbo.deposit_allocation where code = @p_code and allocation_status not in ('HOLD','ON PROCESS')) -- (+) Ari 2024-01-22 ket : jika sudah diproses maka status jadi ON PROCESS
		begin
		...
		begin
			-- jika transaksi detail ada dari cashier received request, update statusnya jadi hold
