CREATE PROCEDURE dbo.xsp_fin_interface_account_transfer_get_todo
(
...
begin
	...
	begin try
		select		@p_todo_code 'todo_code'
					...
					,'PENDING FIN INTERFACE ACCOUNT TRANSFER'
					,count(1) as count
		from		fin_interface_account_transfer
		where		transfer_status = 'HOLD'
					and cre_by		= @p_user_id
		group by	isnull(from_branch_name, to_branch_name) ;
	end try
	begin catch
		declare @error int ;
