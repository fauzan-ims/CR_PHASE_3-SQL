/*
exec xsp_job_interface_in_ifinfin_approval_request
*/
CREATE PROCEDURE dbo.xsp_job_interface_in_ifinfin_approval_request
as
	...
	where	sp_name = 'xsp_job_interface_in_ifinfin_approval_request' -- sesuai dengan nama sp ini
	...
	begin
		...
		select		id
					...
					,request_status
		from		dbo.fin_interface_approval_request
		where		settle_date is null
					and request_status = 'POST'
					and isnull(approval_status, '') <> ''
					and job_status in ('HOLD','FAILED')
					AND id NOT IN (1186,1187) --diexclude karena sudah dikonfirm langsung ke bu oliv, tapi belum ada action dari user
			...
			begin try
				begin transaction
					...
					begin
						...
						exec dbo.xsp_fin_interface_approval_request_approve @p_code				= @reff_no
					...
					begin
						exec dbo.xsp_fin_interface_approval_request_return @p_code				= @reff_no
																			,@p_approval_status	= @approval_status
																			,@p_approval_code	= @approval_code
																			,@p_request_status	= @request_status
																			,@p_mod_date		= @mod_date		
					...
					begin
						exec dbo.xsp_fin_interface_approval_request_reject @p_code				= @reff_no
																			,@p_approval_status	= @approval_status
																			,@p_approval_code	= @approval_code
																			,@p_request_status	= @request_status
																			,@p_mod_date		= @mod_date		
					...
					--cek poin
					update	dbo.fin_interface_approval_request
