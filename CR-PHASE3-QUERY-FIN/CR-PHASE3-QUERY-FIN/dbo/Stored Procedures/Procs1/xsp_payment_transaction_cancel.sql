...
begin
	...
	begin try
		...
		if exists (select 1 from dbo.payment_transaction where code = @p_code and payment_status not in ('HOLD', 'ON PROCESS'))
		begin
		...
		begin
		    ...
		    set    tax_amount = 0 
		    where  payment_request_code in (select payment_request_code 
