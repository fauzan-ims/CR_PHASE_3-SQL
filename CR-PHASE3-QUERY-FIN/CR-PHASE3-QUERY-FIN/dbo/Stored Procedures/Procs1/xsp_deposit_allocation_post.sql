CREATE PROCEDURE [dbo].[xsp_deposit_allocation_post]
(
	@p_code					nvarchar(50)
...
begin
	...
	begin try
		...
		if exists (select 1 from dbo.deposit_allocation where code = @p_code and deposit_amount < allocation_orig_amount)
		begin
			set @msg = dbo.xfn_get_msg_err_must_be_lower_or_equal_than('Orig Amount','Outstanding Deposit Amount');
			raiserror(@msg ,16,-1)
		end
		
		if not exists (select 1 from dbo.deposit_allocation_detail where deposit_allocation_code = @p_code)
		begin
		...
		where		dad.deposit_allocation_code = @p_code
					and isnull(dad.installment_no,0) <> 0
		order by	dad.transaction_code asc, dad.installment_no asc
		...
		begin
				...
				if ((select count(1) from dbo.deposit_allocation_detail where deposit_allocation_code = @p_code and transaction_code = @transaction_code) > 1)
				begin
				    if exists (select 1 from dbo.deposit_allocation_detail where deposit_allocation_code = @p_code and transaction_code = @transaction_code and is_paid = '1')
					begin
						if (@first = 1)
						begin
						 --   if (@is_paid <> '1')
							...
							--	set @msg = 'Please allocate installment sequentially sss';
							--	raiserror(@msg ,16,-1)
							--end
							--else
							begin
								if exists (select 1 from dbo.deposit_allocation_detail where id = @detail_id and innitial_amount <> orig_amount)
								begin
								    set @parsial = 1;
								end
								else
		...
		if exists (select 1 from dbo.deposit_allocation where code = @p_code and allocation_status <> 'ON PROCESS')
		begin
		...
		begin
			...
			begin
				...
				begin
					...
					where	code = @p_code
					set @reff_source_name = 'Deposit Allocation, Deposit No : '+ @deposit_code + ' ' + @allocationt_remarks
					...
					exec dbo.xsp_fin_interface_journal_gl_link_transaction_insert @p_id							= 0
																				  ...
																				  ,@p_transaction_value_date	= @allocation_trx_date
																				  ,@p_transaction_code			= @p_code
																				  ,@p_transaction_name			= N'Deposit Allocation'
				...
				begin
					...
					set		request_status			= 'PAID'
							...
							,voucher_no				= @gl_link_transaction_code
							--
							--,process_gl_link_code	= @deposit_gl_link_code
			...
			exec dbo.xsp_fin_interface_agreement_deposit_history_insert @p_id						= 0                    
						                                                ...
						                                                ,@p_source_reff_name		= 'Deposit Allocation'
						                                                ,@p_cre_date				= @p_cre_date		
																		,@p_cre_by					= @p_cre_by			
																		,@p_cre_ip_address			= @p_cre_ip_address
																		,@p_mod_date				= @p_mod_date		
