CREATE PROCEDURE dbo.xsp_job_interface_pull_ifinfin_ifinsvy_payment_request
as
	...
	where	sp_name = 'xsp_job_interface_pull_ifinfin_ifinsvy_payment_request' -- sesuai dengan nama sp ini
...
begin
	...
	declare curr_cashier_payment_request cursor for
		...
		select 		id
		from		ifinsvy.dbo.svy_interface_payment_request
	...
	begin
		begin try
			...
			select	prd.payment_request_code
					...
					,@mod_by
					,@mod_ip_address 
			from	ifinsvy.dbo.svy_interface_payment_request_detail prd 
					inner join ifinsvy.dbo.svy_interface_payment_request pr on (pr.code = prd.payment_request_code)
