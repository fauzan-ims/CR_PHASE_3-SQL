CREATE PROCEDURE [dbo].[xsp_received_voucher_paid]
(
	@p_code				nvarchar(50)
...
begin
	...
	begin try
		...
		if	(	
				(select received_base_amount from dbo.received_voucher where code = @p_code) <>
				(select sum(base_amount) from dbo.received_voucher_detail where received_voucher_code = @p_code) 
			)
		begin
		...
		if exists (select 1 from dbo.received_voucher where code = @p_code and received_orig_amount <= 0)
		begin
			set @msg = dbo.xfn_get_msg_err_must_be_greater_than('Orig Amount','0');
			raiserror(@msg ,16,-1)
		end

		if exists (select 1 from dbo.received_voucher where code = @p_code and received_status <> 'HOLD')
		begin
		...
		begin
			...
			exec dbo.xsp_bank_mutation_history_insert @p_id						= 0
													  ...
													  ,@p_source_reff_name		= N'Received Voucher' -- nvarchar(250)
			...
			set @reff_source_name = 'Received Voucher - ' + @received_remarks
			exec dbo.xsp_fin_interface_journal_gl_link_transaction_insert @p_id							= 0
																		  ...
																		  ,@p_transaction_name			= N'Received Voucher'
...
,																		   @p_reversal_reff_no			= null
			...
			exec dbo.xsp_fin_interface_journal_gl_link_transaction_detail_insert @p_id							= 0
																				 ...
																				 ,@p_agreement_no				= @received_remarks--null
																				 ,@p_orig_currency_code			= @received_orig_currency_code
			...
			declare cur_received_voucher_detail cursor fast_forward read_only for
			...
			where	received_voucher_code = @p_code

			open cur_received_voucher_detail
		
			fetch next from cur_received_voucher_detail 
			into	@branch_code
					,@branch_name
			...
			begin
			-- journal
				if (@received_base_amount > 0)
				begin
					set @db_base_amount =  0;
					set @cr_base_amount =  @received_base_amount;
				end
				else 
				begin
					set @db_base_amount =  abs(@received_base_amount);
					set @cr_base_amount =  0;
				end
				
				if (@received_orig_amount > 0)
				begin
					set @db_orig_amount =  0;
					set @cr_orig_amount =  @received_orig_amount;
				end
				else 
				begin
					set @db_orig_amount =  abs(@received_orig_amount);
			...
			close cur_received_voucher_detail
			deallocate cur_received_voucher_detail
