/*
exec xsp_job_interface_pull_ifinfin_ifinopl_agreement_update
*/

-- Louis Kamis, 27 April 2023 15.00.57 --
CREATE PROCEDURE dbo.xsp_job_interface_pull_ifinfin_ifinopl_agreement_update
as
	...
	where	sp_name = 'xsp_job_interface_pull_ifinfin_ifinopl_agreement_update' 

	if (@is_active = '1')
	begin
		--module core interface agreement agreement main
		declare cur_finoplagreementmain cursor for

			select 	id
			from  ifinopl.dbo.opl_interface_agreement_update_out
			where id > @last_id_from_job
			order by id asc offset 0 rows
			fetch next @row_to_process rows only;

		open cur_finoplagreementmain		
		fetch next from cur_finoplagreementmain 
		into @id_interface
		...
		begin
			...
			begin catch
					...
					close cur_finoplagreementmain
					deallocate cur_finoplagreementmain
		...
		begin -- close cursor
			if cursor_status('global', 'cur_finoplagreementmain') >= -1
			begin
				if cursor_status('global', 'cur_finoplagreementmain') > -1
				begin
					close cur_finoplagreementmain ;
