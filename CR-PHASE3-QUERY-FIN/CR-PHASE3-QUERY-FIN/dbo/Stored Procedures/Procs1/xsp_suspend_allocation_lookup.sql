CREATE PROCEDURE dbo.xsp_suspend_allocation_lookup
(
...
begin
	...
	where	sa.branch_code		=  case @p_branch_code
		...
		select		code 'code'
					,sa.allocation_base_amount 'amount'
		...
		order by	case
					when @p_sort_by = 'asc' then case @p_order_by
														when 1 then code
														when 2 then cast(sa.allocation_trx_date as sql_variant)
														when 3 then am.agreement_external_no
														when 4 then sa.allocation_base_amount
												 end
					end asc
					,case
