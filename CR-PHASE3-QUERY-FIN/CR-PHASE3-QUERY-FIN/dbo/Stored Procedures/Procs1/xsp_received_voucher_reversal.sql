CREATE PROCEDURE dbo.xsp_received_voucher_reversal
(
	@p_code				nvarchar(50)
...
begin
	...
	begin try
		...
		if exists (select 1 from dbo.received_voucher where code = @p_code and received_status <> 'ON REVERSE')
		begin
		...
		begin
			...
			exec dbo.xsp_bank_mutation_insert @p_code				= @bank_mutation_code output 
											  ...
											  ,@p_balance_amount	= @received_base_amount
			...
			exec dbo.xsp_bank_mutation_history_insert @p_id						= 0
													  ...
													  ,@p_source_reff_name		= N'Reversal Received Voucher' -- nvarchar(250)
			...
			set @reff_source_name = 'Reversal Received Voucher ' + @received_remarks
			...
			exec dbo.xsp_fin_interface_journal_gl_link_transaction_detail_insert @p_id							= 0
																				 ...
																				 ,@p_orig_currency_code			= @received_orig_currency_code
																				 ,@p_orig_amount_db				= 0
																				 ,@p_orig_amount_cr				= @received_base_amount
																				 ,@p_exch_rate					= 1
																				 ,@p_base_amount_db				= 0
																				 ,@p_base_amount_cr				= @received_base_amount
																				 ,@p_remarks					= @received_remarks
			...
			-- journal
				...
				begin
					set @db_amount =  @received_base_amount;
					set @cr_amount =  0;
				end
				else 
				begin
					set @db_amount =  0;
					set @cr_amount =  abs(@received_base_amount);
				end
