CREATE PROCEDURE dbo.xsp_deposit_allocation_generate_allocation
(
	@p_code				nvarchar(50)
...
begin
	...
	begin try
		...
		begin
			...
			select	@allocation_currency_code	= isnull(am.currency_code,da.allocation_currency_code)
			...
			begin
				...
				where	mcp.is_default = '1'
						and mt.code not in (
												...
												where	sgp.code in ('TRXDPINSI','TRXDPINST','TRXDPOTH')
											)
				order	by order_no
			end
			else
			...
			--	-- exec  @last_id_job,  @last_id output, @row_count output   -- for example
	
			--	exec dbo.xsp_deposit_allocation_detail_insert @p_id						= 0
			--												   ,@p_deposit_allocation_code = @p_code
