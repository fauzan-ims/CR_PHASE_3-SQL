CREATE PROCEDURE dbo.xsp_job_interface_pull_ifinfin_ifinrep_received_request
as
...
where	sp_name = 'xsp_job_interface_pull_ifinfin_ifinrep_received_request' ; -- sesuai dengan nama sp ini
...
begin
	...
	select		id
	from		ifinrep.dbo.rep_interface_received_request
	where		id					> @last_id_from_job
				and received_status = 'HOLD'
	...
	begin
		begin try
			...
			select	rrd.received_request_code
					...
					,@mod_by
					,@mod_ip_address
			from	ifinrep.dbo.rep_interface_received_request_detail rrd
					inner join ifinrep.dbo.rep_interface_received_request rr on (rr.code = rrd.received_request_code)
