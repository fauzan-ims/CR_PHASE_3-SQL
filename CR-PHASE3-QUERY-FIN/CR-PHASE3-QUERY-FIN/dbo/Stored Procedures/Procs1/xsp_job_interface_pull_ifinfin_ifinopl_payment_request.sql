create PROCEDURE dbo.xsp_job_interface_pull_ifinfin_ifinopl_payment_request
as
	...
	where	sp_name = 'xsp_job_interface_pull_ifinfin_ifinopl_payment_request' -- sesuai dengan nama sp ini
		
	if (@is_active = '1')
	begin
		--get payment request module ifinfun
		declare curr_ifinfinifinoplpaymentrequest cursor for

			select 		id
						,code
			from		ifinopl.dbo.opl_interface_payment_request
			where		id > @last_id_from_job
			order by	id asc offset 0 rows fetch next @row_to_process rows only ;

		open curr_ifinfinifinoplpaymentrequest
		fetch next from curr_ifinfinifinoplpaymentrequest 
		into @id_interface
			 ,@code_interface
		...
		begin
			begin try
				...
				select payment_request_code
					  ...
					  ,@mod_by
					  ,@mod_ip_address 
				from ifinopl.dbo.opl_interface_payment_request_detail
			...
			begin catch
				...
				close curr_ifinfinifinoplpaymentrequest
				deallocate curr_ifinfinifinoplpaymentrequest
		...
		begin -- close cursor
			if cursor_status('global', 'curr_ifinfinifinoplpaymentrequest') >= -1
			begin
				if cursor_status('global', 'curr_ifinfinifinoplpaymentrequest') > -1
				begin
					close curr_ifinfinifinoplpaymentrequest ;
