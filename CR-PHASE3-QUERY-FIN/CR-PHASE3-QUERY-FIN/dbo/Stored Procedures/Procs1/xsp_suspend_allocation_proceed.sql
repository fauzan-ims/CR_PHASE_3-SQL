--created by, Rian at 18/07/2023 

CREATE PROCEDURE dbo.xsp_suspend_allocation_proceed
(
	@p_code				nvarchar(50)
	--
	,@p_mod_date		datetime
...
begin
	...
	begin try
		...
		select	@branch_code			= sa.branch_code
				,@branch_name			= sa.branch_name
				,@client_name			= am.client_name
				,@agreement_external_no	= am.agreement_external_no
				,@request_amount		= sa.allocation_base_amount
		from	dbo.suspend_allocation sa
				inner join dbo.agreement_main am on (am.agreement_no = sa.agreement_no)
		where	sa.code = @p_code ;

		if(@request_amount <> (select sum(base_amount) from dbo.suspend_allocation_detail where suspend_allocation_code = @p_code))
		begin
		    raiserror('Transaction Amount Allocation Not Balance',16,1)
			return
		end
		...
		begin
			...
			set @interface_remarks = 'Approval Suspend Allocation: ' + @p_code + ' - '  + @agreement_external_no + ' - ' + @client_name ;
			...
			exec dbo.xsp_fin_interface_approval_request_insert @p_code						= @request_code output
															   ...
															   ,@p_reff_no					= @p_code
															   ,@p_reff_name				= N'SUSPEND ALLOCATION'
				...
				where	approval_code = 'SUSPEND ALLOCATION'
				...
				begin 
					...
					exec dbo.xsp_get_table_value_by_dimension @p_dim_code	 = @dimension_code
															  ,@p_reff_code	 = @p_code
															  ,@p_reff_table = 'suspend_allocation'
