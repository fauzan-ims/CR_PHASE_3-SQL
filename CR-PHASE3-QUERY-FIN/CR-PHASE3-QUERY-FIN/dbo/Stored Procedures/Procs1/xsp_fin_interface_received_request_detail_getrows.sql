CREATE PROCEDURE dbo.xsp_fin_interface_received_request_detail_getrows
(
...
begin
	...
	from	dbo.fin_interface_received_request_detail prd
			left join dbo.agreement_main am on (am.agreement_no = prd.agreement_no)
	where	prd.received_request_code = @p_received_request_code
			and	(
	...
	select		id
				...
				,remarks
				,@rows_count 'rowcount'
	from		fin_interface_received_request_detail prd
				left join dbo.agreement_main am on (am.agreement_no = prd.agreement_no)
	where		prd.received_request_code = @p_received_request_code
				and	(
						prd.branch_name					like '%' + @p_keywords + '%'
