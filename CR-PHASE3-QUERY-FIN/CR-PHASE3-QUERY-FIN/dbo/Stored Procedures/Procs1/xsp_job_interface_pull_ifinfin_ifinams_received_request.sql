...
CREATE PROCEDURE dbo.xsp_job_interface_pull_ifinfin_ifinams_received_request
as
	...
	where	sp_name = 'xsp_job_interface_pull_ifinfin_ifinams_received_request' -- sesuai dengan nama sp ini

	if (@is_active = '1')
	begin
		--get cashier received request
		declare curr_ifinfinifinamsreceivedrequest cursor for

			select 		id
						,code
			from		ifinams.dbo.efam_interface_received_request
			where		id > @last_id_from_job
			order by	id asc offset 0 rows fetch next @row_to_process rows only ;

		open curr_ifinfinifinamsreceivedrequest
		fetch next from curr_ifinfinifinamsreceivedrequest 
		into @id_interface
			 ,@code_interface
		...
		begin
			begin try
				...
				select received_request_code
					  ...
					  ,@mod_by
					  ,@mod_ip_address 
				from ifinams.dbo.efam_interface_received_request_detail
			...
			begin catch
				...
				close curr_ifinfinifinamsreceivedrequest
				deallocate curr_ifinfinifinamsreceivedrequest
		...
		begin -- close cursor
			if cursor_status('global', 'curr_ifinfinifinamsreceivedrequest') >= -1
			begin
				if cursor_status('global', 'curr_ifinfinifinamsreceivedrequest') > -1
				begin
					close curr_ifinfinifinamsreceivedrequest ;
