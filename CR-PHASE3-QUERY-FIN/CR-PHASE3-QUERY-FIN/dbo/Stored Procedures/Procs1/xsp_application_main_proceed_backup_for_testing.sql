CREATE PROCEDURE [dbo].[xsp_application_main_proceed_backup_for_testing]
(
...
begin
	...
	begin try 
		...
		begin 
			...
			begin
				SELECT 'masuk'select	@asset_no = aa.asset_no
				from	dbo.application_asset aa
						left join dbo.application_amortization am on (
																		 am.asset_no			 = aa.asset_no
																		 and   am.installment_no = 1
		...
		begin 
			...
			begin
				...
				begin
					...
					set @remarks = 'PROCEED from ' + isnull(@workflow_name, '') + ' to ' + isnull(@to_workflow_name,'GO LIVE');
				end
