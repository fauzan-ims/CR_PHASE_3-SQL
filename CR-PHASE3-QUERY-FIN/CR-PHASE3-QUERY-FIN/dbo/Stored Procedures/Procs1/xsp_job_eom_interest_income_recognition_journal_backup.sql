...
CREATE PROCEDURE [dbo].[xsp_job_eom_interest_income_recognition_journal_backup]
as
begin
	...
	begin try
		...
		begin   
			...
			begin  
				--declare curraccruetype cursor fast_forward read_only for
				--select	accrue_type
				--from	dbo.agreement_asset_interest_income
				--where	isnull(reff_no, '') = ''
				--		and branch_code			 = @branch_code and accrue_type = @header_accrue_type

				--open curraccruetype ;

				--fetch next from curraccruetype
				--into @accrue_type ;
				...
				--begin
					...
					if (@accrue_type = 'PR')
					begin
						set @reverse_value_date = dateadd(day, 1, @eod_date)
						...
						set @transaction_name = 'REVERSE ' + upper(@income_name) + ' BRANCH ' + @branch_name + ' PERIODE ' + @periode ;
						set @transaction_code = 'RAIR.' + @accrue_type + '.' +@branch_code + '.' + @periode

						exec dbo.xsp_opl_interface_journal_gl_link_transaction_insert	@p_code						= @reverse_gllink_trx_code output
																						,@p_branch_code				= @branch_code
																						,@p_branch_name				= @branch_name
																						,@p_transaction_status		= 'HOLD'
																						,@p_transaction_date		= @reverse_value_date
																						,@p_transaction_value_date	= @reverse_value_date
																						,@p_transaction_code		= @transaction_code--'EOM'
																						,@p_transaction_name		= 'REVERSE ACCRUE INCOME RECOGNITION'
																						,@p_reff_module_code		= 'IFINOPL'
																						,@p_reff_source_no			= @transaction_code--'EOM'
																						,@p_reff_source_name		= @transaction_name
																						--
																						...
																						,@p_mod_by					= @mod_by
																						,@p_mod_ip_address			= @mod_ip_address
						update	dbo.opl_interface_journal_gl_link_transaction
						set		is_journal_reversal = '1'
								,reversal_reff_no	= @gllink_trx_code
								--
								,mod_date			= @mod_date
								,mod_by				= @mod_by
								,mod_ip_address		= @mod_ip_address
						where	code				= @reverse_gllink_trx_code ;
					end
					else
					begin
						exec dbo.xsp_opl_interface_journal_gl_link_transaction_insert	@p_code						= @gllink_trx_code output
					...
					begin
						...
						begin
							...
							if (@accrue_type = 'PR')
							begin 
								set @transaction_name = 'REVERSE ' + upper(@income_name) + ' For Invoice No : ' + @invoice_external_no + ', Asset No : ' + @asset_no + ', and Billing No : ' + cast(@installment_no as nvarchar(10)) ;

								exec dbo.xsp_opl_interface_journal_gl_link_transaction_detail_insert @p_gl_link_transaction_code	 = @reverse_gllink_trx_code
																										,@p_branch_code				 = @agreement_branch_code
																										,@p_branch_name				 = @agreement_branch_name
																										,@p_gl_link_code			 = @gl_link_code
																										,@p_agreement_no			 = @agreement_no
					...
					where	branch_code					= @branch_code
							and isnull(reff_no, '')		= ''
							and accrue_type				= @accrue_type
							and month(transaction_date) = month(@eod_date);
				...
				--close curraccruetype ;
				--deallocate curraccruetype ;
