-- Louis Kamis, 29 Desember 2022 16.44.36 --
/*
exec xsp_job_interface_pull_ifinfin_ifinopl_agreement_main
*/
CREATE PROCEDURE dbo.xsp_job_interface_pull_ifinfin_ifinopl_agreement_main
as
	...
	where	sp_name = 'xsp_job_interface_pull_ifinfin_ifinopl_agreement_main' 

	if (@is_active = '1')
	begin
		--module core interface agreement agreement main out
		declare cur_ifinoplagreementmain cursor for
		select 	id
		from ifinopl.dbo.opl_interface_agreement_main_out
		where id > @last_id_from_job
		order by id asc offset 0 rows
		fetch next @row_to_process rows only;

		open cur_ifinoplagreementmain		
		fetch next from cur_ifinoplagreementmain 
		into @id_interface
		...
		begin
			begin try
				begin transaction
					...
					select	amo.agreement_no
							...
							,'0' --is_remedial
							,'0' --is_wo
							,null --tco.installment_amount
							,null --tco.first_due_date
							,amo.overdue_days
							,amo.factoring_type
							,amo.reff_1
							,amo.reff_2
							...
							,@mod_by
							,@mode_address
					from	ifinopl.dbo.opl_interface_agreement_main_out amo 
			...
			begin catch
					...
					close cur_ifinoplagreementmain
					deallocate cur_ifinoplagreementmain
		...
		begin -- close cursor
			if cursor_status('global', 'cur_ifinoplagreementmain') >= -1
			begin
				if cursor_status('global', 'cur_ifinoplagreementmain') > -1
				begin
					close cur_ifinoplagreementmain ;
