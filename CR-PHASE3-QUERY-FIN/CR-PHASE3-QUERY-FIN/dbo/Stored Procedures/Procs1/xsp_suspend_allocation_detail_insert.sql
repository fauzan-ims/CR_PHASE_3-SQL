CREATE PROCEDURE dbo.xsp_suspend_allocation_detail_insert
(
...
begin
	...
	begin try
	if (isnull(@p_innitial_amount,0) <> 0 or @p_transaction_code in ('DPINSI','DPINST','DPOTH'))
	begin
		...
		begin
			select	@sum_amount = isnull(sum(orig_amount),0) + @p_orig_amount
			from	dbo.suspend_allocation_detail
			where	suspend_allocation_code = @p_suspend_allocation_code
					and is_paid = '1'

			if	((select suspend_amount from dbo.suspend_allocation where code = @p_suspend_allocation_code) < @sum_amount / @allocation_exch_rate)
			begin
				set @msg = dbo.xfn_get_msg_err_must_be_lower_or_equal_than('Total Allocation Amount','Outstanding Suspend Amount, for Request No : ' + @p_received_request_code);
				raiserror(@msg ,16,-1)
			end
		end

		insert into suspend_allocation_detail
		(
		...
		begin
			...
			set		request_status		= 'ON PROCESS'
					,process_reff_code	= @p_suspend_allocation_code
					,process_reff_name	= 'SUSPEND'
					,mod_date			= @p_mod_date
					,mod_by				= @p_mod_by
