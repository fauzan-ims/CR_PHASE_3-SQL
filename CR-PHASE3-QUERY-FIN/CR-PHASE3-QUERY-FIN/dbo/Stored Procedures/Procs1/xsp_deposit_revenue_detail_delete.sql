CREATE PROCEDURE dbo.xsp_deposit_revenue_detail_delete
(
	@p_id bigint
)
as
begin
	...
	begin try
		select	@deposit_revenue_code	= deposit_revenue_code
				,@deposit_code			= deposit_code
		from	dbo.deposit_revenue_detail
		where	id = @p_id ;

		delete deposit_revenue_detail
		where	id = @p_id ;

		select	@sum_amount		= sum(revenue_amount)
		from	dbo.deposit_revenue_detail
		where	deposit_revenue_code = @deposit_revenue_code

		update	dbo.deposit_revenue
		set		revenue_amount	= isnull(@sum_amount,0)
		where	code			= @deposit_revenue_code
