...
CREATE PROCEDURE dbo.xsp_job_interface_in_ifinfin_agreement_update 
as
	...
	declare @msg						nvarchar(max)
			...
			,@payment_with_code			nvarchar(50)
			,@payment_with_name			nvarchar(250)
	...
	where	sp_name = 'xsp_job_interface_in_ifinfin_agreement_update' 
	select	   row_to_process
		    , last_id
		    , code
			, is_active
	from	dbo.sys_job_tasklist
	...
	if (@is_active = '1')
	begin
		--module core interface collateral out
		declare cur_insagreementupdate cursor for
		...
		open cur_insagreementupdate		
		fetch next from cur_insagreementupdate 
		into @id_interface
			 ,@agreement_no
			 ,@agreement_status		   
			 ,@agreement_sub_status		   
			...
			begin try
				begin transaction
					...
					set agreement_status    		= @agreement_status		
						...
						,last_paid_installment_no	= @last_paid_period
						,installment_due_date		= @last_installment_due_date
		...
		begin -- close cursor
			if cursor_status('global', 'cur_insagreementupdate') >= -1
			begin
				if cursor_status('global', 'cur_insagreementupdate') > -1
				begin
					close cur_insagreementupdate ;
				end ;

				deallocate cur_insagreementupdate ;
			end ;
		end ;
