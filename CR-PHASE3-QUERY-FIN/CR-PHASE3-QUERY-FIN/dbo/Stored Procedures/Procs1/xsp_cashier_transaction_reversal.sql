CREATE PROCEDURE dbo.xsp_cashier_transaction_reversal
(
	@p_code				nvarchar(50)
	--
	,@p_cre_date			datetime
...
begin
	declare @msg								  nvarchar(max)
			...
			,@cashier_orig_amount_mutation		  decimal(18, 2)
			,@cashier_base_amount_mutation		  decimal(18, 2)
			,@suspend_orig_amount				  decimal(18, 2)
			,@suspend_base_amount				  decimal(18, 2)
			...
			,@acc_periode						  nvarchar(10)

	begin try
	
		--(+) Raffy 2025/06/16 jika approve dilakukan beda bulan, maka reversal date diambil dari system date 	
		select	@acc_periode = value
		from	ifinopl.dbo.sys_global_param
		where	code = 'ACCPERIODE'

		select	@reversal_date = case when convert(varchar(6), reversal_date, 112) <> @acc_periode then dbo.xfn_get_system_date() --Raffy (2025/06/09) penambahan agar jika approval sudah melewati bulan dari pengajuan, maka posting date mengikuti tanggal di approve nya 
										else reversal_date
									end
		...
		begin
			...
			set  @cashier_orig_amount_mutation = @cashier_orig_amount * -1
			set  @cashier_base_amount_mutation = @cashier_base_amount * -1
			...
			exec dbo.xsp_bank_mutation_insert @p_code					= @bank_mutation_code output 
											  ...
											  ,@p_balance_amount		= @cashier_orig_amount_mutation
			...
			exec dbo.xsp_bank_mutation_history_insert @p_id						= 0
													  ...
													  ,@p_source_reff_name		= N'Reversal Cashier Transaction' -- nvarchar(250)
													  ,@p_orig_amount			= @cashier_orig_amount_mutation
													  ,@p_orig_currency_code	= @cashier_currency_code
													  ,@p_exch_rate				= @cashier_exch_rate
													  ,@p_base_amount			= @cashier_base_amount_mutation
			...
			begin
				...
				if (@index = 1)
				begin
					set @reff_source_name = 'Reversal Cashier Transaction ' + @cashier_remarks
					exec dbo.xsp_fin_interface_journal_gl_link_transaction_insert @p_id							= 0
																				  ...
																				  ,@p_transaction_date			= @reversal_date--@cashier_trx_date
																				  ,@p_transaction_value_date	= @cashier_value_date
																				  ,@p_transaction_code			= @p_code
																				  ,@p_transaction_name			= N'Reversal Cashier Transaction'
					...
					begin
						...
						exec dbo.xsp_fin_interface_journal_gl_link_transaction_detail_insert @p_id							= 0
																						     ...
																						     ,@p_orig_currency_code			= @cashier_currency_code
																						     ,@p_orig_amount_db				= 0
																						     ,@p_orig_amount_cr				= @deposit_used_amount
																						     ,@p_exch_rate					= @cashier_exch_rate
																						     ,@p_base_amount_db				= 0
																						     ,@p_base_amount_cr				= @deposit_base_amount
																						     ,@p_remarks					= @cashier_remarks
																						     ,@p_division_code				= null
																						     ,@p_division_name				= null
																						     ,@p_department_code			= null
						...
						set @deposit_base_amount = @deposit_base_amount 
						set @deposit_used_amount = @deposit_used_amount 
						exec dbo.xsp_fin_interface_agreement_deposit_history_insert @p_id						= 0                    
						                                                            ...
						                                                            ,@p_source_reff_code		= @p_code
						                                                            ,@p_source_reff_name		= 'REVERSAL CASHIER'
						                                                            ,@p_cre_date				= @p_cre_date		
																					,@p_cre_by					= @p_cre_by			
																					,@p_cre_ip_address			= @p_cre_ip_address
																					,@p_mod_date				= @p_mod_date		
				...
				-- suspend
					...
					begin
						...
						if exists(	select	1
									...
									and		remaining_amount < @orig_amount)
						begin
						    set @msg = 'Remaining Suspend Amount cannot cover reversal cashier amount';
    						raiserror(@msg, 16, -1) ;
						end
						else
						begin
							
							select	@suspend_main_code = code
							from	dbo.suspend_main
							where	reff_no = @p_code
							
							update dbo.suspend_main
							set		remaining_amount = remaining_amount - @orig_amount
									,used_amount	= used_amount + @orig_amount
									,mod_date		= @p_mod_date
									,mod_by			= @p_mod_by
									,mod_ip_address	= @p_mod_ip_address
							where	code = @suspend_main_code
						
							set @suspend_orig_amount = @orig_amount * -1
							set @suspend_base_amount = (@base_amount * -1) * @exch_rate

							exec dbo.xsp_suspend_history_insert @p_id					= 0
																,@p_branch_code			= @branch_code
																,@p_branch_name			= @branch_name
																,@p_suspend_code		= @suspend_main_code
																,@p_transaction_date	= @reversal_date--@p_cre_date
																,@p_orig_amount			= @suspend_orig_amount
																,@p_orig_currency_code	= @currency_code
																,@p_exch_rate			= @exch_rate
																,@p_base_amount			= @suspend_base_amount
																,@p_agreement_no		= @agreement_no
																,@p_source_reff_code	= @p_code
																,@p_source_reff_name	= N'REVERSAL CASHIER'
																,@p_cre_date			= @p_cre_date		
																,@p_cre_by				= @p_cre_by			
																,@p_cre_ip_address		= @p_cre_ip_address
																,@p_mod_date			= @p_mod_date		
					...
					begin
						...
						if exists(	select	1
									...
									where	agreement_no = @agreement_no
									and		deposit_amount < @orig_amount)
						begin
						    set @msg = 'Remaining Deposit Amount cannot cover reversal cashier amount';
    						raiserror(@msg, 16, -1) ;
						end
						else
						begin
					...
					begin
						exec dbo.xsp_agreement_amortization_payment_sync @p_id					= @cashier_transaction_id
																		 ,@p_Type				= N'REVERSAL CASHIER' -- nvarchar(10)
																		 ,@p_cre_date			= @p_cre_date		
																		 ,@p_cre_by				= @p_cre_by			
																		 ,@p_cre_ip_address		= @p_cre_ip_address
																		 ,@p_mod_date			= @p_mod_date		
					...
					begin 
						set @remarks = 'Reversal ' + @remarks + @cashier_remarks
						set @charges_amount = @orig_amount
						exec dbo.xsp_fin_interface_agreement_fund_in_used_history_insert @p_agreement_no				= @agreement_no
																						 ,@p_charges_date				= @reversal_date--@cashier_trx_date
					...
					begin
						set @reten_orig_amount = @orig_amount
						set @reten_base_amount = @base_amount
			...
			where	cashier_transaction_code = @p_code
				...
				begin
					--insert invoice
					set @remarks_invoice = 'Reversal Cashier Invoice Allocation ' + @cashier_remarks
					exec dbo.xsp_fin_interface_agreement_invoice_ledger_history_insert @p_id				     = 0
																					   ...
																					   ,@p_transaction_date	     = @reversal_date--@cashier_trx_date
																					   ,@p_transaction_no	     = @p_code
																					   ,@p_transaction_name	     = 'REVERSAL CASHIER TRANSACTION'
