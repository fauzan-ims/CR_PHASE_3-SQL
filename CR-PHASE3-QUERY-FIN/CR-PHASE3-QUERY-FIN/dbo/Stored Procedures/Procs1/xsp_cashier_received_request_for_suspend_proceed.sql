CREATE PROCEDURE dbo.xsp_cashier_received_request_for_suspend_proceed
(
	@p_code					nvarchar(50)
	,@p_rate				decimal(18, 6)
	--
	...
	begin try 
		...
		begin
			...
			from	dbo.sys_global_param sgp
					inner join ifinsys.dbo.sys_branch sb on (sb.code = sgp.value)
			where	sgp.code = 'HO' ;

			if (isnull(@branch_code, '') = '' or isnull(@branch_name,'') = '')
			begin
				set @msg = 'Branch HO does not exists';
				raiserror(@msg ,16,-1)
			end
			...
			begin
					...
					exec dbo.xsp_suspend_allocation_insert @p_code						= @suspend_allocation_code output 
														   ...
														   ,@p_suspend_code				= null
														   ,@p_suspend_amount			= 0 -- decimal(18, 2)
														   ,@p_agreement_no				= @agreement_no
														   ,@p_suspend_gl_link_code		= N'' -- nvarchar(50)
			...
			begin
			    select	@suspend_allocation_code= code 
				from	dbo.suspend_allocation 
				where	allocation_status				= 'HOLD' 
						and allocation_currency_code	= @request_currency_code
						and branch_code					= @branch_code
			...
			exec dbo.xsp_suspend_allocation_detail_insert @p_id							= 0
														  ,@p_suspend_allocation_code	= @suspend_allocation_code
