CREATE PROCEDURE dbo.xsp_payment_transaction_update
(
	@p_code						   nvarchar(50)
	...
	,@p_branch_bank_account_no	   nvarchar(50)
...
begin
	...
	begin try
		...
		--- value date berubah
		--if exists(select 1 from dbo.payment_transaction where code = @p_code and payment_value_date <> @p_payment_value_date)
		--begin

		--	--- detail
		--	declare cur_payment_transaction_detail cursor fast_forward read_only for
			
		--	select	ptd.orig_curr_code
		--			,ptd.orig_amount
		--			,ptd.id
		...
		--		where	 mcr.currency_code = @currency_code
		--				 and mcr.eff_date <= cast(@p_payment_value_date as date)
		--		order by mcr.eff_date desc

		--		update	dbo.payment_transaction_detail
		--		set		exch_rate		= @exch_rate
		--				,base_amount	= @orig_amount * @exch_rate
		--		where	id				= @id
		...
		--	--- header
		--	select	 top 1
		--			 @p_payment_exch_rate =  exch_rate
		--	from	 master_currency_rate mcr
		--	where	 mcr.currency_code = @p_payment_orig_currency_code
		--			 and mcr.eff_date <= cast(@p_payment_value_date as date)
		--	order by mcr.eff_date desc

		--	select	@p_payment_base_amount	= isnull(sum(base_amount),0) 
		--	from	dbo.payment_transaction_detail
		--	where	payment_transaction_code	= @p_code

		--	set @p_payment_orig_amount = @p_payment_base_amount / @p_payment_exch_rate
		--end
		...
		set		branch_code						= @p_branch_code
				...
				,branch_bank_account_no			= @p_branch_bank_account_no
