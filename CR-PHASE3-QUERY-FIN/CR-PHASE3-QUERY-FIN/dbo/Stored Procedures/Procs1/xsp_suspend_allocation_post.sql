CREATE PROCEDURE dbo.xsp_suspend_allocation_post
(
	@p_code					nvarchar(50)
...
begin
	...
	begin try
		-- Louis Senin, 22 April 2024 17.14.46 -- mengupdate tanggal transaksi agar sesuai dengan tanggal system date
		update	dbo.suspend_allocation
		set		allocation_trx_date = dbo.xfn_get_system_date()
				,mod_date = @p_mod_date
				,mod_by = @p_mod_by
				,mod_ip_address = @p_mod_ip_address
		where	code = @p_code ;

		if exists (select 1 from dbo.suspend_allocation where code = @p_code and suspend_amount < allocation_orig_amount)
		begin
			set @msg = dbo.xfn_get_msg_err_must_be_lower_or_equal_than('Orig Amount','Outstanding Suspend Amount');
			raiserror(@msg ,16,-1)
		end
		
		if not exists (select 1 from dbo.suspend_allocation_detail where suspend_allocation_code = @p_code)
		begin
		...
		if (	
					(select isnull(allocation_base_amount,0) from dbo.suspend_allocation where code = @p_code) <> 
					(select isnull(sum(base_amount),0) from dbo.suspend_allocation_detail where suspend_allocation_code = @p_code)
				)
		begin
		...
		where		sad.suspend_allocation_code = @p_code
					and isnull(sad.installment_no,0) <> 0
		order by	sad.transaction_code asc, sad.installment_no asc
		...
		begin
				...
				if ((select count(1) from dbo.suspend_allocation_detail where suspend_allocation_code = @p_code and transaction_code = @transaction_code) > 1)
				begin
				    if exists (select 1 from dbo.suspend_allocation_detail where suspend_allocation_code = @p_code and transaction_code = @transaction_code and is_paid = '1')
					begin
						if (@first = 1)
						begin
						 --   if (@is_paid <> '1')
							...
							--else 
							begin
								if exists (select 1 from dbo.suspend_allocation_detail where id = @detail_id and innitial_amount <> orig_amount)
								begin
								    set @parsial = 1;
								end
								else
		...
		(
			...
			where	code				  = @p_code
					and allocation_status <> 'ON PROCESS'
		)
		begin
		...
		begin
			...
			begin
				...
				begin
					...
					set @reff_source_name = 'Suspend Allocation, Suspend No : ' + @suspend_code + ' for ' + @agreement_external_no + ' - ' + @client_name + '. ' + @allocationt_remarks
					...
					exec dbo.xsp_fin_interface_journal_gl_link_transaction_detail_insert @p_id							= 0
																						 ...
																						 ,@p_orig_amount_db				= @allocation_orig_amount
																						 ,@p_orig_amount_cr				= 0
																						 ,@p_exch_rate					= @allocation_exch_rate
																						 ,@p_base_amount_db				= @allocation_base_amount
																						 ,@p_base_amount_cr				= 0
				...
				begin
					...
					set		request_status		= 'PAID'
							,process_date		= @allocation_value_date
							,process_reff_code	= @p_code
							,process_reff_name	= 'SUSPEND ALLOCATION'
					...
					set		request_status			= 'PAID'
							...
							,voucher_no				= @gl_link_transaction_code
							--
							--,process_gl_link_code	= @suspend_gl_link_code
							--,process_curr			= @allocation_currency_code
							--,process_exch_rate		= @allocation_exch_rate
							--,process_amount			= @allocation_orig_amount
							,mod_date				= @p_mod_date
							,mod_by					= @p_mod_by
							,mod_ip_address			= @p_mod_ip_address
					where	code					= @received_request_code
			...
			exec dbo.xsp_suspend_history_insert @p_id					= 0
												...
												,@p_agreement_no		= @agreement_no
												,@p_source_reff_code	= @p_code
												,@p_source_reff_name	= N'Suspend Allocation'
												,@p_cre_date			= @p_cre_date		
												,@p_cre_by				= @p_cre_by			
												,@p_cre_ip_address		= @p_cre_ip_address
												,@p_mod_date			= @p_mod_date		
