...
CREATE PROCEDURE [dbo].[xsp_received_transaction_reversal_request]
(
	@p_code					nvarchar(50)
...
begin
	...
	begin try
		...
		if exists (select 1 from dbo.received_transaction where code = @p_code and received_status <> 'PAID')
		begin
		...
		begin
			...
			exec dbo.xsp_reversal_main_insert @p_code					= 0
											  ...
											  ,@p_reversal_remarks		= @p_reversal_remarks
											  ,@p_source_reff_code		= @p_code
											  ,@p_source_reff_name		= N'Received Transaction'
			...
			set		received_status		= 'ON REVERSE'
					,mod_date			= @p_mod_date
					,mod_by				= @p_mod_by
