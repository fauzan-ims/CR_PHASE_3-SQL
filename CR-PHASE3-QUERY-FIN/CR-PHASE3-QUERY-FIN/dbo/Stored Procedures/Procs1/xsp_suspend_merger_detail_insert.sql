CREATE PROCEDURE dbo.xsp_suspend_merger_detail_insert
(
...
begin
	...
	begin try
		...
		insert into suspend_merger_detail
		(
			suspend_merger_code
			,suspend_code
			,suspend_amount
		...
		where	suspend_merger_code = @p_suspend_merger_code

		update	dbo.suspend_merger
		set		merger_amount	= @sum_amount
				,mod_date		= @p_mod_date
				,mod_by			= @p_mod_by
				,mod_ip_address	= @p_mod_ip_address
		where	code			= @p_suspend_merger_code

		update	dbo.suspend_main
		set		transaction_code	= @p_suspend_merger_code
				,transaction_name	= 'MERGER'
				,mod_date			= @p_mod_date
				,mod_by				= @p_mod_by
				,mod_ip_address		= @p_mod_ip_address
		where	code				= @p_suspend_code
