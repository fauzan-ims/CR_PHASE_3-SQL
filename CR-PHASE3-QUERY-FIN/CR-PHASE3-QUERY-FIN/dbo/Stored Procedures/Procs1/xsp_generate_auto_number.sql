--created by, Rian at 20/07/2023 

CREATE procedure [dbo].[xsp_generate_auto_number]
(
	@p_unique_code		  nvarchar(50)	output
	,@p_branch_code		  nvarchar(10)	= ''
	,@p_year			  nvarchar(4)	= ''
	,@p_month			  nvarchar(2)	= ''
	,@p_fin_code		  nvarchar(250)
	...
	begin
		...
		*/
		...
		begin
			if (@p_delimiter = '')
			begin 
				set @p_unique_code = @p_fin_code + @p_year + @p_month + @next_run_number;
			end
			else
			begin
				set @p_unique_code = @next_run_number + @p_delimiter + @p_fin_code + @p_delimiter + @p_month + @p_delimiter + @p_year;
			end
		end
		else if (@p_month = '')
		begin
			set @p_unique_code = @next_run_number + @p_delimiter + @p_fin_code + @p_delimiter + @p_branch_code + @p_delimiter + @p_year;
		end
		else
		begin
			set @p_unique_code = @next_run_number + @p_delimiter + @p_fin_code + @p_delimiter + @p_branch_code + @p_delimiter + @p_month + @p_delimiter + @p_year;
		end
	end ;
end ;
