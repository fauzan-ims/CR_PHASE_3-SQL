create procedure dbo.xsp_upload_validation_cannot_be_less_than_system_date
(
...
begin
    ...
    -- validasi tidak boleh kurang dari atau sama dengan Tanggal Sistem.

		--set dateformat ymd;
        if (convert(date, @p_value_check, 103) < @system_date)
        begin

            set @error_msg = @p_column_name + ' cannot be less than or equal to System Date.';
