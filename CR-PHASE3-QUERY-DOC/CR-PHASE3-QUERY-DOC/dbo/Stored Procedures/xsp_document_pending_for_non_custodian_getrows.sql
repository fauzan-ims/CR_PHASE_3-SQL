CREATE PROCEDURE dbo.xsp_document_pending_for_non_custodian_getrows
(
	...
	,@p_initial_branch_code nvarchar(50) -- cabang asal nya
...
begin
	...
	from	document_pending dp
			left join dbo.agreement_main am on (am.agreement_no		= dp.agreement_no)
			left join dbo.agreement_collateral ac on (ac.collateral_no = dp.collateral_no)
			left join dbo.agreement_asset ags on (ags.asset_no			= dp.asset_no)
	...
	where	document_status			= case @p_document_status
			...
			and dp.initial_branch_code = case @p_initial_branch_code
											 when 'ALL' then dp.initial_branch_code
											 else @p_initial_branch_code
										 end
			and	document_status			   IN ( 'NEW', 'TRANSIT' )
			...
			and (
					...
					or	dp.branch_name									like '%' + @p_keywords + '%'
					or	dp.initial_branch_name							like '%' + @p_keywords + '%'
	...
	begin
		...
		order by	case @p_order_by
						when 1 then dp.initial_branch_name  
						when 2 then dp.branch_name
