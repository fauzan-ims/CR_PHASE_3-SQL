...
CREATE PROCEDURE dbo.xsp_rpt_tanda_terima_jaminan
(
...
BEGIN
	...
	begin try
		...
		from	dbo.document_main dm
				...
				OUTER APPLY (
					select	TOP 1 asst.agreement_external_no, asst.client_name, asst.rental_status, asst.status, dde.doc_no 
					from	ifinams.dbo.asset asst 
					inner join dbo.document_main dmn on dmn.asset_no = asst.code
					inner join dbo.document_detail dde on dde.document_code = dmn.code
					inner join dbo.document_movement_detail dvd on dvd.document_code = dmn.code
					where	dvd.movement_code = dmt.code 					
				)ast
				OUTER APPLY (
					select dmre.document_no, dmre.document_name, dmre.remarks from dbo.document_movement_replacement dmre
					where dmre.movement_code =  dmt.code 
				)dmr
		where	dmt.code = @p_mutation_no
		--from	dbo.document_main dm
		--		left join dbo.document_movement_detail dmd on dmd.document_code = dm.code
