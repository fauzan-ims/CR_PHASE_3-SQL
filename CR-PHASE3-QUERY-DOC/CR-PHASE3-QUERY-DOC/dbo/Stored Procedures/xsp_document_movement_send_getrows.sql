CREATE PROCEDURE [dbo].[xsp_document_movement_send_getrows]
(
	@p_keywords			NVARCHAR(50)
	,@p_pagenumber		INT
	,@p_rowspage		INT
	,@p_order_by		INT
	,@p_sort_by			NVARCHAR(5)
	,@p_branch_code		NVARCHAR(50)
	,@p_movement_status NVARCHAR(20)
)
AS
BEGIN
	declare @rows_count int = 0 ;

	--,@to_branch_code		nvarchar(50);
	if exists
	(
		select	1
		from	sys_global_param
		where	code	  = 'HO'
				and value = @p_branch_code
	)
	begin
		set @p_branch_code = 'ALL' ;
	end ; 

	select	@rows_count = count(1)
	from	document_movement
	where	@p_branch_code		  = case @p_branch_code
										when 'all' then @p_branch_code
										else case movement_type + movement_location
												 when 'returnbranch' then branch_code
												 else branch_code
											 end
									end
			and movement_status	  = case @p_movement_status
										when 'ALL' then movement_status
										else @p_movement_status
									end
			and movement_type	  = 'SEND'
			and (
							branch_name										like '%' + @p_keywords + '%'
							or	code										like '%' + @p_keywords + '%'
							or	convert(varchar(30), movement_date, 103)	like '%' + @p_keywords + '%'
							or	movement_status								like '%' + @p_keywords + '%'
							or	movement_type								like '%' + @p_keywords + '%'
							or	case 
									when movement_location = 'BORROW CLIENT' then 'BORROW CUSTOMER'
									when movement_location = 'CLIENT' then 'REALEASED PERMANENT'
									else movement_location
								end	 like '%' + @p_keywords + '%'
							or	case
									 when movement_type = 'entry' then movement_from
									 when movement_location = 'branch' then branch_name
									 when movement_location = 'department' then movement_from_dept_name
									 when movement_location = 'third party' then branch_name
									 else movement_from -- enrty , third party
								 end  like '%' + @p_keywords + '%' 
							or	case
									 when movement_type = 'entry' then movement_to
									 when movement_location = 'branch' then MOVEMENT_TO_BRANCH_NAME
									 when movement_location = 'department' then MOVEMENT_TO_DEPT_NAME
									 when movement_location = 'client' then MOVEMENT_TO_CLIENT_NAME
									 when movement_location = 'borrow client' then MOVEMENT_TO_CLIENT_NAME
									 else movement_to -- enrty , third party
								 end like '%' + @p_keywords + '%' 
				) ;

		select		code
					,branch_code
					,branch_name
					,convert(varchar(30), movement_date, 103) 'movement_date'
					,movement_status
					,movement_type
					,case 
						when movement_location = 'BORROW CLIENT' then 'BORROW CUSTOMER'
						when movement_location = 'CLIENT' then 'REALEASED PERMANENT'
						else movement_location
					end 'movement_location'
					--,case
					--	when movement_location = 'client' then 'RELEASE CUSTOMER'
					--	when movement_location = 'borrow client' then 'BORROW CUSTOMER'
					--	when movement_location = 'third party' then 'BORROW THIRD PARTY'
					--	else movement_location
					--end 'movement_location'
					,case
						 when movement_type = 'entry' then movement_from
						 when movement_location = 'branch' then branch_name
						 when movement_location = 'department' then movement_from_dept_name
						 when movement_location = 'third party' then branch_name
						 when movement_location = 'client' then branch_name
						 else movement_from -- enrty , third party
					 end 'movement_from'
					,case
						 when movement_type = 'entry' then movement_to
						 when movement_location = 'branch' then MOVEMENT_TO_BRANCH_NAME
						 when movement_location = 'department' then MOVEMENT_TO_DEPT_NAME
						 when movement_location = 'client' then movement_to_client_name
						 when movement_location = 'borrow client' then MOVEMENT_TO_CLIENT_NAME
						 else movement_to -- enrty , third party
					 end 'movement_to'
					,movement_by_emp_code
					,movement_by_emp_name
					,movement_courier_code
					,movement_remarks
					,receive_status
					,convert(varchar(30), receive_date, 103) 'receive_date'
					,receive_remark
					,@rows_count 'rowcount'
		from		document_movement
		where		@p_branch_code		  = case @p_branch_code
												when 'all' then @p_branch_code
												else case movement_type + movement_location
														 when 'returnbranch' then branch_code
														 else branch_code
													 end
											end
					and movement_status	  = case @p_movement_status
												when 'ALL' then movement_status
												else @p_movement_status
											end
					and movement_type	  = 'SEND'
					and (
							branch_name										like '%' + @p_keywords + '%'
							or	code										like '%' + @p_keywords + '%'
							or	convert(varchar(30), movement_date, 103)	like '%' + @p_keywords + '%'
							or	movement_status								like '%' + @p_keywords + '%'
							or	movement_type								like '%' + @p_keywords + '%'
							or	case 
									when movement_location = 'BORROW CLIENT' then 'BORROW CUSTOMER'
									when movement_location = 'CLIENT' then 'REALEASED PERMANENT'
									else movement_location
								end	 like '%' + @p_keywords + '%'
							or	case
									 when movement_type = 'entry' then movement_from
									 when movement_location = 'branch' then branch_name
									 when movement_location = 'department' then movement_from_dept_name
									 when movement_location = 'third party' then branch_name
									 else movement_from -- enrty , third party
								 end  like '%' + @p_keywords + '%' 
							or	case
									 when movement_type = 'entry' then movement_to
									 when movement_location = 'branch' then MOVEMENT_TO_BRANCH_NAME
									 when movement_location = 'department' then MOVEMENT_TO_DEPT_NAME
									 when movement_location = 'client' then MOVEMENT_TO_CLIENT_NAME
									 when movement_location = 'borrow client' then MOVEMENT_TO_CLIENT_NAME
									 else movement_to -- enrty , third party
								 end like '%' + @p_keywords + '%' 
						)
		order by case  
					when @p_sort_by = 'asc' then case @p_order_by
													when 1 then code
													when 2 then branch_name 
													when 3 then (movement_type + movement_location)
													when 4 then case
																	when movement_type = 'entry' then movement_from
																	when movement_location = 'branch' then branch_name
																	when movement_location = 'department' then movement_from_dept_name
																	when movement_location = 'third party' then branch_name
																	else movement_from -- enrty , third party
																end
													when 5 then case
																	when movement_type = 'entry' then movement_to
																	when movement_location = 'branch' then MOVEMENT_TO_BRANCH_NAME
																	when movement_location = 'department' then MOVEMENT_TO_DEPT_NAME
																	when movement_location = 'client' then MOVEMENT_TO_CLIENT_NAME
																	when movement_location = 'borrow client' then MOVEMENT_TO_CLIENT_NAME
																	else movement_to -- enrty , third party
																end
													when 6 then cast(movement_date as sql_variant)
													when 7 then movement_status
												 end
				end asc 
				,case when @p_sort_by = 'desc' then case @p_order_by
														when 1 then code
														when 2 then branch_name 
														when 3 then (movement_type + movement_location)
														when 4 then case
																		when movement_type = 'entry' then movement_from
																		when movement_location = 'branch' then branch_name
																		when movement_location = 'department' then movement_from_dept_name
																		when movement_location = 'third party' then branch_name
																		else movement_from -- enrty , third party
																	end
														when 5 then case
																		when movement_type = 'entry' then movement_to
																		when movement_location = 'branch' then MOVEMENT_TO_BRANCH_NAME
																		when movement_location = 'department' then MOVEMENT_TO_DEPT_NAME
																		when movement_location = 'client' then MOVEMENT_TO_CLIENT_NAME
																		when movement_location = 'borrow client' then MOVEMENT_TO_CLIENT_NAME
																		else movement_to -- enrty , third party
																	end
														when 6 then cast(movement_date as sql_variant)
														when 7 then movement_status
													end
		end desc offset ((@p_pagenumber - 1) * @p_rowspage) rows fetch next @p_rowspage rows only ; 
end ;
