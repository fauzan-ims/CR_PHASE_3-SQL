CREATE PROCEDURE dbo.xsp_document_main_lookup_for_retrive
(
...
begin
	...
	from	dbo.document_main dmn
			...
			left join dbo.master_locker ml on ml.code = dmn.locker_code
			left join dbo.master_drawer md on md.code = dmn.drawer_code
			left join dbo.master_row mr on mr.code = dmn.row_code
	where	dmn.branch_code			= @p_branch_code
			and dmn.locker_position = 'IN LOCKER'
