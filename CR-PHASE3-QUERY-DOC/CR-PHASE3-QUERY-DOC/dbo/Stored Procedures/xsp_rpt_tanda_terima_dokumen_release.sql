...
CREATE PROCEDURE dbo.xsp_rpt_tanda_terima_dokumen_release
(
...
begin
	...
	begin try
		...
		select	@p_user_id
				...
				,dm.document_type
				,case
					 when dm.document_type = 'BPKB' then case when isnull(av.bpkb_no,'') = '' then docdetail.doc_no else av.bpkb_no	end--ISNULL(av.bpkb_no,docdetail.doc_no)
					 WHEN dm.DOCUMENT_TYPE = 'SUPPLEMENTARY' THEN docdetail.doc_no
					 else '-'
				 end 'document_no'
		...
		from	document_movement_detail dmd
				...
				OUTER APPLY 
				(	
					select	dd.doc_name, dd.doc_no
					from	dbo.document_detail dd 
					where	dm.code = dd.document_code -- (+) Raffyanda 2024/06/12 perubahan agar suplementary dapat diambil sesuai dengan asset nya 
