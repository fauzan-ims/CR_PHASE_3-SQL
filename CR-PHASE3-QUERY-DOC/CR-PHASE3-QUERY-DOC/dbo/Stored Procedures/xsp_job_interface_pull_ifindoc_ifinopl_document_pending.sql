/*
exec xsp_job_interface_pull_ifindoc_ifinopl_document_pending
*/
CREATE PROCEDURE dbo.xsp_job_interface_pull_ifindoc_ifinopl_document_pending
as
...
where	sp_name = 'xsp_job_interface_pull_ifindoc_ifinopl_document_pending' ; -- sesuai dengan nama sp ini
...
begin
	...
	select		id
				,code
				,agreement_no
	from		ifinopl.dbo.opl_interface_document_pending
	...
	begin
		begin try
			...
			select	sr.code
					...
					,@mod_by
					,@mod_ip_address
			from	ifinopl.dbo.opl_interface_document_pending sr
			...
			select	ldp.document_pending_code
					...
					,@mod_by
					,@mod_ip_address
			from	ifinopl.dbo.opl_interface_document_pending_detail ldp
			where	ldp.document_pending_code = @code ;

			insert into dbo.doc_interface_sys_document_upload
			(
				reff_no
				,reff_name
				,reff_trx_code
			...
			select	reff_no
					...
					,@mod_by
					,@mod_ip_address
			from	ifinopl.dbo.opl_interface_sys_document_upload
			where	reff_no = @agreement_no ;
