CREATE PROCEDURE dbo.xsp_work_order_post
(
	@p_code			   nvarchar(50)
	--
	,@p_mod_date	   datetime
...
begin
	...
	begin try
		...
		--if(month(@faktur_date_source) < month(dbo.xfn_get_system_date()))
		--begin
		...
		--Update status WO jadi POST --(+)Raffy 20241028 Perubahan pada posisi update status, agar jika data terproses 2 kali, maka validasi akan muncul karena status sudah berubah
		...
		begin
			...
			begin -- journal
				...
				else if (isnull(@faktur_date_source, '') = '')
				begin
					set @journal_date = dbo.xfn_get_system_date() ;
				end ;
				else
				begin
					set @journal_date = @faktur_date_source ;
				end ;

				set @source_name = N'Work Order for ' + @plat_no + N' - ' + @p_code ;
				...
				exec dbo.xsp_efam_interface_journal_gl_link_transaction_insert @p_code						= @journal_code output
																			   ...
																			   ,@p_transaction_date			= @date_journal --@journal_date
				...
				select	mt.sp_name
						...
						,isnull(mnt.nitku, mv.nitku)
						,isnull(mnt.npwp_pusat,mv.npwp_pusat)
				...
				begin
					...
					begin
						...
						begin
								if len(@faktur_no_invoice) = 16 --Raffy 2024/06/12 Penambahan string jika invoice nya 16 digit agar di SAP tidak terbaca sebagai faktur
								begin
					...
					begin
						...
						begin
							...
							set @faktur_date = @faktur_date_source ;
							set @faktur_jurnal = @faktur_no ;
							SET @vendor_nitku  = @vendor_nitku ;						 
							SET @vendor_npwp_ho = @vendor_npwp_ho ;
						end ;
					end ;
					else
					begin
					...
					begin
						...
						begin
							exec dbo.xsp_efam_interface_journal_gl_link_transaction_detail_insert @p_gl_link_transaction_code		= @journal_code
																								  ...
																								  ,@p_ext_sale_type					= ''
																								  ,@p_ext_vendor_nitku				= @vendor_nitku
																								  ,@p_ext_vendor_npwp_pusat			= @vendor_npwp_ho
																								  ,@p_cre_date						= @p_mod_date
																								  ,@p_cre_by						= @p_mod_by
																								  ,@p_cre_ip_address				= @p_mod_ip_address
																								  ,@p_mod_date						= @p_mod_date
				...
				set @payment_remark = N'Payment Work Order for : ' + @p_code + N' - ' + isnull(@plat_no, '') + N' - ' + N'Invoice : ' + isnull(@invoice_no, '') + N' - ' + isnull(@remark, '') + N' - SPK No. :' + isnull(@spk_no, '') ;
				...
				begin
					...
					begin
						...
						set @remarks = isnull(@transaction_name, '') + N', ' + convert(nvarchar(3), @quantity) + N' ' + isnull(@service_type, '') + N' ' + isnull(@service_name, '') ;
						...
						if @return_value <> 0
						begin
							if(@transaction_code = 'PWO')
							begin
								if not exists (select 1 from dbo.payment_request_detail where gl_link_code = @gl_link_code and payment_request_code = @code_payment_request)
								begin
								...
								begin
									...
									set		orig_amount = orig_amount + @orig_amount_db
									where	gl_link_code			 = @gl_link_code
											and payment_request_code = @code_payment_request ;
								end
							end
							else
							begin
					...
					begin
						set @msg = N'Payment Amount does not balance' ;
				...
				begin
					...
					begin
						...
						set @reff_remark = N'Maintenance for ' + @asset_code + N' - ' + @item_name ;
			...
			if @is_reimburse = '1'
			begin
				set @description_request = N'Invoice Maintenance ' + format(@payment_amount, '#,###.00', 'DE-de') + N' for ' + @asset_code + N' - ' + @item_name + N'.' ;
				set @invoice_name = N'Invoice Maintenance for ' + @agreement_external_no + N' - ' + @plat_no ;
				...
				exec dbo.xsp_ifinams_interface_additional_request_insert @p_id = 0
																		 ...
																		 ,@p_invoice_date = @date
																		 ,@p_invoice_name = @invoice_name	--'Invoice Maintenance'
