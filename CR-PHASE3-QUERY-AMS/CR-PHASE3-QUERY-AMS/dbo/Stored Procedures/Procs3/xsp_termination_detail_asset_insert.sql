CREATE PROCEDURE dbo.xsp_termination_detail_asset_insert
(
	@p_id					    bigint = 0 output
	,@p_termination_code	    nvarchar(50)
	,@p_policy_asset_code	    nvarchar(50)
	,@p_refund_amount		    decimal(18, 2)	= 0
	,@p_policy_code				nvarchar(50)	= ''
	--
	,@p_cre_date				datetime
	,@p_cre_by					nvarchar(15)
	,@p_cre_ip_address			nvarchar(15)
	,@p_mod_date				datetime
	,@p_mod_by					nvarchar(15)
	,@p_mod_ip_address			nvarchar(15)
)
as
begin
	declare @msg					nvarchar(max)
			,@termination_date		datetime
			,@termination_amount	decimal(18,2)

	begin try
		select @termination_date = termination_date 
		from dbo.termination_main
		where code = @p_termination_code

		set @termination_amount = dbo.xfn_get_terminate(@p_policy_code, @termination_date)

		insert into termination_detail_asset
		(
			termination_code
			,policy_asset_code
			,estimate_refund_amount
			,refund_amount
			--
			,cre_date
			,cre_by
			,cre_ip_address
			,mod_date
			,mod_by
			,mod_ip_address
		)
		values
		(
			@p_termination_code
			,@p_policy_asset_code
			,@termination_amount
			,@p_refund_amount
			--
			,@p_cre_date
			,@p_cre_by
			,@p_cre_ip_address
			,@p_mod_date
			,@p_mod_by
			,@p_mod_ip_address
		) ;

		set @p_id = @@identity ;
	end try
	begin catch
		declare @error int ;

		set @error = @@error ;

		if (@error = 2627)
		begin
			set @msg = dbo.xfn_get_msg_err_code_already_exist() ;
		end ;

		if (len(@msg) <> 0)
		begin
			set @msg = N'V' + N';' + @msg ;
		end ;
		else
		begin
			set @msg = N'E;' + dbo.xfn_get_msg_err_generic() + N';' + error_message() ;
		end ;

		raiserror(@msg, 16, -1) ;

		return ;
	end catch ;
end ;
