CREATE PROCEDURE dbo.xsp_rpt_asset_backup_20250902
(
	...
	(
	    ...
	    asset_code
			...
			SELECT	DISTINCT	@p_branch_code
								...
								,av.colour
								,ass.last_meter --use_life --milliage
			...
			from	ifinams.dbo.asset ass with (nolock)
					...
					left join ifinbam.dbo.master_item mim with (nolock) on (mim.code = ass.item_code)
					left join ifinbam.dbo.sys_general_subcode sgs4 with (nolock) on (sgs4.code = mim.registration_class_type)
					OUTER APPLY
						(
							select	TOP 1 am.AGREEMENT_NO, 
									am.marketing_name, 
					...
					OUTER APPLY
						(
							SELECT	TOP 1 aps.PLAT_COLOUR, aps.MOBILIZATION_AMOUNT
							FROM	ifinopl.dbo.application_asset aps with (nolock)
					...
					outer apply
						(
							...
							from	ifinopl.dbo.application_asset_budget asd with (nolock)
					...
					outer apply
						(
							...
							from	ifinopl.dbo.application_asset_budget asd1 with (nolock)
							where	asset_no	  = ass.asset_no
					...
					outer apply 
						...
						from	ifinopl.dbo.agreement_asset ast with (nolock)
					...
					(
						...
						from ifinopl.dbo.AGREEMENT_INFORMATION with (nolock)
						where AGREEMENT_NO = ass.AGREEMENT_NO
						--and ASSET_NO = ass.ASSET_NO
					) maxdate
					outer apply
						(
					...
					outer apply 
						...
						from dbo.insurance_policy_main ipm with (nolock)
						inner join ifinams.dbo.master_insurance mi with (nolock) ON (mi.code = ipm.insurance_code)
						inner join ifinams.dbo.insurance_policy_asset ipa with (nolock) on ipa.fa_code = ass.code
					...
					outer apply
						...
						from ifinopl.dbo.agreement_asset ags with (nolock)
						where ags.agreement_no = ass.agreement_no
			...
			--		left join ifinopl.dbo.agreement_main aman on (aman.agreement_no = ass.agreement_no)
			--		--left join ifinopl.dbo.agreement_asset asmt on (asmt.agreement_no = aman.agreement_no)
			--		--left join ifinams.dbo.insurance_policy_main ipm on (ipm.code = ipa.policy_code)
			--		--												   and (ipm.policy_status = 'ACTIVE')
			--		--left join ifinams.dbo.master_insurance mi on (mi.code = ipm.insurance_code)
			--		left join ifinopl.dbo.application_asset apss on (apss.fa_code = ass.code and aman.application_no = apss.application_no)
			...
			--		--left join ifinopl.dbo.application_asset ast on ast.asset_no = ass.rental_reff_no
			--		outer apply
			--			(
			--				select	budget_amount 'replacement_amount'
