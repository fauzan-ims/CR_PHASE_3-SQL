...
CREATE PROCEDURE dbo.xsp_rpt_delivery_collect_order
(
...
BEGIN
	...
	delete dbo.rpt_delivery_collect_order
	...
	begin try
		...
		set	@report_title = 'Report Delivery Replacement';
	...
	BEGIN
			...
			insert into rpt_delivery_collect_order
			(
				user_id
				,report_company
				,report_title
			...
			select	distinct @p_user_id
					...
					,avi.engine_no
					,avi.colour
					,eta_date --rmain.delivery_date
					,hast.handover_date
					,had.upload_bast_date	
					,@p_is_condition
			from	ifinams.dbo.handover_asset hast
					inner join ifinams.dbo.asset ast on ast.code = hast.fa_code
					inner join ifinams.dbo.asset_vehicle avi on avi.asset_code = ast.code
					--left join ifinams.dbo.register_main rmain on rmain.fa_code = ast.code
					inner join dbo.handover_request hr on (hr.handover_code = hast.code)
					outer apply (	
			...
			where	hast.type in ('REPLACE IN','REPLACE OUT')
					and hast.branch_code = case @p_branch_code
														when 'ALL' then hast.branch_code
														else @p_branch_code
													end	
					and  cast( hast.transaction_date as date)between cast(@p_from_date as date) and cast(@p_to_date as date) ;

			if not exists (select * from dbo.rpt_delivery_collect_order where user_id = @p_user_id)
			begin
					
					insert into dbo.rpt_delivery_collect_order
					(
					    user_id
					    ,report_company
					    ,report_title
