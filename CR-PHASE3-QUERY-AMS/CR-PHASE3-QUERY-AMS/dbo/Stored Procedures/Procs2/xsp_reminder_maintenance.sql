CREATE PROCEDURE dbo.xsp_reminder_maintenance
(
	@p_eod_date				datetime
	,@p_cre_date			datetime
...
begin
	...
	where	email_notification_type = 'MNTANC' -- Arga 03-Nov-2022 ket : split for wom (-/+) --in ('EXPDOC','MNTANC','OPNAME')
	and		is_active = '1'
	...
	begin
		...
		if @reminder_type = 'MNTANC'
		begin
		    if exists (select	1
						from	dbo.asset_maintenance_schedule
						where	maintenance_status <> 'DONE'
						and		datediff(day,cast(maintenance_date as date),cast(@p_eod_date as date)) between @start_days and @end_days
						)
				begin
					
					-- jika terdapat perubahan pada nama file dibawah, pastikan kondisi pada sp xsp_generate_reminder_data disesuaikan juga
					set @save_file_name = 'reminderdatamaintenanceasset_' + convert(char(8), getdate(), 112) + replace(convert(char(12), getdate(), 14),':','') + '.xlsx' ;
					
					-- send mail attachment based on setting ================================================
					--exec dbo.xsp_master_email_notification_broadcast @p_code			= 'MNTANC'
					--												,@p_doc_code		= ''
					--												,@p_attachment_flag = 1
					--												,@p_attachment_file = @save_file_name
					--												,@p_attachment_path = @save_file_path
					--												,@p_company_code	= @company_code
					--												,@p_trx_no			= ''
					--												,@p_trx_type		= 'MAINTENANCE ASET'
