...
CREATE PROCEDURE [dbo].[xsp_rpt_delivery_vehicle_bast]
(
...
BEGIN
	...
	delete	dbo.rpt_delivery_vehicle_bast
	...
	begin try
		...
		set	@report_title = 'Report Delivery Mobilisasi';
	...
	begin
			insert into rpt_delivery_vehicle_bast
			(
				user_id
				,report_company
				,report_title
			...
			select	distinct @p_user_id
					...
					,avi.colour
					,hast.plan_date
					,isnull(hast.handover_date,null)
			...
			where	hast.type in ('MOBILISASI')
					and hast.status in ('HOLD','POST')
					and hast.handover_date is not null
			...
			if not exists (select * from dbo.rpt_delivery_vehicle_bast where user_id = @p_user_id)
					begin
							
							insert into dbo.rpt_delivery_vehicle_bast
							(
							    user_id
							    ,report_company
							    ,report_title
