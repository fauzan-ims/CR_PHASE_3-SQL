CREATE procedure dbo.xsp_monitoring_asset_disposed_getrows
(
...
begin
	...
	from	dbo.asset ast
			...
			inner join dbo.disposal_detail dp on dp.asset_code = ast.code
			inner join dbo.disposal dps on dps.code = dp.disposal_code and dps.status = 'approve'
	where	ast.status			= 'disposed'
	and		ast.fisical_status	= 'disposed'
	...
	and		(
				...
				or convert(varchar(30),	dps.disposal_date, 103)	like '%' + @p_keywords + '%'
				or dps.reason_type								like '%' + @p_keywords + '%'
				...
				or dp.description								like '%' + @p_keywords + '%'
			)
	...
	select		ast.code		
				...
				,av.engine_no
				,av.chassis_no
				,convert(varchar(30),	dps.disposal_date, 103) 'disposal_date'
				,dps.reason_type			'reason'
	...
	order by	case
					when @p_sort_by = 'asc' then case @p_order_by
													 ...
													 when 4 then cast(dps.disposal_date as sql_variant)
													 when 5 then dps.reason_type
													 when 6 then ast.unit_city_name
