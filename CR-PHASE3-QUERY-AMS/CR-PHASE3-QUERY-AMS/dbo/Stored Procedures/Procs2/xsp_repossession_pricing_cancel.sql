CREATE PROCEDURE dbo.xsp_repossession_pricing_cancel
(
	@p_code			   nvarchar(50)
	--
	,@p_mod_date	   datetime
...
begin
	...
	begin try
		...
		(
			select	1
			from	dbo.repossession_pricing
			where	code					= @p_code
					and transaction_status	= 'HOLD'
		)
		begin
			select	@repossession_code	= asset_code
			...
			set		repossession_status_process	= ''
					--
					,mod_date					= @p_mod_date		
					,mod_by						= @p_mod_by			
					,mod_ip_address				= @p_mod_ip_address	
