CREATE PROCEDURE [dbo].[xsp_schedule_maintenance_asset_generate_master]
(
	@p_code				nvarchar(50)
	...
	begin try
		...
		begin
				...
				--begin
					...
					set @maintenance_date = dateadd(month, isnull(@month_maintenance_vhcl,1), cast(@date_purc as date)) ;-- set 1 bulan jika settingan tidak ada input cycle month
		...
		select 1 from dbo.asset_maintenance_schedule where asset_code = @p_code and service_code = 'ROUTINE'
					-- sepria 16092025: hanya masuk 1 schedule rutin ke asset saat masuk dari procurement
					if(@is_month_vhcl = '1')
					begin
						...
						begin
							...
							begin
								...
								if not exists (select 1 from dbo.asset_maintenance_schedule where asset_code = @p_code and service_code = 'ROUTINE')
								begin
