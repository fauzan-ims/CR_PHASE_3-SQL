CREATE PROCEDURE dbo.xsp_rpt_asset
(
...
BEGIN
	...
	INSERT INTO dbo.RPT_ASSET
			...
			--where	
					--ass.branch_code = case @p_branch_code
					--					  when 'ALL' then ass.branch_code
					--					  else @p_branch_code
					--				  end
			WHERE (@p_branch_code = 'ALL' OR ass.branch_code = @p_branch_code)
			...
			OPTION (RECOMPILE); -- Biar plan selalu fresh sesuai parameter
