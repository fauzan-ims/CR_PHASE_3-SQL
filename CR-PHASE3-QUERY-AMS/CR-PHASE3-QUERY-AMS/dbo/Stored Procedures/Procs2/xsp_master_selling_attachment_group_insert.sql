CREATE PROCEDURE [dbo].[xsp_master_selling_attachment_group_insert]
(
...
begin
	...
	(
		select	1
		from	dbo.master_selling_attachment_group
		where	description = @p_description
	)
	begin
		set	@msg = 'Description Already Exists.'
		raiserror (@msg, 16, -1)
	END
	...
	(
		select	1
		from	dbo.master_selling_attachment_group
		where	sell_type = @p_sell_type
	)
	begin
		set	@msg = 'Sell Type Already Exists.'
		raiserror (@msg, 16, -1)
	end
	...
	exec dbo.xsp_get_next_unique_code_for_table @p_code output, 
	                                            ...
	                                            @p_custom_prefix = N'MSAG',                 
	                                            @p_year = @year,                          
	                                            @p_month = @month,                        
	                                            @p_table_name = N'MASTER_SELLING_ATTACHMENT_GROUP',                    
	...
	insert into MASTER_SELLING_ATTACHMENT_GROUP
	(
		code
		,description
		,is_active
