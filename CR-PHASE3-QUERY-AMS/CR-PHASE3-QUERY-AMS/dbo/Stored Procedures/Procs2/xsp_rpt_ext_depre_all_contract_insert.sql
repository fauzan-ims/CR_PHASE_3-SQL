CREATE PROCEDURE dbo.xsp_rpt_ext_depre_all_contract_insert
AS
BEGIN
	...
	*/
		...
		--UPDATE dbo.ASSET
		--SET SALE_DATE = DATEADD(DAY,2,SALE_DATE)
		--WHERE CODE = '4120043478'
		...
		select		isnull(	 am.AGREEMENT_EXTERNAL_NO, case ass.STATUS
													...
													----WHEN 'SOLD' THEN ASS.AGREEMENT_EXTERNAL_NO
												end
						  )
		...
		inner join	dbo.XXX_ASSET_VEHICLE_AFTER_EOM_20250731							av on av.ASSET_CODE = ass.CODE
		left join	dbo.XXX_ASSET_DEPRECIATION_SCHEDULE_COMMERCIAL_AFTER_EOM_20250731	adsc on adsc.ASSET_CODE = ass.CODE and  convert(varchar(6), adsc.depreciation_date , 112) = convert(varchar(6), @p_cre_date , 112)  
		left join	ifinopl.dbo.rpt_ext_agreement_main ram on ass.agreement_external_no = ram.agreement_id
		left join	IFINOPL.dbo.XXX_AGREEMENT_ASSET_AFTER_EOM_20250731					aa on aa.FA_CODE = ass.CODE
																	  and aa.AGREEMENT_NO = ass.AGREEMENT_NO
																	  and aa.ASSET_STATUS = 'RENTED'
																	  and ram.AGREEMENT_ID =  ass.AGREEMENT_NO
		left join	IFINOPL.dbo.XXX_AGREEMENT_MAIN_AFTER_EOM_20250731					am on am.AGREEMENT_NO = aa.AGREEMENT_NO
		...
		--where		CAST(ass.CRE_DATE AS DATE)							  <= @p_cre_date
		--			and (
		...
		--				(
		--					'CANCEL', 'hold','RETURNED'--,'RETURNED','SOLD'
		--				)
		--			and (
		...
		(
			'STOCK', 'REPLACEMENT'
				...
				AND convert(char(6), ass.CRE_DATE, 112) <= convert(char(6), @p_cre_date, 112)
					--and ass.CODE not in
					--	(
					--		'1000.MIG.0001.0019', '2008.AST.2312.00004',  '2010.AST.2312.00002'--,  '2001.AST.2312.00001','2010.AST.2312.00001'
					--	)
					--AND ass.CODE NOT IN -- exclude dilakukan karena Asset tidak masuk di rekon bulan Juli karena termasuk asset rent
					--	(
					--		N'1000.AST.2402.00002',
					--		N'1000.AST.2403.00002',
					--		N'1000.AST.2403.00003',
					--		N'1000.AST.2403.00006',
					--		N'2038.AST.2403.00016',
					--		N'2038.AST.2403.00017',
					--		N'2038.AST.2403.00018'
					--	)
					AND ass.CODE NOT IN ( '4120039877', '4120039969', '4120034350', '4120034516', '4120034644', '4120034811', '4120038595',
											'4120038619', '4120034386', '4120034733', '4120035686', '4120035694', '4120035696', '4120035699',
											'4120035744', '4120037863', '4120041969', '4120043532', '1000.AST.2410.00001', '4120031363',
											'4120031866', '4120031873', '4120031877', '4120038327', '4120038328', '4120038167', '4120039550',
											'4120041916', '4120042039', '4120042141', '4120043395', '4120034882', '4120032896', '4120036559', '4120038332', '4120039366', '4120035687',
											'4120043533', '4120043534', '4120039867', '4120030616', '4120037873', '4120034110', '4120041814',
											'4120034540', '4120037577', '4120043396', '4120034145', '4120039053'
										)
		...
		(
			'STOCK', 'REPLACEMENT'
				...
				AND convert(char(6), ass.CRE_DATE, 112) <= convert(char(6), @p_cre_date, 112)
					...
					--		'1000.MIG.0001.0019', '2008.AST.2312.00004',  '2010.AST.2312.00002'--,  '2001.AST.2312.00001','2010.AST.2312.00001'
					--	)
					--AND ass.CODE NOT IN -- exclude dilakukan karena Asset tidak masuk di rekon bulan Juli
					--	(
					--		N'1000.AST.2402.00002',
					--		N'1000.AST.2403.00002',
					--		N'1000.AST.2403.00003',
		...
		begin -- temp imature
			...
			SELECT distinct DEPRECIATION_DATE FROM RPT_EXT_DEPRE_ALL_CONTRACT where DEPRECIATION_DATE > '2025-07-31' and DEPRECIATION_DATE <= '2028-02-28'
		...
		outer apply (
						...
						from	 IFINAMS.dbo.XXX_ASSET_DEPRECIATION_SCHEDULE_COMMERCIAL_AFTER_EOM_20250731 adsc with (nolock)
						where	 adsc.ASSET_CODE = ass.CODE
						group by adsc.ASSET_CODE
					)				  adsc
		--where		CAST(ass.CRE_DATE AS DATE)							  <= '2024-11-30'
		--			and (
		--					ass.SALE_DATE is null
		--					or ass.SALE_DATE					  > '2024-11-30'
		--				)
		--			and (
		--					ass.DISPOSAL_DATE is null
		--					or ass.DISPOSAL_DATE				  > '2024-11-30'
		--				)
		--			and ass.STATUS not in
		--(
		--	'CANCEL', 'hold','RETURNED'--,'RETURNED','SOLD'
		...
		--			and (
		--					ass.PERMIT_SELL_DATE is null
		--					or cast(ass.PERMIT_SELL_DATE as date) > '2024-11-30'
		--				)
		...
		--				)
					...
					--AND ass.ASSET_FROM = 'BUY'
					and adsc.DEPRECIATION_DATE					  <= '2025-07-31';
		...
		left join	IFINOPL.dbo.XXX_AGREEMENT_ASSET_20240731		   aa on aa.FA_CODE = ass.CODE
															 and aa.AGREEMENT_NO = ass.AGREEMENT_NO
															 and aa.ASSET_STATUS = 'RENTED'
															 --and ram.AGREEMENT_ID = ass.AGREEMENT_NO
		left join	IFINOPL.dbo.XXX_AGREEMENT_MAIN_AFTER_EOM_20250731		   am on am.AGREEMENT_NO = aa.AGREEMENT_NO
		...
		--SET SEQUENCE = '3802'
		--	,AGREEMENT_ID = 'UNIT STOCK'
		--WHERE ISNULL(SEQUENCE,'')=''
