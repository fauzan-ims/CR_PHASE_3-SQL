...
CREATE PROCEDURE dbo.xsp_rpt_sppa_with_payment_status
(
...
begin
	...
	delete dbo.rpt_sppa_with_payment_status
	...
	begin try
		...
		insert into dbo.rpt_sppa_with_payment_status
		(
		    user_id
		    ,sppa_no
		    ,report_company
		...
		select	distinct
				...
				,isnull(avi.merk_name,'-')
				,isnull(avi.type_item_name,'-')
				,isnull(avi.built_year,'-')
				,isnull(avi.plat_no,'-')
				,isnull(avi.colour,'-')
				,isnull(avi.chassis_no,'-')
				,isnull(avi.engine_no,'-')
				,'Premi dibayar per tahun, Nota Debit dibuat installment dikirim per tahun.'
				,coverage.cover
				,ire.year_period
