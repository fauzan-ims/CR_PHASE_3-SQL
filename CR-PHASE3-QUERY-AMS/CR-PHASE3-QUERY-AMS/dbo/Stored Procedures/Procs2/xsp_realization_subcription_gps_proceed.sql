CREATE PROCEDURE [dbo].[xsp_realization_subcription_gps_proceed] 
(
	@p_code					nvarchar(50)
	--
	,@p_cre_date			datetime
...
BEGIN
	...
	begin try
		...
		select	@payment_to				= mps.public_service_name
				...
				--(+) Raffy 2025/02/01 CR NITKU
				,@ext_vendor_nitku		= mps.NITKU
				,@ext_vendor_npwp_pusat = mps.NPWP_PUSAT
		...
		if @payment_status <> 'HOLD' AND @register_status <> 'REALIZATION'
		begin
			set @msg = 'Data already proceed.'
			raiserror(@msg ,16,-1)
		END
		ELSE IF @payment_status <> 'ON PROCESS' AND @register_status = 'REALIZATION'
		begin
			set @msg = 'Data already proceed.'
			raiserror(@msg ,16,-1)
		END
		...
		if @register_status = 'REALIZATION'  --(+)Raffy 20241028 Perubahan pada posisi update status, agar jika data terproses 2 kali, maka validasi akan muncul karena status sudah berubah
		begin
			update	dbo.register_main
			set		register_status						= 'PENDING'
					,payment_status						= 'APPROVE'
					,public_service_settlement_date		= @pbs_settlement_date
					,public_service_settlement_voucher	= @pbs_settlement_voucher
					,mod_date							= @p_mod_date
					,mod_by								= @p_mod_by
		...
		begin
			...
			set		 payment_status						= 'APPROVE'
					,delivery_status					= 'HOLD' -- (+)Inu 20250707 penambahan update status delivery_status
					,public_service_settlement_date		= @pbs_settlement_date
					,public_service_settlement_voucher	= @pbs_settlement_voucher
					,mod_date							= @p_mod_date
					,mod_by								= @p_mod_by
					,mod_ip_address						= @p_mod_ip_address
			where	code = @p_code ;
		end ;--(+)Raffy 20241028 Perubahan pada posisi update status, agar jika data terproses 2 kali, maka validasi akan muncul karena status sudah berubah
		...
		begin
			...
			SELECT @orig_amount_cr, @orig_amount_db
		...
		BEGIN
			SELECT @tax_file_type        = tax_file_type
				   ...
				   --(+) Raffy 2025/02/01 CR NITKU
				   ,@ext_vendor_nitku	 = nitku
				   ,@ext_vendor_npwp_pusat= npwp_pusat
			...
			begin
				...
				else if(@transaction_code IN ('RLZPPH','RLZPPH21')) --(+)raffy Agar Jika vendor personal, SL nya juga terkirim 
				begin
					if(@return_value > 0)
					begin
				...
				begin
                    ...
                    SET @ext_vendor_nitku		= ''
					SET @ext_vendor_npwp_pusat = ''
				end
			...
			exec dbo.xsp_payment_request_insert @p_code							= @code	output        
												...
												,@p_payment_source				= 'REALIZATION FOR PUBLIC SERVICE'
												,@p_payment_request_date		= @realization_date
												,@p_payment_source_no			= @p_code                 
												,@p_payment_status				= 'HOLD'                  
												,@p_payment_currency_code		= 'IDR'              
												,@p_payment_amount				= @realization_amount          
			...
			select  mt.sp_name
					...
					,mps.tax_file_address
					--,rm.faktur_no
					,isnull(rm.faktur_no,rm.realization_invoice_no) 'faktur_no' -- (+) Ari 2023-12-18 ket : req pah hary jika faktur null pakai invoice 
					,rm.faktur_date
					,mps.tax_file_type
					,ass.branch_code
					,ass.branch_name
			...
			from	dbo.master_transaction_parameter mtp 
					...
					inner join dbo.order_main om on (om.code collate Latin1_General_CI_AS = rm.order_code)
					inner join dbo.master_public_service mps on mps.code = om.public_service_code
					left join dbo.master_public_service_address mpsa on (mpsa.public_service_code = mps.code and mpsa.is_latest = '1')
					inner join dbo.asset ass on (ass.code = rm.fa_code)
			where	mtp.process_code = 'PMRL' --'JRPV'	
			...
			begin 
				...
				set @remarks = @transaction_name + ', ' + format (@orig_amount_db, '#,###.00', 'DE-de') +  ' for ' + @public_service_name
				...
				set @agreement_external_no = isnull(@agreement_external_no,@asset_code) -- (+) Ari 2023-11-20 ket : jika tidak ada no kontrak di asset pakai asset code info pak hary
		...
		--if  @public_service_settlement_amount = 0
		--begin
		--	select	@regis_status			= 'PENDING'
		--end
		--else
		--begin
		--	select	@regis_status			= 'REALIZATION PROCEED'
		--end

		set @delivery_receive_by = @name

		
		if @realization_amount = 0
		begin
			select @pbs_settlement_date			= @system_date
					,@pbs_settlement_voucher	= '-'
		end
		else
		begin
		    select @pbs_settlement_date			= null
					,@pbs_settlement_voucher	= null
		end

			if @reimburse_to_cust = '1' and @reimburse_fee <> '0'
			begin
				set @description_request = 'Invoice Registration ' + format (@reimburse_fee, '#,###.00', 'DE-de') + ' for ' + @fa_code + ' - ' + @item_name + '.'
				set @invoice_name = 'Invoice Registration ' + @agreement_external_no + ' ' + @plat_no
				exec dbo.xsp_ifinams_interface_additional_request_insert @p_id					= 0
																	 ...
																	 ,@p_invoice_date			= @realization_date
																	 ,@p_invoice_name			= @invoice_name --'Invoice Registration'
																	 ...
																	 ,@p_quantity				= 1
																	 ,@p_billing_amount			= @reimburse_fee
																	 ,@p_discount_amount		= 0
																	 ,@p_ppn_pct				= 0
																	 ,@p_ppn_amount				= 0
																	 ,@p_pph_pct				= 0
																	 ,@p_pph_amount				= 0
																	 ,@p_total_amount			= @reimburse_fee
																	 ,@p_request_status			= 'HOLD'
																	 ,@p_reff_code				= @p_code
																	 ,@p_reff_name				= 'REALIZATION'
																	 ,@p_settle_date			= null
																	 ,@p_job_status				= 'HOLD'
																	 ,@p_failed_remarks			= ''
																	 ,@p_cre_date				= @p_mod_date	  
		...
		--if @register_status = 'REALIZATION'
		--begin
		--	update	dbo.register_main
		--	set		register_status						= 'PENDING'
