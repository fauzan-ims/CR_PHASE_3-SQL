CREATE PROCEDURE dbo.xsp_repossession_main_for_inquiry_getrows
(
...
begin
	...
	where	rmn.exit_date is null
			...
			and rmn.repossession_status <> 'REPO I'
			...
			and	(
					...
					or	rmn.status like '%' + @p_keywords + '%'
					or	rmn.fisical_status like '%' + @p_keywords + '%'
					or	rmn.rental_status like '%' + @p_keywords + '%'
		...
		from		repossession_main rmn
					inner join dbo.asset ast on (ast.code = rmn.asset_code)
		where		rmn.exit_status is not null
					and rmn.branch_code	=	case @p_branch_code
												when 'ALL' then rmn.branch_code
												else @p_branch_code
											end
