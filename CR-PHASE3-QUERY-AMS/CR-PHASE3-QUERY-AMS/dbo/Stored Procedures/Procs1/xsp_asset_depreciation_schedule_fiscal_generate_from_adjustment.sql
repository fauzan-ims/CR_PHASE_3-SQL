...
CREATE PROCEDURE [dbo].[xsp_asset_depreciation_schedule_fiscal_generate_from_adjustment] 
(
	@p_code						nvarchar(50)
	--
	,@p_mod_date				datetime
...
begin
	...
	begin try
		...
		begin
			...
			select @purchase_date = isnull(min(depreciation_date),@purchase_date)
			from dbo.asset_depreciation_schedule_fiscal
			where asset_code = @asset_code
			...
			and convert(char(6),depreciation_date,112) >= convert(char(6),@purchase_date,112)
		...
		begin
			...
			begin
				...
				set @depre_amount = isnull(@depre_amount,0)
			...
			begin
				...
				begin
					...
					begin
						...
						set @sisa_bulan = case when @sisa_bulan = 0 then 1 end
