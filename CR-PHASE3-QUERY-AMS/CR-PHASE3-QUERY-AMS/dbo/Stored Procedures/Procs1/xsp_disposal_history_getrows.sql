CREATE procedure dbo.xsp_disposal_history_getrows
(
...
begin
	...
	from	dbo.disposal_history ds
			left join dbo.sys_general_subcode sgs on (ds.reason_type		 = sgs.code)
													 and   (sgs.company_code = ds.company_code)
	where	ds.branch_code		 = case @p_branch_code
									   when '' then ds.branch_code
									   else @p_branch_code
								   end
			and ds.location_code = case @p_location_code
									   when '' then ds.location_code
	...
	order by	case
					when @p_sort_by = 'asc' then case @p_order_by
													 when 1 then ds.code
													 when 2 then cast(disposal_date as sql_variant)
													 when 3 then ds.branch_name
