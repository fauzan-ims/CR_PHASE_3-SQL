CREATE PROCEDURE dbo.xsp_endorsement_main_getrows
(
	@p_keywords			   nvarchar(50)
	,@p_pagenumber		   int
	,@p_rowspage		   int
	,@p_order_by		   int
	,@p_sort_by			   nvarchar(5)
	,@p_branch_code		   nvarchar(50)
	,@p_endorsement_status nvarchar(10)
	,@p_insurance_type	   nvarchar(10)
)
as
begin
	declare @rows_count int = 0 ;

	if exists
	(
		select	1
		from	sys_global_param
		where	code	  = 'HO'
				and value = @p_branch_code
	)
	begin
		set @p_branch_code = 'ALL' ;
	end ;

	select	@rows_count = count(1)
	from	endorsement_main em
			inner join dbo.insurance_policy_main ipm on (ipm.code = em.policy_code)
			--inner join dbo.asset ass on (ass.code				  = ipm.fa_code)
			left join dbo.master_insurance mi on (mi.code		  = ipm.insurance_code)
	where	em.branch_code			  = case @p_branch_code
											when 'ALL' then em.branch_code
											else @p_branch_code
										end
			and em.endorsement_status = case @p_endorsement_status
											when 'ALL' then em.endorsement_status
											else @p_endorsement_status
										end
			and ipm.insurance_type	  = case @p_insurance_type
											when 'ALL' then ipm.insurance_type
											else @p_insurance_type
										end
			and (
					em.code													like '%' + @p_keywords + '%'
					or em.endorsement_request_code							like '%' + @p_keywords + '%'
					or em.branch_name										like '%' + @p_keywords + '%'
					or convert(varchar(30), em.endorsement_date, 103)		like '%' + @p_keywords + '%'
					or ipm.policy_no										like '%' + @p_keywords + '%'
					or	case em.endorsement_type
							when 'FN' then 'FINANCIAL'
							else 'NON FINANCIAL'
						end													like '%' + @p_keywords + '%'
					or em.endorsement_status								like '%' + @p_keywords + '%'
					or insurance_name										like '%' + @p_keywords + '%'
					or	case ipm.insurance_type
							when 'LIFE' then 'LIFE'
							when 'CREDIT' then 'CREDIT'
							else 'COLLATERAL'
						end													like '%' + @p_keywords + '%'
					--or ipm.fa_code											like '%' + @p_keywords + '%'
					--or ass.item_name										like '%' + @p_keywords + '%'
				) ;

	select		em.code
				,em.branch_name
				,convert(varchar(30), em.endorsement_date, 103) 'endorsement_date'
				,ipm.policy_no
				,case em.endorsement_type
					 when 'FN' then 'FINANCIAL'
					 else 'NON FINANCIAL'
				 end 'endorsement_type'
				,em.endorsement_status
				,insurance_name
				,case ipm.insurance_type
					 when 'LIFE' then 'LIFE'
					 when 'CREDIT' then 'CREDIT'
					 else 'COLLATERAL'
				 end 'insurance_type'
				--,ipm.fa_code
				--,ass.item_name 'fa_name'
				,@rows_count 'rowcount'
	from		endorsement_main em
				inner join dbo.insurance_policy_main ipm on (ipm.code = em.policy_code)
				--inner join dbo.asset ass on (ass.code				  = ipm.fa_code)
				left join dbo.master_insurance mi on (mi.code		  = ipm.insurance_code)
	where		em.branch_code			  = case @p_branch_code
												when 'ALL' then em.branch_code
												else @p_branch_code
											end
				and em.endorsement_status = case @p_endorsement_status
												when 'ALL' then em.endorsement_status
												else @p_endorsement_status
											end
				and ipm.insurance_type	  = case @p_insurance_type
												when 'ALL' then ipm.insurance_type
												else @p_insurance_type
											end
				and (
						em.code													like '%' + @p_keywords + '%'
						or em.endorsement_request_code							like '%' + @p_keywords + '%'
						or em.branch_name										like '%' + @p_keywords + '%'
						or convert(varchar(30), em.endorsement_date, 103)		like '%' + @p_keywords + '%'
						or ipm.policy_no										like '%' + @p_keywords + '%'
						or	case em.endorsement_type
								when 'FN' then 'FINANCIAL'
								else 'NON FINANCIAL'
							end													like '%' + @p_keywords + '%'
						or em.endorsement_status								like '%' + @p_keywords + '%'
						or insurance_name										like '%' + @p_keywords + '%'
						or	case ipm.insurance_type
								when 'LIFE' then 'LIFE'
								when 'CREDIT' then 'CREDIT'
								else 'COLLATERAL'
							end													like '%' + @p_keywords + '%'
						--or ipm.fa_code											like '%' + @p_keywords + '%'
						--or ass.item_name										like '%' + @p_keywords + '%'
					)
	order by	case
					when @p_sort_by = 'asc' then case @p_order_by
													 when 1 then em.code
													 when 2 then em.branch_name
													 when 3 then ipm.policy_no + em.endorsement_type
													 --when 3 then ipm.fa_code + ass.item_name
													 when 4 then cast(em.endorsement_date as sql_variant)
													 when 5 then insurance_name + ipm.insurance_type
													 when 6 then em.endorsement_status
												 end
				end asc
				,case
					 when @p_sort_by = 'desc' then case @p_order_by
													 when 1 then em.code
													 when 2 then em.branch_name
													 when 3 then ipm.policy_no + em.endorsement_type
													 --when 3 then ipm.fa_code + ass.item_name
													 when 4 then cast(em.endorsement_date as sql_variant)
													 when 5 then insurance_name + ipm.insurance_type
													 when 6 then em.endorsement_status
												   end
				 end desc offset ((@p_pagenumber - 1) * @p_rowspage) rows fetch next @p_rowspage rows only ;
end ;
