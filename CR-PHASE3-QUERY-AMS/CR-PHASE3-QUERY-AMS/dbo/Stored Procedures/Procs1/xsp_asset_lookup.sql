CREATE PROCEDURE dbo.xsp_asset_lookup 
(
	...
	from
			(
				select	ass.code
						...
						,ass.department_code
						,ass.department_name
						--,ass.sub_department_code
						--,ass.sub_department_name
						--,ass.units_code
						--,ass.units_name
						,av.built_year
						,ass.type_code
				...
				from	dbo.asset ass
						left join dbo.sys_general_subcode sgs on (sgs.code = ass.type_code)
						left join dbo.asset_vehicle av on (av.asset_code	= ass.code)
				where	status in ('REPLACEMENT', 'STOCK')
				...
				from	dbo.asset ass
						left join dbo.sys_general_subcode sgs on (sgs.code = ass.type_code)
						left join dbo.asset_he am on (am.asset_code	= ass.code)
				where	status in ('REPLACEMENT', 'STOCK')
						and ass.type_code	  = case @p_type_code
													when 'ALL' then ass.type_code
													else @p_type_code
