CREATE PROCEDURE dbo.xsp_asset_depreciation_post
(
	--@p_code				nvarchar(50)
...
BEGIN
	...
	begin try
		...
		--select modby dicomment sementara
		--select	@p_mod_by			= 'DPRPR'
		--		,@p_mod_ip_address	= '10.0.9.205'

		--if(cast(@p_month as int) <> month(@smonth))
		--begin
		--	set @msg = 'Selected month must be equal from sys month'
		--	raiserror(@msg,16,-1)
		--end

		
		---- Arga 12-Oct-2022 ket : for WOM to control back date based on setting (+) ====
		--set @date_depre = cast(@period_depre as date)
		--set @is_valid = dbo.xfn_date_validation(@date_depre)
		...
		exec dbo.xsp_efam_journal_depreciation_register @p_process_code			= 'DEPRE'
