CREATE PROCEDURE [dbo].[xsp_asset_as_replacement_approve]
(
	@p_code			   nvarchar(50)
	--
	,@p_mod_date	   datetime
...
begin
	...
	begin try
		...
		if @activity_status = 'APPROVAL ASSET AS REPLACEMENT'
		begin
	
			set @msg = 'Approval asset as replacement on process.';
		...
		begin
				set @interface_remarks = 'Approval Asset As Replacement ' + @p_code ;
				...
				exec dbo.xsp_ams_interface_approval_request_insert @p_code						= @request_code output
																   ...
																   ,@p_reff_no					= @p_code
																   ,@p_reff_name				= 'APPROVAL ASSET AS REPLACEMENT'
		...
		update	dbo.asset
		set		activity_status = 'APPROVAL ASSET AS REPLACEMENT'
		where	code = @p_code ;
