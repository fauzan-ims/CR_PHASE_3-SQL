CREATE PROCEDURE dbo.xsp_ams_interface_approval_request_getrows
(
...
begin
	...
	from	dbo.ams_interface_approval_request liar
	where	liar.branch_code		= case @p_branch_code
			...
			and (
					...
					or	liar.approval_status													like '%' + @p_keywords + '%'
				) ;
	...
	order by	case
					when @p_sort_by = 'asc' then case @p_order_by
														...
														when 7 then cast(liar.mod_date as datetime)
														when 8 then liar.request_status
													end
				end asc
				,case
