CREATE PROCEDURE dbo.xsp_asset_machine_update
(
...
begin
	...
	begin try
		if exists (select 1 from dbo.asset_machine where chassis_no  = @p_chassis_no and chassis_no <> '')
		begin
			set @msg = 'The chassis number already exists on another asset.';
			raiserror(@msg ,16,-1);	    
		end
		if exists (select 1 from dbo.asset_machine where engine_no  = @p_engine_no and engine_no <> '')
		begin
			set @msg = 'The machine number already exists in another asset.';
			raiserror(@msg ,16,-1);	    
		end
		if exists (select 1 from dbo.asset_machine where serial_no  = @p_serial_no and serial_no <> '')
		begin
