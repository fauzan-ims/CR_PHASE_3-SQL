CREATE PROCEDURE [dbo].[xsp_maintenance_getrow]
(
	@p_code NVARCHAR(50)
)
AS
BEGIN
	...
	(
		...
		where	reff_name			  = 'WORK ORDER'
			...
			outer apply(select isnull(apg.budget_amount,0) budget_amount from dbo.asset ast
			inner join ifinopl.dbo.agreement_asset ags on ags.asset_no = ast.asset_no and ags.fa_code = ast.code
			inner join ifinopl.dbo.application_asset_budget apg on apg.asset_no = ags.asset_no
			where ags.asset_no = ass.ASSET_NO AND apg.cost_code = 'mbdc.2208.000001'
	) validator
