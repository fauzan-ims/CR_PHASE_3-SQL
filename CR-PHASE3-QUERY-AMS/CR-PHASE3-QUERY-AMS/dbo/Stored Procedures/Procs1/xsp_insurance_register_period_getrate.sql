/*
	alterd : Nia, 22 Mei 2020
*/
CREATE PROCEDURE dbo.xsp_insurance_register_period_getrate
(
    @p_insurance_register_code nvarchar(50),
...
begin
    ...
    BEGIN TRY
        select @insurance_type				= insurance_type,
               ...
               @eff_rate					= ir.eff_rate,
               @tenor						= ir.year_period,-- am.tenor,-- hari - 26 august 2021 07:52 pm - ambil dari register, bisa beda dengan agreement
               @collateral_type				= ira.collateral_type,
               @collateral_category_code	= ira.collateral_category_code,
               @region_code					= isnull(ira.region_code, ''),
               @occupation_code				= isnull(ira.occupation_code, ''),
               @sum_insured					= ira.sum_insured_amount,
               --@is_authorized_workshop		= isnull(ir.is_authorized_workshop, '0'),
               --@is_comercial				= isnull(ir.is_commercial, '0'),
               @month_period				= ir.year_period, -- in month
               @periode_start_date			= dateadd(year, (@p_year_periode - 1), ir.from_date),
               @periode_end_date			= ir.to_date,
        ...
        where ir.code						= @p_insurance_register_code;
	
        if @insurance_type = 'NON LIFE'
        begin
			if not exists( select 1 from dbo.master_depreciation_detail where depreciation_code = @depreciation_code and tenor = @p_year_periode * 12)
			begin
			...
			begin
			    select @sum_insured = @sum_insured * rate/100.00
				from dbo.master_depreciation_detail 
				where depreciation_code = @depreciation_code and tenor = @p_year_periode * 12
			end
            ...
            begin  
                ...
                if ((@p_year_periode * 12) < @month_period) -- jika full maka tambah 1 tahun
                begin
                    set @periode_end_date = dateadd(year, 1, @periode_start_date);
                end;
				else
				begin
					set @periode_end_date = dateadd(month, @month_period, dbo.xfn_get_system_date()) ;
				end
                set @day_period = DATEDIFF(DAY, @periode_start_date, @periode_end_date);
			
                if (@day_in_year_code = 'ACTUAL')
                begin
                    set @day_in_year = DATEPART(dy, DATEFROMPARTS(@p_year_periode, 12, 31));
                end;
				else
                begin
