CREATE PROCEDURE dbo.xsp_efam_journal_change_category_register
(
...
begin
	...
	begin try
		...
		select	@branch_code			= ac.branch_code
				...
				,@from_item_code		= ac.from_item_code
				,@to_item_code			= ac.to_item_code
				,@trx_date				= ac.date
				,@item_group_code		= ast.item_group_code
		from	dbo.change_category	 ac
				inner join dbo.asset ast on ast.code = ac.asset_code
				left join dbo.master_category mcf on ac.from_category_code = mcf.code
				left join dbo.master_category mct on ac.to_category_code = mct.code
		where	ac.code				 = @p_change_category_code ;
		
		select	@from_uselife = mdcc.usefull
		...
		where	mc.code = @item_group_code
		
		select	@to_uselife = mdcc.usefull
		from	dbo.master_category mc 
			inner join dbo.master_depre_category_commercial mdcc on mc.depre_cat_commercial_code = mdcc.code
		where	mc.code = @item_group_code
		
		-- ambil description - by @p_process_code
		select @description = description + ' FOR ASSET NO '+ @asset_code
		...
		exec dbo.xsp_efam_interface_journal_gl_link_transaction_insert @p_code						= @gllink_trx_code output
																	   ...
																	   ,@p_reff_source_name			= 'FIXED ASSET CHANGE CATEGORY'
																	   ,@p_transaction_type			= 'FAMCCT'
																	   ---
		...
		if @p_process_code = 'CHCYKVS'
		begin
		    
			if @from_uselife < @to_uselife
			begin
				set @exclude_order_key = 10
				...
				where	process_code		 = @p_process_code
						...
						and mtp.order_key	<> @exclude_order_key
				order by mtp.order_key ;
			end
			else
			begin
				set @exclude_order_key = 9

				declare c_inf_jour_gl cursor fast_forward for
				select	mt.sp_name
						,case
							when mtp.order_key in (7,8,10)
								then case mtp.debet_or_credit when 'DEBIT' then 'CREDIT' else 'DEBIT' end
							else mtp.debet_or_credit
						end
						,mtp.gl_link_code
						,mt.transaction_name
						,mtp.order_key
		...
		begin
			...
			begin
				...
				exec @sp_name @p_company_code
							  ,@p_change_category_code
							  ,@asset_code ;
				
				if @trx_code in ('BJA','BJA2')
				begin
				    delete @myTampTable
					insert	@myTampTable
					(
				...
				begin
					...
					if @order_key in (7,8) and @p_process_code <> 'CHCYKVS' and @base_amount < 0
					begin
						set @amount = abs(@amount)
						set @base_amount = @amount * @exch_rate ;
					    set @orig_amount_db = 0 ;
					...
					if @order_key in (9,10) and @p_process_code = 'CHCYKVS' and @base_amount < 0
					begin
						set @amount = abs(@amount)
						set @base_amount = @amount * @exch_rate ;
					    set @orig_amount_db = 0 ;
				...
				set @detail_desc  = @description				

				-- Arga 28-Oct-2022 ket : auto get value from master category (+)
				...
				if @order_key in (1,6)
					select @gl_link_code = dbo.xfn_get_gl_code_from_category_by_item(@from_item_code, @p_company_code, @gl_link_code)
				if @order_key = 2
					select @gl_link_code = dbo.xfn_get_gl_code_from_item_grup_gl(@from_item_code, @p_company_code, @gl_link_code)
				if @order_key = 3
					select @gl_link_code = dbo.xfn_get_gl_code_from_item_grup_gl(@to_item_code, @p_company_code, @gl_link_code)
				else if @order_key in (4,5,7,8)
					select @gl_link_code = dbo.xfn_get_gl_code_from_category_by_item(@to_item_code, @p_company_code, @gl_link_code)
