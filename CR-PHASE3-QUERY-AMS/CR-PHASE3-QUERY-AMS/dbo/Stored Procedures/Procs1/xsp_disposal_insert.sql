CREATE PROCEDURE dbo.xsp_disposal_insert
(
...
begin
	...
	set @is_valid = dbo.xfn_date_validation(@p_disposal_date)
	...
	else if (@p_disposal_date > dbo.xfn_get_system_date() )
	begin
		set @msg = 'The Disposal Date must be less than system date.';
		raiserror(@msg ,16,-1);	    
	end
		
	-- Arga 06-Nov-2022 ket : request wom back date only for register aset (+)
	if datediff(month,@p_disposal_date,dbo.xfn_get_system_date()) > 0
	begin
