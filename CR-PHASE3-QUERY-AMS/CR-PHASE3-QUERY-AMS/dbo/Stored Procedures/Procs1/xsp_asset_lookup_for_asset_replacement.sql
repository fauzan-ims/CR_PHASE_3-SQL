CREATE PROCEDURE [dbo].[xsp_asset_lookup_for_asset_replacement]
(
...
begin
	...
	from
			(
				...
				where	ass.status			  = 'REPLACEMENT'
						and isnull(ass.rental_status, '') = ''
						and isnull(ass.fisical_status, '') = 'ON HAND'
						and isnull(ass.process_status, '') = ''
						and	isnull(ass.activity_status, '') = ''
						and	ass.code <> @p_fa_code
