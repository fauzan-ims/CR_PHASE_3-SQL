CREATE PROCEDURE dbo.xsp_adjustment_asset_lookup
(
...
begin
	...
	where	ass.code not in
			(
				...
				where	md.status_received	  = 'SENT'
						...
						or mt.status in ('HOLD', 'ON PROCESS')
			...
			and ass.code not in
				(
					...
					where	ds.status in ('HOLD', 'ON PROCESS')
			...
			and ass.code not in
				(
					...
					where	status in ('HOLD', 'ON PROCESS')
			...
			and ass.code not in
				(
					...
					where	sl.status in ('HOLD', 'ON PROCESS')
	...
	select	ass.code
			...
			,ass.original_price
			,ass.purchase_price
			,convert(varchar(30), ass.purchase_date, 103) 'new_purchase_date'
			,avh.plat_no
			,@rows_count 'rowcount'
	from	dbo.asset ass
	left join dbo.asset_vehicle avh on (avh.asset_code = ass.code)
