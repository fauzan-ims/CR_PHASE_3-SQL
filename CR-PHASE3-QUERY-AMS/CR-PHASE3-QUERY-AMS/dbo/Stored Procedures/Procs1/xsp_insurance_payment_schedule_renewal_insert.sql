CREATE PROCEDURE dbo.xsp_insurance_payment_schedule_renewal_insert
(
	@p_code						   nvarchar(50) output
	,@p_payment_renual_status	   nvarchar(10)
	,@p_policy_code				   nvarchar(50)
	,@p_year_period				   int
	,@p_policy_eff_date			   datetime
	,@p_policy_exp_date			   datetime
	,@p_sell_amount				   decimal(18, 2)
	,@p_discount_amount			   decimal(18, 2)
	,@p_buy_amount				   decimal(18, 2)
	,@p_adjustment_sell_amount	   decimal(18, 2)
	,@p_adjustment_discount_amount decimal(18, 2)
	,@p_adjustment_buy_amount	   decimal(18, 2)
	,@p_total_amount			   decimal(18, 2)
	,@p_ppn_amount				   decimal(18, 2)
	,@p_pph_amount				   decimal(18, 2)
	,@p_total_payment_amount	   decimal(18, 2)
	--
	,@p_cre_date				   datetime
	,@p_cre_by					   nvarchar(15)
	,@p_cre_ip_address			   nvarchar(15)
	,@p_mod_date				   datetime
	,@p_mod_by					   nvarchar(15)
	,@p_mod_ip_address			   nvarchar(15)
)
as
begin
	declare @msg			nvarchar(max)
			,@branch_code	nvarchar(50)
			,@year			nvarchar(2)
			,@month			nvarchar(2) ;

	set @year = substring(cast(datepart(year, @p_cre_date) as nvarchar), 3, 2) ;
	set @month = replace(str(cast(datepart(month, @p_cre_date) as nvarchar), 2, 0), ' ', '0') ;

	begin TRY
		select	@branch_code = branch_code
		from	dbo.insurance_policy_main
		where	code = @p_policy_code;

		exec dbo.xsp_get_next_unique_code_for_table @p_unique_code = @p_code output
											,@p_branch_code = @branch_code
											,@p_sys_document_code = N''
											,@p_custom_prefix = 'AMSPRN'
											,@p_year = @year
											,@p_month = @month
											,@p_table_name = 'INSURANCE_PAYMENT_SCHEDULE_RENEWAL'
											,@p_run_number_length = 6
											,@p_delimiter = '.'
											,@p_run_number_only = N'0' ;

		insert into insurance_payment_schedule_renewal
		(
			code
			,payment_renual_status
			,policy_code
			,year_period
			,policy_eff_date
			,policy_exp_date
			,sell_amount
			,discount_amount
			,buy_amount
			,adjustment_sell_amount
			,adjustment_discount_amount
			,adjustment_buy_amount
			,total_amount
			,ppn_amount
			,pph_amount
			,total_payment_amount
			--
			,cre_date
			,cre_by
			,cre_ip_address
			,mod_date
			,mod_by
			,mod_ip_address
		)
		values
		(	@p_code
			,@p_payment_renual_status
			,@p_policy_code
			,@p_year_period
			,@p_policy_eff_date
			,@p_policy_exp_date
			,@p_sell_amount
			,@p_discount_amount
			,@p_buy_amount
			,@p_adjustment_sell_amount
			,@p_adjustment_discount_amount
			,@p_adjustment_buy_amount
			,@p_total_amount
			,@p_ppn_amount
			,@p_pph_amount
			,@p_total_payment_amount
			--
			,@p_cre_date
			,@p_cre_by
			,@p_cre_ip_address
			,@p_mod_date
			,@p_mod_by
			,@p_mod_ip_address
		) ;
	end try
	begin catch
		declare @error int ;

		set @error = @@error ;

		if (@error = 2627)
		begin
			set @msg = dbo.xfn_get_msg_err_code_already_exist() ;
		end ;

		if (len(@msg) <> 0)
		begin
			set @msg = 'V' + ';' + @msg ;
		end ;
		else
		begin
			if (error_message() like '%V;%' or error_message() like '%E;%')
			begin
				set @msg = error_message() ;
			end
			else 
			begin
				set @msg = 'E;' + dbo.xfn_get_msg_err_generic() + ';' + error_message() ;
			end
		end ;

		raiserror(@msg, 16, -1) ;

		return ;
	end catch ; 
end ;

