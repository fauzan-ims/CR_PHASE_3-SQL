CREATE PROCEDURE dbo.xsp_dashboard_get_so_asset_nasional
(
...
begin
	...
	and op.status = 'POST'
	and op.opname_date between cast(year(dbo.xfn_get_system_date()) as char(4)) + '-' + right(convert(char(6),dbo.xfn_get_system_date(),112) - 1,2) + '-' +  cast(26 as char(2))
	and cast(year(dbo.xfn_get_system_date()) as char(4)) + '-' + right(convert(char(6),dbo.xfn_get_system_date(),112),2) + '-' +  cast(day(dbo.xfn_get_system_date()) as char(2))
