...
CREATE PROCEDURE [dbo].[xsp_generate_auto_skn_no]
(
...
begin
	...
	begin
		set @query = '	select	@next_run_numberOUT = replace(str(cast((isnull(max(substring(' + @p_column_name + ', 19, ' + cast(@p_run_number_length as nvarchar) + ')), 0) +1) as nvarchar), ' + cast(@p_run_number_length as nvarchar) + ', 0), '' '', ''0'')
				from	' + @p_table_name + '
		...
		*/
		execute sp_executesql @query
			...
			set @p_unique_code = @p_branch_code + @p_delimiter + @p_jkn + @p_delimiter + @p_year + @p_month + @p_delimiter + @next_run_number;
		--end
