CREATE PROCEDURE dbo.xsp_efam_journal_sale_register
(
...
begin
	...
	begin try
		...
		set @reff_source_name = 'Sell asset for ' + @p_sale_code
		...
		begin 
				...
				--if (@buy_type = 'By Unit')
				--begin
				--	if @p_process_code = 'SELL'
				--	begin
				--		-- cursor assets
				--		declare c_sale_asset cursor fast_forward for
				--		select	distinct
				--				0
				--				--,slb.asset_code
				--				,ast.item_name
				...
				--				--inner join dbo.sale_bidding sbd on sbd.sale_code = sd.sale_code
				--				--inner join dbo.sale_bidding_detail slb on slb.asset_code = sd.asset_code
				--				inner join dbo.asset ast on ast.code = sd.asset_code
				--		where	sd.sale_code = @p_sale_code 
				--		--and		sbd.is_winner = '1'
				--		and		sd.gain_loss >= 0;	
				--	end		
				--	else
		...
		--	set @validation_mssg += '. Your Transaction Journal Number is ' + @gllink_trx_code
		--	if (@validation_mssg <> '')
		--	begin
