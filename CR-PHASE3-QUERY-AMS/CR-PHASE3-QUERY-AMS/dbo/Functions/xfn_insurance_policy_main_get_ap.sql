CREATE function [dbo].[xfn_insurance_policy_main_get_ap]
(
...
begin
	...
	begin
		select	@total_buy_amount = isnull(sum(ipac.initial_buy_amount), 0)
		--isnull(sum(ipac.initial_buy_amount - (ipac.initial_discount_amount + ipac.initial_discount_ppn - ipac.initial_discount_pph)),0)
